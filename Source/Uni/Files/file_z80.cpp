...
*/
static void write_compressed_page ( FD& fd, uint8 flag, CoreByte const* q, uint qsize ) throws //FileError,bad_alloc
{
	xlogIn("write_compressed_page(%i)", int(flag));

	assert( qsize>=1 kB && qsize<=64 kB );

	CoreByte const* qe = q + qsize;
	...
	while( q<qe )
	{
		...
		{
			int n=1; while( n<255 && q<qe && uint8(*q)==c ) { n++; q++; }
...
static void read_compressed_page( FD& fd, uint qsize, CoreByte* z, uint zsize ) throws
{
	...
	{
		c=*q++;									// read next byte
		if(c!=0xed)  { store(c); continue; }	// single byte
		if(q==q_end) { store(c); continue; } 	// rare case: last source byte is 0xed

		c=*q++;									// read next byte
		if(c!=0xed) { store(0xed); q--; continue; };

		n=*q++; c=*q++;							// read count & char
		if((z+n)>z_end) goto e;
		if(q>q_end) goto e;
...
*zxsp*	14		SPECTRA ram[1]		SPECTRA ram[1]		SPECTRA ram[1]
	...
	*/
	...
	if(varying_ramsize)
	{
		...
		{
			if(pages & (1<<i)) { write_compressed_page( fd, 3+i, &ram[addr], 0x400<<i ); addr += 0x400<<i; }
		}
	}
	else
	{
		...
		else					// paged memory		note: should work for 256k-Scorpion and Timex too
		{
			if(mmu->hasPort7ffd() && Mmu128kPtr(mmu)->port7ffdIsLocked())
			{
				...
				CoreByte noram[0x4000]; memset(&noram[0],0,sizeof(CoreByte)*0x4000);
				for(uint i=0;i<n;i++)
			...
			{
				for(uint i=0;i<n;i++)
					write_compressed_page( fd, i+3, &ram[i*0x4000], 0x4000 );
			}
		}
	}
...
void Machine::loadZ80_attach_joysticks(uint z80head_im)
{
	...
	{
		if(joysticks[i] && joysticks[i]->isConnected()) { j->insertJoystick(idx,i); break; }
	}
}
...
*/
void Machine::loadZ80(FD &fd) noexcept(false) /*FileError,DataError*/
{
