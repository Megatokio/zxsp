...
void RzxBlock::scan_ucbu()
{
	...
	while( i+4 <= busize )
	{
		...
		{
			#ifdef XLOG
			  nframes++; mini=min(mini,in); maxi=max(maxi,in); gesi+=in; numi++;
			#endif
			i += 4 + in;
		}
...
//		if(ic==0)    { xlogline("frame with instr. count = 0. (truncated)"); goto x; }
		...
		if(ic>20000) { xlogline("instr. count too high: %u. (truncated)", ic); goto x; }	// 3.5e6/50/4 = 17500

		// 3.5e6/50/(28/2)*0.8 = 4000		denk: contended ram: *0.8 ?
		// 3.5e6/60/(28/2)*0.8 = 3333		60 Hz
		// 3.25e6/60/(28/2)    = 3869		b&w machines
		if(ic>9 && ic<4000 && num_frames && i<ucsize) xlogline("instr. count too low: %u. (ignored)",ic); // just log
...
//		}
		...
		#ifdef XLOG
		  if(num_frames) minic = min(minic,ic); maxic=max(maxic,ic); gesic+=ic;
		#endif
...
//		return;						// => don't store a frame BUMMER: may be needed for INT after loading a snapshot!
	}
...
//	if( fpos!=0 && (cnt==0 || (cnt == epos-apos && memcmp(&ucbu[apos],&ucbu[fpos+4],cnt)==0)) ) // picky other emulators…
//	if( fpos!=0 && cnt == epos-apos && memcmp(&ucbu[apos],&ucbu[fpos+4],cnt)==0 ) // non-compliant picky other emulators…
//	{
...
void RzxBlock::startRecording()
{
	...
	else if(epos <= fpos)	// repeated frame
	{
		...
		if(fpos + 4 + ipos-apos > ucmax)
			resize_ucbu(fpos+4+20000);
	...
	if(fpos+4+9999 > ucmax)
		resize_ucbu(ucmax+20000);
...
/*	ANY TIME: read Input Recording Block from rzx file
	...
	throw FileError: real problem, not just EndOfFile. the block is empty.
...
*/
void RzxBlock::readInputRecordingBlock(FD& fd, uint32 blen) throws // FileError,DataError
{
	...
	if(flags&2)	// compressed data
	{
		...
		catch(DataError& e)
		{
			scan_ucbu();
			rewind();
			if(csize==blen) throw;		// throw zlib error only if not after EndOfFile error
...
*/
void RzxBlock::readSnapshotBlock(FD& fd, uint32 blen, cstr filename) throws // FileError,DataError
{
	...
	char ext[] = "1234"; fd.read_bytes(ext,4);	// filename extension
	...
	else if(flags&2)	// compressed snapshot
	{
		...
		create_dir(tmpdir);
		ssfn = catstr( tmpdir, basename_from_path(filename),
