...
/*	DCK files keeps information about memory content of various Timex memory expansions,
	...
	255: 	HOME bank:	mainly useless, HOME content is typically stored in a Z80 file
						however, using this bank ID you may replace content of Timex HOME ROM or turn Timex HOME ROM into RAM
	...
	That's all if only one bank is stored in the DCK file. Else a new 9-byte header for the next bank follows, and so on.
*/
...
uint32 hashes[] =
{
	...
	// these are the known official ones:
		0x711BC8B4,0x291D4645,		// Androids,Budgeter
		0x7AF2664A,0x64DACE8C,		// Casino1,CrazyBugs,
		0xCCE6105F,0xFCF48C07,		// FlightSimulator,Pinball,
		0x6CE0D15B,0xA764667E,		// StatesAndCapitals,Penetrator,
	// non-releases:
		0xC75FD9A9,0x70D353F3,		// Backgammon,Chess
		0x524C549B,					// Gyruss,
		0x7EBC77C9,0xF8A68738,		// HoraceAndTheSpiders,HungryHorace,
		0x2BC430ED,0x689A1AA6,		// LocoMotion,MontezumasRevenge,
		0xF8CF0C1C,0x63CD4488,		// Planetoids,Popeye
		0xAC486933,0xEA9A6A7F,		// QBert,ReturnOfTheJediDeathStarBattle,
		0xA00FBEDB,0x2014FE7C,		// ShadowOfTheUnicorn,SpaceRaiders,
		0x4F2AD26C,0x851827A0,		// StarWarsTheArcadeGame,Swordfight,
	// utilities etc.:
		0xE00804AD,0xC34D5E38,		// EToolkit,MTerm
		0xAE8E3E3D,					// Rwp32,
		0x58995A0B,					// SuperHotZDisassemblerV251: AROS and EXROM versions have identical roms
		0x2109D635,0x61A8EA54,		// TaswordII,TimeWord
		0x82FA73FF,0xF65BC8D5,		// VuCalc,VuFile,
		0xEE4CAFA5,0x4163E796,		// ZebraOS64,Tc2048Emu
		0xAEDD9AE3,					// JupiterAceEmu,
		0x4C1FDCC8,0x845E7691 		// ZxseEmu,ZxspEmu
};
...
{
	union{ uint8 bu8[0x2000]; uint64 bu64[0x0400]; };
	...
	home_r(0),  home_w(0),  home_d(0)
{
	...
	try
	{
		...
		while(fd.file_remaining()>=9)
		{
			...
			{
				dock_r=r; dock_w=w; dock_d=d;

				for(int i=0;i<8;i++)
				{
					if(d&(1<<i)) xxlogline("  reading block");
//					if(r&(1<<i)) dock[i].grow(0x2000);						// bank present => allocate ram
					if(r&(1<<i)) dock[i]=new Memory(machine,usingstr("TCC Dock Bank %i",i),0x2000);	// bank present => allocate ram
			...
			{
				exrom_r=r; exrom_w=w; exrom_d=d;

				for(int i=0;i<8;i++)
				{
					if(d&(1<<i)) xxlogline("  reading block");
//					if(r&(1<<i)) exrom[i].grow(0x2000);						// bank present => allocate ram
					if(r&(1<<i)) exrom[i]=new Memory(machine,usingstr("TCC Exrom bank %i",i),0x2000);	// bank present => allocate ram
		...
		for(TccRomId i=(TccRomId)0; i<NELEM(hashes); i=(TccRomId)(i+1)) { if(hash32==hashes[i]) { id=i; break; } }
...
void TccRom::saveAs(cstr filepath)
{
	...
	{
		save_as(filepath);
		delete[] fpath; fpath = newcopy(filepath);
...
void TccRom::save_as(cstr fpath) throws // AnyError
{
	...
	if(dock_r)
	{
		...
		for(int8 m=1; m; m+=m) fd.write_uint8(r&m ? (w&m) + (d&m)*2 : 0x00);	// 8 chunk flags
		for(int i=0;i<8;i++) { if(d&(1<<i)) { Z80::c2b(dock[i].getData(),bu,0x2000); fd.write_bytes(bu,0x2000); } }
	}
	...
	if(exrom_r)
	{
		...
		for(int8 m=1; m; m+=m) fd.write_uint8(r&m ? (w&m) + (d&m)*2 : 0x00);	// 8 chunk flags
		for(int i=0;i<8;i++) { if(d&(1<<i)) { Z80::c2b(exrom[i].getData(),bu,0x2000); fd.write_bytes(bu,0x2000); } }
	}
}
