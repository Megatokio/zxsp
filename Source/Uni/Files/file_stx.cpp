...
struct StxHeader
{
	enum
	{	// flags:
		ALTERNATETIMINGS = 1,	// The emulated Spectrum uses alternate timings (one cycle later than normal timings). If reset, the emulated Spectrum uses standard timings.
...
uint32
	StxidZ80		= ID('Z', '8', '0', 'R'),
	StxidSpecRegs	= ID('S', 'P', 'C', 'R'),
	StxidAtaSp		= ID('Z', 'X', 'A', 'T'),
	StxidAtaSpRam	= ID('A', 'T', 'R', 'P'),
	StxidAY			= ID('A', 'Y',  0 ,  0 ),
	StxidCF			= ID('Z', 'X', 'C', 'F'),
	StxidCFRam		= ID('C', 'F', 'R', 'P'),
	StxidCovox		= ID('C', 'O', 'V', 'X'),
	StxidBeta128	= ID('B', '1', '2', '8'),
	StxidBetaDisk	= ID('B', 'D', 'S', 'K'),
	StxidCreator	= ID('C', 'R', 'T', 'R'),
	StxidDock		= ID('D', 'O', 'C', 'K'),
	StxidGS			= ID('G', 'S',  0 ,  0 ),
	StxidGSRamPage	= ID('G', 'S', 'R', 'P'),
	StxidKeyboard	= ID('K', 'E', 'Y', 'B'),
	StxidIf1		= ID('I', 'F', '1',  0 ),
	StxidIf2Rom		= ID('I', 'F', '2', 'R'),
	StxidJoystick	= ID('J', 'O', 'Y',  0 ),
	StxidMdrCart	= ID('M', 'D', 'R', 'V'),
	StxidMouse		= ID('A', 'M', 'X', 'M'),
	StxidMultiface	= ID('M', 'F', 'C', 'E'),
	StxidOpus		= ID('O', 'P', 'U', 'S'),
	StxidOpusDisk	= ID('O', 'D', 'S', 'K'),
	StxidPlus3		= ID('+', '3',  0 ,  0 ),
	StxidPlus3Disk	= ID('D', 'S', 'K',  0 ),
	StxidPlusD		= ID('P', 'L', 'S', 'D'),
	StxidPlusDDisk	= ID('P', 'D', 'S', 'K'),
	StxidRamPage	= ID('R', 'A', 'M', 'P'),
	StxidRom		= ID('R', 'O', 'M',  0 ),
	StxidTimex		= ID('S', 'C', 'L', 'D'),
	StxidSIDE		= ID('S', 'I', 'D', 'E'),
	StxidSpecDrum	= ID('D', 'R', 'U', 'M'),
	StxidTape		= ID('T', 'A', 'P', 'E'),
	StxidUSpeech	= ID('U', 'S', 'P', 'E'),
	StxidZxPrinter	= ID('Z', 'X', 'P', 'R'),
...
struct StxZ80 : StxBlock
{
	...
	uint8	im;					// 0, 1 or 2
	uint32	cpu_cycle;			// The t-states value at the time the snapshot was made. This counts up from zero to the maximum number of t-states per frame for the specific Spectrum model.
	uint8	int_time_cc;		// The number of t-states left on restart when an interrupt can occur. This is used to support interrupt re-triggering properly.
								// On the Spectrum, the ULA holds the INTREQ line of the Z80 low for up to 48 t-states (depending on the model). If interrupts are enabled during this time, the Z80 will accept the request and invoke the appropriate interrupt service routine.
	...
	uint8	flags;				// EILAST	The last instruction executed was an EI instruction or an invalid $DD or $FD prefix.
								// HALTED	The last instruction executed was a HALT instruction. The CPU is currently executing NOPs and will continue to do so until the next interrupt occurs.
								// This flag is mutually exclusive with EILAST.
	uint16	mem_ptr;			// Internal Z80 register used to generate bits 5 and 3 of the F register after executing a BIT x,(HL) instruction. Set to 0 (zero) if not supported.
};
...
struct StxSpecRegs : StxBlock
{
	...
	union
	{
	uint8	port_1ffd;		// The current value of port $1ffd which controls additional memory paging features on the Spectrum +2A/+3 and the ZS Scorpion.
							// Should be set to 0 (zero) for other models.
	uint8	port_eff7;		// The current value of port $eff7 on the Pentagon 1024 which controls access to the additional memory. Not used on other models.
	};
	uint8	port_fe;		// The last value of written to port $fe. Only bits 3 and 4 (the MIC and EAR bits) are guaranteed to be valid.
							// Use 'border' to set the current border colour.
...
// Sami Vehmaa's ZXATASP IDE hard disk and CompactFlash card interface for the 16K, 48K, 128, +2, +2A and +3 ZX Spectrums.
// This block will be followed by zero or more ZXSTATARAM blocks which store the contents of the interfaces onboard memory.
struct StxAtaSp : StxBlock
{
	...
	uint8	active_page;	// Specifies the RAM page currently paged into $0000 - $3fff, or 255 if no paging is currently active.
};
...
{
	enum{ COMPRESSED = 1 };	// The RAM page data (beginning from chData) will be compressed using the Zlib compression library.

	uint16	flags;			// COMPRESSED
	uint8	page_no;		// Page number of this 16KB RAM page. For the 128KB version of the ZXATASP interface, this is 0 - 7. For the 512KB version, this is 0 - 31.
	uint8	data[1];		// The actual compressed or uncompressed memory page data. When uncompressed, this member is exactly 16KB (16,384) bytes in size.
};
//	Note: when loading Zlib compressed pages, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTATARAM ) - sizeof( ZXSTBLOCK ) - 1 );
...
// This block will be followed by zero or more ZXSTCFRAM blocks which store the contents of the interface's onboard memory.
...
struct StxCF : StxBlock
{
	...
	uint8	num_rampages;	// The amount of RAM installed in this ZXCF interface, in units of 16KB. Normally either 32 (512KB version) or 64 (1024KB version).
};
...
struct StxCFRam : StxBlock
{
	...
	uint16	flags;			// COMPRESSED
	uint8	page_no;		// Page number of this 16K RAM page. For the 512KB version of the ZXCF interface, this is 0 - 31. For the 1024KB version, this is 0 - 63.
	uint8	data[1];		// The actual compressed or uncompressed memory page data. When uncompressed, this member is exactly 16KB (16,384) bytes in size.
};
// When loading Zlib compressed pages, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTCFRAM ) - sizeof( ZXSTBLOCK ) - 1 );
...
struct StxBeta128 : StxBlock
{
	enum
	{
		...
		SEEKLOWER   = 16,	// The WD179x FDC's current seek direction is towards lower cylinder numbers. Otherwise, it is towards higher ones.
		COMPRESSED  = 32	// If a custom TR-DOS ROM is embedded in this block, it has been compressed with the Zlib compression library.
	};
	...
	uint8	rom_data[1];	// A Zlib compressed or uncompressed custom TR-DOS ROM (if one was installed). The uncompressed ROM size is always 16,384 bytes.
};
// When loading Zlib compressed ROMs, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTBETA128 ) - 1 - sizeof( ZXSTBLOCK ));
...
struct StxBetaDisk : StxBlock
{
	...
	{	// Flags
		...
		COMPRESSED	= 2,	// If ZXSTBDF_EMBEDDED is also set, the disk image in this block is compressed using the Zlib compression library.
	...
	union
	{
	char	filename[1];	// The file name of the disk image which should be opened and inserted into this drive, if the disk image is not embedded.
	uint8	disk_image[1];	// If the disk image is embedded, this member is the (possibly compressed) disk image to be inserted into this drive.
	};
};
...
struct StxDock : StxBlock
{
	enum
	{
		...
		EXROMDOCK	= 4		// 1: The page is from the DOCK bank, 0: the page is from the EXROM bank.			NOTE: was 3 on the SZX webpage!
	};

	uint16	flags;			// COMPRESSED | RAM | EXROMDOCK
	uint8	page_no;		// Page number of this 8K RAM page in the applicable bank (DOCK or EXROM). This can range from 0-7, and pages are only written if they are provided by an expansion cartridge (i.e. no page is written for the Timex ROM 1 in the EXROM bank at page 0).
							// If the machine being emulated is a Spectrum SE, there will be 8 DOCK pages and 8 EXROM pages of RAM.
	uint8	data[1];		// The actual compressed or uncompressed memory page data. When uncompressed, this member is exactly 8KB (8,192) bytes in size.
};
// When loading Zlib compressed pages, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTDOCK ) - sizeof( ZXSTBLOCK ) - 1 );
...
struct StxPlus3Disk : StxBlock
{
	enum
	{
		...
		SIDEB       = 4		// When a double-sided disk is inserted into a single-sided drive, specifies the side being read from/written to. If set, Side B is the active side, otherwise it is Side A.
	};
...
struct StxGS : StxBlock
{
	...
	{	// GS model
		...
		HALTED		= 2,	// The last instruction executed was a HALT instruction. The CPU is currently executing NOPs and will continue to do so until the next interrupt occurs.
							// This flag is mutually exclusive with EILAST.
		CUSTOMROM	= 64,	// A custom GS ROM is installed. The ROM image is stored in rom_data[]. Note: The default ROM is version 1.04.
		COMPRESSED	= 128	// If a custom GS ROM is embedded in this block, it has been compressed with the Zlib compression library.
	};
	...
	uint8	channel_vol[4];	// The 6-bit volume level of each of the four sound channels. Bits 6 and 7 should be set to zero.
	...
	uint8	i,r,iff1,iff2;	// iff's must be 0 or 1
	uint8	im;				// 0, 1 or 2
	uint32	cpu_cycle;		// The t-states value at the time the snapshot was made. This counts up from zero to the maximum number of t-states per 50Hz frame.
	uint8	int_time_cc;	// The number of t-states left on restart when an interrupt can occur. This is used to support interrupt re-triggering properly.)
	uint8	f53bit_reg;		// Internal Z80 register used to generate bits 5 and 3 of the F register after executing a BIT x,(HL) instruction.
	uint8	rom_data[1];	// A Zlib compressed or uncompressed custom GS ROM (if one was installed). The uncompressed ROM size is always 32,768 bytes.
};
// When loading Zlib compressed ROMs, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTGS ) - 1 - sizeof( ZXSTBLOCK ));
// Remarks: Memory state is stored separately in ZXSTGSRAMPAGE blocks which will follow this block.
...
struct StxGSRamPage : StxBlock
{
	...
	uint16	flags;			// COMPRESSED:	The RAM page data (beginning from chData) will be compressed using the Zlib compression library.
...
// When loading Zlib compressed pages, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTGSRAMPAGE ) - sizeof( ZXSTBLOCK ) - 1 );
// Remarks: A ZXSTGS block will proceed the first of these blocks.
...
struct StxKeyboard : StxBlock
{
	...
	uint32	flags;			// ISSUE2
	uint8	joystick;		// Specfies which joystick the PC keyboard should emulate (the actual keys are emulator dependant). This can be one of:
							// KEMPSTON, FULLER, CURSOR, SINCLAIR1, SINCLAIR2, SPECTRUMPLUS, TIMEX1, TIMEX2 or NONE
};
...
struct StxIf1 : StxBlock
{
	enum
	{
		...
		COMPRESSED  = 2,	// Specifies the custom Interface 1 ROM image at chRomData has been compressed by the Zlib compression library.
...
// When loading Zlib compressed ROMs, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTIF1 ) - 1 - sizeof( ZXSTBLOCK ));
// Remarks: This block will appear before any Microdrive cartridge blocks (ZXSTMCART) in the file.
...
struct StxJoystick : StxBlock
{
	...
	uint32	flags;			// ZXSTJOYF_ALWAYSPORT31:	deprecated as it is an emulator feature rather than hardware state information.
...
struct StxMdrCart : StxBlock
{
	...
	uint8	data[1];		// The file name of the Microdrive cartridge file (.mdr) which should be opened and inserted into this drive
};
...
struct StxMultiface : StxBlock
{
	...
	{	// model_48k:
		...
		SOFTWARELOCKOUT    = 0x04,	// Indicates that the software lockout feature of Multiface 128s and Multiface 3s has been enabled. This flag is not valid and should be set to 0 (zero) when emulating the Multiface 1.
		REDBUTTONDISABLED  = 0x08,	// Indicates that the red (magic) button is disabled.
		DISABLED           = 0x10,	// Indicates that the user has disabled Multiface 1 emulation with the physical disable switch. This flag is not valid and should be set to 0 (zero) when emulating the Multiface 128 or Multiface 3.
		RAM16KMODE         = 0x20	// Specifies Multiface hardware which has had the 8KB of ROM and 8KB of RAM replaced with 16KB of RAM. This is required by advanced Multiface programs such as SoftCrack.
									// If RAM16KMODE is set, the RAM image stored at data[] will expand to 16,384 bytes.
	};
...
// When loading the Zlib compressed RAM image, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTMULTIFACE ) - 1 - sizeof( ZXSTBLOCK ));
// Remarks: The last bytes written to ports $1ffd and $7ffd (which are used by the Multiface) can be read from the ZXSTBID_SPECREGS block which appears earlier in the file.
...
struct StxOpus : StxBlock
{
	enum
	{
		...
		COMPRESSED   = 2,	// Specifies the RAM (and any custom ROM) have been compressed with the Zlib compression library.
		SEEKLOWER    = 4,	// If set, the WD1770 FDC's current seek direction is towards lower cylinder numbers. Otherwise, it is towards higher ones.
		CUSTOMROM    = 8	// A custom ROM is installed. The Zlib compressed or uncompressed ROM image begins after the RAM image at chRam. dwcbRom contains the size of the compressed (or uncompressed) data.
	};

	uint32	flags;				// PAGED | COMPRESSED | SEEKLOWER | CUSTOMROM
	uint32	compressed_ram_size;// Size in bytes of the Zlib compressed or uncompressed Opus Discovery RAM. The uncompressed RAM size is always 2,048 bytes.
	uint32	compressed_rom_size;// Size in bytes of the Zlib compressed or uncompressed custom Opus Discovery ROM (if one was installed). The uncompressed ROM size is always 8,192 bytes.
	...
	uint8	ram[1];				// A Zlib compressed or uncompressed image of the Opus Discovery's RAM. The uncompressed RAM size is always 2048 bytes.
...
struct StxOpusDisk : StxBlock
{
	...
	{	// flags:
		EMBEDDED      = 1,	// // The actual disk image is embedded in this block.
		COMPRESSED    = 2,	// If ZXSTOPDF_EMBEDDED is also set, the disk image in this block is compressed using the Zlib compression library.
		WRITEPROTECT  = 4,	// Specifies whether or not the disk image is write-protected.
		// disk_type:
		OPD           = 0,	// The disk image is in the .opd format.
		OPU           = 1,	// The disk image is in the .opu format.
		FLOPPY0       = 2,	// Real disk mode. drive 'drive_num' should use the first real 3½" floppy disk drive in a system. In Windows, this is the A: drive. (filename and disk_image void)
		FLOPPY1       = 3	// Real disk mode. drive 'drive_num' should use the second real 3½" floppy disk drive in a system. In Windows, this is the B: drive. (filename and disk_image void)
	};
...
struct StxPlusD : StxBlock
{
	enum
	{
		...
		COMPRESSED	= 2,	// Specifies the RAM (and any custom ROM) have been compressed with the Zlib compression library.
		SEEKLOWER	= 4,	// If set, the WD1772 FDC's current seek direction is towards lower cylinder numbers. Otherwise, it is towards higher ones.
		// rom_type:
		GDOS		= 0,	// The standard G+DOS ROM (Version 1.A).
		UNIDOS		= 1,	// Uni-DOS ROM.
		CUSTOM		= 2		// A custom ROM is installed. The Zlib compressed or uncompressed ROM image is stored starting at ram[compressed_ram_size].
	};

	uint32	flags;				// PAGED | COMPRESSED | SEEKLOWER
	uint32	compressed_ram_size;// Size in bytes of the Zlib compressed or uncompressed custom Plus D RAM. The uncompressed RAM size is always 8,192 bytes.
	uint32	compressed_rom_size;// Size in bytes of the Zlib compressed or uncompressed custom Plus D ROM (if one was installed). The uncompressed ROM size is always 8,192 bytes.
	...
	uint8	ram[1];				// A Zlib compressed or uncompressed image of the Plus D's RAM. The uncompressed RAM size is always 8192 bytes.
								// If a custom ROM is installed. The image follows immediately after the RAM data.
};
...
struct StxPlusDDisk : StxBlock
{
	enum
	{
		// flags:
		EMBEDDED      = 1,	// The actual disk image is embedded in this block.
		COMPRESSED    = 2,	// If EMBEDDED is also set, the disk image in this block is compressed using the Zlib compression library.
		WRITEPROTECT  = 4,	// Specifies whether or not the disk image is write-protected.
		// disk_type:
		MGT           = 0,	// The disk image is in the .mgt format.
		IMG           = 1,	// The disk image is in the .img format.
		FLOPPY0       = 2,	// Real disk mode. Drive 'drive_num' should use the first real 3½" floppy disk drive in a system, e.g. Drive A. 'filename' and 'disk_image' are unused.
		FLOPPY1       = 3	// Real disk mode. Drive 'drive_num' should use the second real 3½" floppy disk drive in a system, e.g. Drive B. 'filename' and 'disk_image' are unused.
	};
...
// When loading this block, you should use the 'disk_type' member to determine the disk image format, rather than the file extension of 'filename'. This is especially important since these members are not valid when chDiskType is ZXSTPDDT_FLOPPYx.
...
struct StxRamPage : StxBlock
{
	...
	uint8	page_no;		// Memory page number (usually 0-7, but see below as the Pentagon 512/1024 and ZS Scorpion machines have additional memory).
...
// When loading Zlib compressed pages, the compressed size can be obtained by: compressedSize = blk.dwSize - ( sizeof( ZXSTRAMPAGE ) - sizeof( ZXSTBLOCK ) - 1 );
// Remarks: The RAM page blocks are not guaranteed to be in any specific order.
...
struct StxRom : StxBlock
{
	...
	uint8	data[1];			// Either a Zlib compressed or uncompressed ROM image depending on whether the COMPRESSED bit of 'flags' is set.
};
// Remarks: When a custom ROM is not installed, the standard UK ROMs should be used for all official ZX Spectrum models. For the Spectrum +2A/+3, this is the v4.0 ROM.
...
struct StxTimex : StxBlock
{
	...
	uint8	port_ff;		// port $ff byte which controls the screen mode, interrupt state, high resolution colours, and the remainder of the memory paging system.
};
...
struct StxTape : StxBlock
{
	...
	uint16	current_block_no;	// The current block number (ie. the position of the virtual tape head) starting from 0.
								// For .wav and .voc files, Spectaculator splits these up into 5 second chunks. 'current_block_no' will refer to one of these chunks.
	uint16	flags;				// Various flags indicating whether a tape file is embedded in this block or linked to on disk. This can be a combination of:
								// EMBEDDED | COMPRESSED
	uint32	uncompressed_size;	// For Zlib compressed tape files, this is the size in bytes they uncompress to. This value is undefined if the tape file is not embedded.
	uint32	compressed_size;	// The size of the data at data[].
	char	file_extension[16];	// File extension (case insensitive) of an embedded tape file. This value is undefined if the tape file is not embedded.
	...
	uint8	data[1];			// The data stored here is one of:
								...
								//						If the COMPRESSED bit in 'flags' is also set, the embedded file will be compressed using the Zlib compression libarary.
								//						compressed_size contains the size of the compressed data at data[].
...
// if the joystick is enabled, then a block ZXSTKEYBOARD or ZXSTJOYSTICK for KEMPSTON emulation refer to this joystick port.
// if the joystick is enabled and no such block is present, then the joystick port uses any joystick it finds.
// if a block ZXSTIF2ROM is present, then this rom is plugged into the SPECTRA interface
