 ...
 */
FloppyDisk::FloppyDisk(cstr fpath) : bytes_per_track(6250), modified(no), filepath(newcopy(fullpath(fpath)))
{
	cstr err;
...
void FloppyDisk::saveDisk() throws
{
	if (!filepath)
	{
		showAlert("The disc could not be saved because there was no filename assigned with it");
		return;
	}
...
{
	while (sides[0].count() > n)
	{
		delete[] sides[0].last();
		sides[0].drop();
	}
	while (sides[1].count() > n)
	{
		delete[] sides[1].last();
		sides[1].drop();
	}
...
uint8* FloppyDisk::writeSector(SectorFormatInfo& f, uint8* p)
{
	...
	memset(p, 0x4E, f.gap3);
	p += f.gap3; // 54*	4E		GAP1=50 (fixed size); GAP3=54 (DOS)
	memset(p, 0x00, 12);
	p += 12; // 12*	00
	memset(p, 0xA1, 3);
	p += 3;		 // 3*	A1
...
void FloppyDisk::writeTrack(Array<SectorFormatInfo>& format, uint8*& track, uint gap4a)
{
	...
	if (totalsize > bytes_per_track)
	{
		logline("FloppyDisk.writeTrack: track data exceeds track size");
		return;
	}
	...
	*/
	...
	for (uint t = 0; t < num_tracks; t++)	 // tracks
		for (uint s = 0; s < num_sides; s++) // sides
		{
			...
			uint8 track = tp[0x10];
			if (track != t) return "disk file corrupted (wrong track)";
			uint8 side = tp[0x11];
			if (side != s) return "disk file corrupted (wrong side)";
			uint8 gap3 = tp[0x16];
			if (gap3 < 20) return "disk file corrupted (gap3 too short)";
			uint8 scnt = tp[0x15];
			if (scnt > 29) return "disk file corrupted (too many sectors)";
			...
			for (uint i = 0; i < scnt; i++) // sectors
			{
				...
				logline(
					" h=%i, t=%i, s=%i, sz=%i", int(f.side_id), int(f.track_id), int(f.sector_id), int(f.sector_sz));
				sdp += ssize;
				sip += 8;
			}
	...
	for (uint t = 0; t < num_tracks; t++)	 // tracks
		for (uint s = 0; s < num_sides; s++) // sides
		{
			...
			for (uint i = 0; i < scnt; i++) // sectors
			{
				...
				if (actual_data_length != ssize) { TODO(); }
...
void FloppyDisk::parse_track(uint8* track, TrackFormatInfo& trackinfo) const
{
	...
	while (i < bytes_per_track)
	{
		...
		n = 0;
		while (i < bytes_per_track && track[i] == 0x4e)
		{
			i++;
			n++;
		} // GAP1 or GAP3
		...
		n = 0;
		while (i < bytes_per_track && track[i] == 0x00)
		{
			i++;
			n++;
		} // VCO SYNC
		...
		i += 2;
		if (crc16(track + i - 10, 10) != 0) continue; // ID CRC wrong
...
void FloppyDisk::parse_disk(Array<TrackFormatInfo> trackinfo[2])
{
	...
	for (uint s = 0; s < 2; s++)
		for (uint t = 0; t < sides[s].count(); t++) { parse_track(sides[s][t], trackinfo[s][t]); }
}
...
void FloppyDisk::write_extended_disk_file(FD& fd) const throws
{
	...
	uint8 diskinfo[0x100];
	memset(diskinfo, 0, 0x100);
	uint8* dip = diskinfo;
	...
	dip += 34;
	strcpy(ptr(dip), catstr(APPL_NAME, " ", APPL_VERSION_STR));
	dip += 14;
	...
	*dip++ = uint8(num_sides);
	...
	for (uint s = 0; s < num_sides; s++)
	{
		for (uint t = 0; t < num_tracks; t++)
		{
			...
			max_sectorsize = 0x80u << max_sectorsize;
			if (max_sectorsize == 0x2000) max_sectorsize = 0x1800;
	...
	for (uint t = 0; t < num_tracks; t++)	 // tracks
		for (uint s = 0; s < num_sides; s++) // sides
		{
			...
			uint8 trackinfo[256];
			memset(trackinfo, 0, 256);
			uint8* tip = trackinfo;

			memcpy(tip, "Track-Info", 13);
			tip += 16;
			*tip++ = uint8(t); //	track#
			*tip++ = uint8(s); //	side#
			tip += 2;		   //	unused
