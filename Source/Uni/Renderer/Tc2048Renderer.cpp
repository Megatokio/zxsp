...
*/
void Tc2048Renderer::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8* attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase, uint32 cc_vbi )
{
	...
	int32 cc_end_of_visible_screen	  = min(int(cc_vbi),cc_start_of_visible_screen + int(height) * int(cc_per_scanline) - cc_row_flyback);
				...
				// draw screen row:
					...
					else	// 32 column mode
					{
						...
						while(p<e)
						{
							...
							{
								if(pixels&m) { *p++ = pen_color; *p++ = pen_color; } else { *p++ = paper_color; *p++ = paper_color; }
							}
						}
					}
				}
...
/*
	...
	*** Da niemand mehr das Pixel-Seitenverhältnis beachtet, kann kein Gif mit 512x192 Pixeln erzeugt werden.
		...
		Im 64column-Mode werden immer 2 Pixel gemischt, was wg. der möglichen Farbkombinationen im 64column-Mode immer 50% grau ergibt.
*/
void Tc2048GifWriter::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8 *attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase )
{
	assert((cc_start_of_screenfile&3) == 0);
			...
			while(p<ee)
			{
				...
				// draw screen row:
					if(mode64)	// 64 column mode
					{
						...
						{
							uint pixels = *q++;
							for(uint m=0xC0;m;m=m>>2) { *p++ = pixels&m ? ~pixels&m ? mixed_color : pen_color : paper_color; }
						}
					}
					else	// 32 column mode
					{
