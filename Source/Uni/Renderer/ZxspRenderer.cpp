...
*/
void ZxspRenderer::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8* attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase, uint32 cc_vbi )
{
	...
	int32 cc_start_of_visible_screen  = cc_start_of_screenfile - cc_h_border - v_border*cc_per_scanline;
										if(int32(cc_vbi)<cc_start_of_visible_screen) return;				// video beam ist noch über dem sichtbaren Bildschirmausschnitt
	int32 cc_end_of_visible_screen	  = min(int32(cc_vbi),cc_start_of_visible_screen + int(height) * int(cc_per_scanline) - cc_row_flyback);
...
void ZxspGifWriter::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8 *attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase )
{
	assert((cc_start_of_screenfile&3) == 0);
			...
			while(p<ee)
			{
				...
				// draw screen row:
					for(e+=screen_width; p<e;)
					{
						...
						uint pen_color   = (attr&7) + ((attr>>3)&8);	if(pen_color==transp) pen_color=0;
						uint paper_color = (attr>>3)&15;				if(paper_color==transp) paper_color=0;
...
*/
void ZxspGifWriter::writeFrame(IoInfo* ioinfo, uint ioinfo_count, uint8 *attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase) throws
{
	...
	drawScreen(ioinfo,ioinfo_count,attr_pixels,cc_per_scanline,cc_start_of_screenfile,flashphase);	// bits := new screen
	*diff = *bits;
...
/*	Save the screenshot already rendered in bits[] and bits2[] to file
	this is the version for ZX Spectrum-style screens
*/
void ZxspGifWriter::saveScreenshot( cstr path, IoInfo *ioinfo, uint ioinfo_count, uint8 *attr_pixels,
...
// Calculate Colormaps and total number of colors in image:
	Colormap cmap  = global_colormap; bits->reduceColors(cmap);
	Colormap cmap2 = global_colormap; bits2->reduceColors(cmap2);
