...
bool MachineZxsp::handleLoadTapePatch()
{
	...
	/*	load data from tape file as with the ZX Spectrum Rom load routine
		...
		ok:
			...
			• FLANK2 ($05E3) und darin FLANK1 ($05E7) werden für jedes Bit aufgerufen und überschreiben die geladenen
	   Daten err: NC    = break or error NC NZ = break						IX,DE entspr. geladenen bytes NC NZ = wrong
	   block type			IX,DE untouched;  EX AF,AF';  L = block type found
	...
	*/
	...
	if (!trunc)
	{
		qlen--;
		zlen--;
	} // if not truncated then exclude the crc byte from buffer data
...
void SnaHead::setRegisters(const Z80Regs& regs)
{
...
void MachineZxsp::loadSna(FD& fd)
{
	...
	{
		xlog("x");
		read_mem(fd, cpu->wrPtr(0x4000 + i), CPU_PAGESIZE);
	}
