	...
	void remove(volatile Machine* m)
			{
				PLocker z(_lock);
				//Array::removeitem(m);
				for(uint i=0; i<cnt; i++) { if(m == data[i]) { Array::remove(i); return; } }
			}

	uint count() { return cnt; }
	volatile Machine* operator[] (uint i) { assert(i<cnt); return data[i]; }
};
...
#define ForAllItems( WHAT ) { Item*p=lastitem; do{ p->WHAT; } while( (p=p->prev()) ); }
...
void Machine::memoryRemoved(Memory* m)
{
	...
	{
		if (memory[i] == m) { memory.remove(i); break; }
	}
	memoryModified(m,1);
}
...
*/
void Machine::loadRom(FD& fd) throws /*FileError,DataError*/
{
	assert(is_locked());
...
void Machine::saveAs(cstr filepath) throws
{
	...
	if(eq(ext,".rom")) { saveRom(fd); return; }
	if(eq(ext,".scr")) { saveScr(fd); return; }
	if(eq(ext,".sna")) { saveSna(fd); return; }     // MachineZxsp.cpp
	if(eq(ext,".ace")) { saveAce(fd); return; }     // MachineJupiter.cpp
	if(eq(ext,".z80")) { saveZ80(fd); return; }
	if(eq(ext,".o"))   { saveO80(fd); return; }     // MachineZx80.cpp: ZX80 must be waiting at the command prompt!
	if(eq(ext,".80"))  { saveO80(fd); return; }     // MachineZx80.cpp: ZX80 must be waiting at the command prompt!
	if(eq(ext,".p"))   { saveP81(fd,0); return; }   // MachineZx81.cpp: ZX81 must be waiting at the command prompt!
	if(eq(ext,".81"))  { saveP81(fd,0); return; }   // MachineZx81.cpp: ZX81 must be waiting at the command prompt!
	if(eq(ext,".p81")) { saveP81(fd,1); return; }   // MachineZx81.cpp: ZX81 must be waiting at the command prompt!
...
uint8 Machine::handleRomPatch(uint16 pc, uint8 opcode)
{
	...
	else if(instrptr == rom.getData()+model_info->tape_save_routine)
	{
		...
		if(ula->isA(isa_UlaZx80)) UlaZx80Ptr(ula)->enableMicOut(1); else
		if(ula->isA(isa_UlaZx81)) UlaZx81Ptr(ula)->enableMicOut(1);
	}
	...
	else if(instrptr == rom.getData()+model_info->tape_load_ret_addr)
	{
		...
		if(ula->isA(isa_UlaZx80)) UlaZx80Ptr(ula)->enableMicOut(0); else
		if(ula->isA(isa_UlaZx81)) UlaZx81Ptr(ula)->enableMicOut(0);
	}
...
uint8 Machine::inputAtCycle( int32 cc, uint16 addr )
{
	...
	if(rzx_file)
	{
		...
		else if(rzx_file->isRecording())
		{
			rzx_file->storeInput(c);
		}
	}
...
void Machine::runForSound( int32 cc_final )
{
	...
	if(rzx_file && rzx_file->isPlaying())
	{
		...
		do
		{
			...
			{
				if( cc < cc_final && ic < ic_end )
					result = cpu->run(cc_final,ic_end,cpu_options);
			}
			...
			if(ic >= ic_end)	// next rzx frame:
			{
				...
				{								// rval = -1 and state = Snapshot -> call getSnapshot()
					...
					if(controller->isRzxAutostartRecording())
					{
						rzxStartRecording();
					}
	...
	{
a:		int32  cc_ffb = ula->cpuCycleOfFrameFlyback();
...
// shift time: this does not change the current time:
	Time t = t_for_cc(cc) >= seconds_per_dsp_buffer()
			? seconds_per_dsp_buffer()		// completed
			: t_for_cc(cc);					// breaked
...
void Machine::stepOver()
{
	...
	switch(o)
	{
	case CALL_NZ: if(f&Z_FLAG) goto s; else goto c;
	case CALL_Z:  if(f&Z_FLAG) goto c; else goto s;
	case CALL_NC: if(f&C_FLAG) goto s; else goto c;
	case CALL_C:  if(f&C_FLAG) goto c; else goto s;
	case CALL_PO: if(f&P_FLAG) goto s; else goto c;
	case CALL_PE: if(f&P_FLAG) goto c; else goto s;
	case CALL_P:  if(f&S_FLAG) goto s; else goto c;
	case CALL_M:  if(f&S_FLAG) goto c; else goto s;

	case RST00:	case RST08: case RST10: case RST18:
	case RST20: case RST28: case RST30: case RST38:
	case CALL:
c:		cpu->setStackBreakpoint( cpu->getRegisters().sp );
		cpu_options |= cpu_break_sp;
		resume();
		break;
...
void Machine::drawVideoBeamIndicator()
{
	...
	if(++beam_cnt<4) return;				// nur jeden 4. Sound-Interrupt ~ ca. 50 Mal / Sek.
...
void Machine::rzxPlayFile(RzxFile* rzx)
{
	...
	{
	default:
		return rzxOutOfSync("Bummer: unexpected state after rewinding the rzx file",yes);
	case RzxFile::OutOfSync:
		return rzxOutOfSync("I'm sorry, but the rzx file is unusable.");
	case RzxFile::EndOfFile:
		return rzxStartRecording("The rzx file is empty. rzx recording started!");
	case RzxFile::Snapshot:
		break;
	}
	...
	{
	default:
		return rzxOutOfSync("Bummer: unexpected state after loading the starter snapshot",yes);
	case RzxFile::OutOfSync:
		return; // message already displayed
	case RzxFile::EndOfFile:
		return rzxStartRecording("The rzx file only contained a snapshot. rzx recording started!");
...
void Machine::rzxStartRecording(cstr msg, bool yellow)
{
	...
	{
	default:
		return rzxOutOfSync(usingstr("Start RZX recording: unexpected file state %i",rzx_file->state), yes);

	case RzxFile::Playing:
		rzx_file->startRecording();
		break;

	case RzxFile::Recording:
		return;
