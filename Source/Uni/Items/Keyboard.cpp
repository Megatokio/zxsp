...
p:	for( uint8*a=&keymap[0]; a<e; a++ )		// loop over lines 1 to 7
	{
...
void Keyboard::update_keymap()
{
	...
	Keymap newkeys; newkeys.clear();
	bool ownmeta = false;			// set if keys supplied own caps/symbol flag

	for(uint k=0;k<numkeys;k++)
	{
		uint8 key = zxkeys[k];
		if(key&c) { newkeys.set_key(csh); ownmeta=true; key-=c; }
		if(key&s) { newkeys.set_key(ssh); ownmeta=true; key-=s; }
...
void Keyboard::realKeyDown(uint16 unicode, uint8 oskeycode, KbdModifiers modifiers)
{
	...
	if(mode==kbdbtzxkbd) { keyBtZxKbd(unicode,oskeycode,modifiers); return; }
	...
	// um eine deshalb hängende Taste mittels nochmaligem Drücken lösen zu können.
		for(uint i=0;i<numkeys;i++) { if(oskeys[i]==oskeycode) goto x; }
...
{
	xlogline("key down event: '%s', unicode=%u, modifiers=$%04x on BtZxKbd",
	...
	else
	{
		...
		for(uint i=0;i<numkeys;i++) { if(zxkeys[i]==pkey) goto x; }
		assert(numkeys < NELEM(zxkeys));
		zxkeys[numkeys]   = pkey;
		oskeys[numkeys++] = oskeycode;
	}
...
void Keyboard::realKeyUp(uint16 unicode, uint8 oskeycode, KbdModifiers modifiers )
{
	...
	{
		xlogline("key up event: '%s', unicode=%u, modifiers=$%04x on BtZxKbd *** ERROR! ***",
			unicode>=' ' && unicode<='~'?charstr(unicode):"non-ascii", uint(unicode), uint(modifiers));
		return;
	}
