...
enum Command
{
	...
	mIllCmd		= ~(mRdTr|mSpecify|mGetDrvSt|mWrSec|mRdSec|mSeekTr0|mGetIntSt|mWrDelSec|mRdSecId|mRdDelSec|mFmtTr|mSeekTr|mScanXX)
};
...
//helper
bool Fdc765::scan_satisfied()
{
	return CMD==mScanEQ ? eq : CMD==mScanGE ? ge : CMD==mScanLE ? le : no;
}
...
#define BEGIN		switch(sm_state) { default: IERR(); case 0:
#define WAIT		do { sm_state = __LINE__; return; case __LINE__:; } while(0)
#define FINISH		}
#define	GOSUB(PROC)	do { sm_stack[sm_sp++] = __LINE__; goto PROC; case __LINE__:; } while(0)
#define	RETURN		goto ret

#define send_a_byte_to_cpu(BYTE)	MSR = msrFdcBusy | msrRQR; byte = BYTE; while(MSR&msrRQM) WAIT;
#define read_a_byte_from_cpu(BYTE)	MSR = msrFdcBusy | msrRQW; while(MSR&msrRQM) WAIT; BYTE = byte;
#define read_byte_from_cpu()		MSR = msrFdcBusy | msrRQW; while(MSR&msrRQM) WAIT;
...
aa:	BEGIN;
	...
	// Get Command:
		...
		while(MSR)
		{
			if(ready!=drive->is_ready) { ready^=1; SR0 |= ICdriveBusyChg | head_unit; raise_interrupt(); }
			WAIT;
		}
		...
		{
			"IllCmd 00",	"IllCmd 01",	"ReadTrack",	"Specify",
			"GetDriveState","WriteSector",	"ReadSector",	"SeekTrack0",
			"GetIntState",	"WriteDelSec",	"ReadSectorID",	"IllCmd 0Bh",
			"ReadDelSec",	"FormatTrack",	"IllCmd 0Eh",	"SeekTrack",
			"IllCmd 10h",	"ScanEQ",		"IllCmd 12h",	"IllCmd 13h",
			"IllCmd 14h",	"IllCmd 15h",	"IllCmd 16h",	"IllCmd 17h",
			"IllCmd 18h",	"ScanLE",		"IllCmd 1Ah",	"IllCmd 1Bh",
...
//			clear_interrupt();
	...
	// Get Most Common Arguments:
		...
		// Get Drive+Head:
		if(CMD & (mRdTr|mWrSec|mRdSec|mWrDelSec|mRdDelSec|mScanXX|mGetDrvSt|mRdSecId|mSeekTr0|mSeekTr|mFmtTr))
		{
			read_byte_from_cpu();
			head_unit = byte & 7;
			head	  = head_unit >> 2;
		...
		{
			read_a_byte_from_cpu(log2sectorsize);  log2sectorsize &= 7;	// Sector size: log2(size)-7
	...
	// Kommandos auswerten:
		...
		{												// TODO: multiple drives
			...
			while( track[unit] < requested_track[unit] )
			{
				...
				timeout+=steprate; while(t<timeout) WAIT;
		...
		{												// TODO: multiple drives
			...
			SR0 = ICok | SeekEnd | head_unit;
			if(!is_track0()) { SR0 |= EquipmentCheck; logline("EQUIPMENT CHECK!"); }
			raise_interrupt();
			continue;
		}
		...
		if(!is_ready())
		{
			// The loss of a READY signal at the beginning of a command execution phase causes SR0 to be set to ICaborted.
		...
		else
		{
			...
			if(CMD & (mRdSec|mRdDelSec|mWrSec|mWrDelSec|mScanXX))
			{
				...
				*/
				...
				for(;;)		// schleife über seiten
				{
					...
					head ^= 1; head_id ^= 1; sector_id = 1;
				}

				if(sector_id == last_sector_id+step)
				{
					if(multitrack && head==0) { head^=1; head_id^=1; } else { track_id += 1; }
					sector_id = 1; /*SR0 = ICaborted | head_unit;*/ SR1 |= EndOfTrack;				// ICaborted funktioniert nicht mit +3DOS
				}
			...
			else if(CMD==mRdTr)		// READ_TRACK:		02+m+s HU TR HD ?? SZ NM GP SL  <R>  S0 S1 S2 TR HD NM SZ
			{
				...
				while(num_sectors && !indexpulses)
				{
					...
					if(n!=4 || byte!=0xFE) { cancel_crc(); continue; }	// not an IDAM or corrupted or terminated by 2nd sync pulse

					// Sector Info:
					GOSUB(copy_byte_from_fdd_to_host); f  = byte==track_id;
					GOSUB(copy_byte_from_fdd_to_host); f &= byte==head_id;
					GOSUB(copy_byte_from_fdd_to_host); f &= byte==sector_id;
					GOSUB(copy_byte_from_fdd_to_host); f &= byte==log2sectorsize;
					crc_on = false;
					GOSUB(copy_byte_from_fdd_to_host); if(byte != (crc>>8))   continue;	// crc error in IDAM
					GOSUB(copy_byte_from_fdd_to_host); if(byte != uint8(crc)) continue;	// crc error in IDAM
...
//					SR0 |= ICaborted;
			...
			// IAM (Index Address Mark):
				...
				n=80; GOSUB(write_gap);										// GAP4A
				GOSUB(write_vco_sync);										// VCO SYNC
				byte=0xC2; for(n=0;n<3;n++) { GOSUB(write_byte_to_fdd); }	// IAM
				byte=0xFC; GOSUB(write_byte_to_fdd);						// IAM
				...
				// Sector IDAM (ID Address Mark):
					...
					GOSUB(write_gap);      if(indexpulses) break;			// GAP1 o. Gap3
					GOSUB(write_vco_sync); if(indexpulses) break;			// VCO SYNC
					start_crc();
					byte=0xA1; for(n=0;n<3;n++) { GOSUB(write_byte_to_fdd); }	// IDAM
					byte=IDAM; GOSUB(write_byte_to_fdd); 					// IDAM

					timeout = t - drive->timeSinceIndex() + (bytepos+1)*drive->timePerByte();
					GOSUB(copy_byte_from_host_to_fdd); track_id = byte;
					GOSUB(copy_byte_from_host_to_fdd); head_id = byte;
					GOSUB(copy_byte_from_host_to_fdd); sector_id = byte;
					GOSUB(copy_byte_from_host_to_fdd); log2sectorsize = byte; if(mfm&&byte==0) log2sectorsize = 1;
					GOSUB(write_crc); if(indexpulses) break;

				// Sector Data:

					n=22; GOSUB(write_gap);	if(indexpulses) break;			// GAP2
					GOSUB(write_vco_sync);	if(indexpulses) break;			// VCO SYNC
					start_crc();
					byte=0xA1; for(n=0;n<3;n++) { GOSUB(write_byte_to_fdd); }	// DAM
					byte=DAM; GOSUB(write_byte_to_fdd);						// DAM

					byte = fillbyte; n = 0x80 << log2sectorsize;
					while(n-- && !indexpulses) { GOSUB(write_byte_to_fdd); }	// Sector Data
					if(!indexpulses) { GOSUB(write_crc); sector_id++; num_sectors--; }

					n = gap3len;							// Gap3 len
				}

				byte=0x4E; while(!indexpulses) { GOSUB(write_byte_to_fdd); }	// Gap4B up to track end
			}
			else IERR();
		}
...
read_sector_id:			// ( -- SR0 )
{
	for(indexpulses=0; indexpulses<2; )
	{
		...
		GOSUB(read_vco_sync); n++;	// VCO SYNC
		if(byte!=0xA1 || n<10 || indexpulses>=2) continue;

		start_crc(); update_crc();	// IDAM  =  0xA1, 0xA1, 0xA1, 0xFE
		GOSUB(read_byte_from_fdd); if(byte!=0xA1) continue;
		GOSUB(read_byte_from_fdd); if(byte!=0xA1) continue;
		GOSUB(read_byte_from_fdd); if(byte!=0xFE) continue;
		...
		GOSUB(read_byte_from_fdd); if(CMD==mRdSecId) head_id   = byte;		f  = byte==head_id;
		GOSUB(read_byte_from_fdd); if(CMD==mRdSecId) sector_id = byte;		f &= byte==sector_id;
		GOSUB(read_byte_from_fdd); if(CMD==mRdSecId) log2sectorsize=byte;	f &= byte==log2sectorsize || (log2sectorsize==0 && (CMD&(mRdSec|mRdDelSec)));
...
io_sector:				// ( -- )
{
	if(CMD & (mRdSec|mRdDelSec|mScanXX))	// Read Sector Data:
	{
		...
		GOSUB(read_vco_sync); n++;		// 12 * 0x00	VCO SYNC
		if(byte!=0xA1 || n<10) goto iox;

		if(terminal_count) RETURN;

		start_crc(); update_crc();
		GOSUB(read_byte_from_fdd); if(byte!=0xA1) goto iox;
		GOSUB(read_byte_from_fdd); if(byte!=0xA1) goto iox;
...
iox:		SR0 |= ICaborted;
	...
	else	// write Sector Data:
	{
		...
		timeout = t - drive->timeSinceIndex() + (bytepos+1) * drive->timePerByte();
		while(n) { GOSUB(copy_byte_from_host_to_fdd); n--; }

		// write dummy bytes if aborted due to terminal_count:
		byte=0x00; while(n) { GOSUB(write_byte_to_fdd); n--; }
...
copy_byte_from_host_to_fdd:
	...
	if(t>timeout) { SR0 |= ICaborted; SR1 |= Overrun; goto std_result_phase; }
...
write_crc:				// ( crc -- )
	crc_on = no;
	byte = crc>>8; GOSUB(write_byte_to_fdd);
	byte = crc;    GOSUB(write_byte_to_fdd);
	RETURN;
...
*/
read_gap:				// ( -- byte n )
	n = -1; do { n++; GOSUB(read_byte_from_fdd); } while(byte==0x4E && indexpulses<2);
	RETURN;
...
read_vco_sync:			// ( -- byte n )
	n = -1; do { n++; GOSUB(read_byte_from_fdd); } while(byte==0x00 && indexpulses<2);
	RETURN;
...
*/
ret:	sm_state = sm_stack[--sm_sp]; goto aa;
