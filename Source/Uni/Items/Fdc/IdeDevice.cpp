...
->	4 =	2nd status register/interrupt and reset register.
		...
		When read this register gives you the same status byte as the primary (/CS0=0, /CS1=1, A2..A0=111B) status
register. The only difference is that reading this register does not clear the interrupt from the IDE device when read.
	...
	7 = active status of the IDE device.
		In this register (read-only) you can find out if the IDE master or slave is currently active and find the
currently selected head number. In a PC environment you can also find out if the floppy is currently in the drive. I
have not used this register yet. bit 0    : master active. If this bit is set then the master IDE device is active. bit
1    : slave active. If this bit is set then the slave IDE device is active. bits 5..2: complement of the currently
active disk head. bit 6    : write bit. This bit is set when the device is writing. bit 7    : in a PC environment this
...
IdeDevice::IdeDevice(cstr fpath, DeviceType dtyp, bool is_master) :
	devicetype(dtyp), is_master(is_master), is_cfa(dtyp == CFCard), is_packet(dtyp == CDRom), is_general(!is_packet),
	can_write(dtyp != CDRom), max_sectors_per_multiple(MAX_SECTORS_PER_MULTIPLE), status_register(0), filepath(nullptr),
	io_mode(io_none), hd_mode(hd_invalid) // damit wir nicht vorzeitig in hd_mode schreiben
{
	// Timing constants
	...
	if (e)
	{
		showAlert("Creating the HDD service failed:\n%s", errorstr());
		return;
	}
...
// static
void* IdeDevice::worker_proc(void* self) { return ((IdeDevice*)self)->worker_proc(); }
	...
	for (;;)
	{
		...
		try
		{
			...
			{			  // caller must block until command finishs!
				...
				{
					uint8 bu[0x100];
					diskfile.read_bytes(bu, 0x100);
					if (memcmp(bu, "RS-IDE\x1A", 7)) throw FileError(diskfile, wrongmagic);
					xlogline("  file version:  %02X", int(bu[7]));
					if (bu[8] & 1)
						throw FileError(diskfile, filecontainslowbytesonly); // does not work with DivIDE anyway
					if (bu[8] & 2) logline("  ATAPI"); // ATAPI ignored. should work as CF card or HDD as well. TODO
					base = peek2Z(bu + 9);
					uint n;
					n = peek2Z(bu + 0x16 + 2 * 1);
					if (n < 16384) dflt_num_cylinders = num_cylinders = n; // if any of CHS is invalid
					n = peek2Z(bu + 0x16 + 2 * 3);
					if (n < 16) dflt_num_heads = num_heads = n; // then C*H*S=0
					n = peek2Z(bu + 0x16 + 2 * 6);
					if (n < 64) dflt_num_sectors = num_sectors = n; // => C*H*S!=total_sectors
				}
...
void IdeDevice::open_diskfile(cstr fpath)
{
	...
	hd_semaphore.release();
	hd_wait_busy();
	if (hd_error != noerror) { showAlert("Open disc file failed:\n%s", errorstr(hd_error)); }
	else
	{
...
{
	if (drive_head & drvhd_LBA_mask) { return (lba_address & 0x0fffffff) < total_sectors; }
	else
	{
...
#define calc_CHS_from_LBA(A, C, H, S)                                                                                  \
  S = (A) % num_sectors + 1, H = ((A) / num_sectors) % num_heads, C = (A) / (num_sectors * num_heads)
...
void IdeDevice::create_identify_drive_page()
{
	...
	//	47		0x8000 + 01h…FFh = Maximum number of sectors that shall be transferred per interrupt on READ/WRITE
	//MULTIPLE commands 	49		bit field: bit 9: LBA supported							0x2f00
	...
	//bit 1. 					   bit 2: the device supports Ultra DMA and the values reported in word 88 are valid. 	54		Current number
	//of cylinders								16383 	55		Current number of heads 16
	...
	// if(is_cfa) words[0] = 0x848A;			// ATA5 pg.45: supports CFA feature set: alt. method, wir setzen aber
	// word83.bit2
	poke2Z(words + 1, dflt_num_cylinders);
	poke2Z(words + 2, 0xC837); // ATA5 pg.89: Device does not require SET FEATURES subcommand to spin-up after power-up
	...
	cstr diskname = filepath == nullptr			  ? "Cemetery" :
					startswith(filepath, "/dev/") ? filepath :
													basename_from_path(filepath);
...
void IdeDevice::create_cfa_translate_sector_page() // ATA4++		ATA5 pg. 71
{
	...
	bool erased = yes;
	for (uptr p = buffer + 512; erased && p > buffer;)
	{
		if (~*--(uint64*&)p) erased = no;
	}
...
void IdeDevice::writeData(Time t, uint16 value) // CS0=0, CS1=1, rrr=0
{
	...
	if (pio_8bit_data_mode)
	{
		poke1Z(buffer + buffer_ptr, value);
		buffer_ptr += 1;
	}
	else
	{
		poke2Z(buffer + buffer_ptr, value);
		buffer_ptr += 2;
	}

	if (buffer_ptr >= 512)
	{
		if (command_register == WRITE_BUFFER)
			end_command();
		else
...
uint16 IdeDevice::readData(Time t) // CS0=0, CS1=1, rrr=0
{
	...
	if (pio_8bit_data_mode)
	{
		rval = 0xff00 + peek1Z(buffer + buffer_ptr);
		buffer_ptr += 1;
	} // CFA only
	else
	{
		rval = peek2Z(buffer + buffer_ptr);
		buffer_ptr += 2;
	}
...
uint8 IdeDevice::readRegister(Time t, uint reg)
{
	...
	//	 SEL,  BSY	=> Place the contents of the Status register on the data bus.	CmdReg: exit the interrupt pending
	//state. 	 SEL, !BSY	=> Place the contents of the register on the data bus.			CmdReg: exit the interrupt
	//pending state.
...
void IdeDevice::writeRegister(Time t, uint reg, uint8 value)
{
	...
	//														ATA5 pg.50++
	//	!SEL,  BSY		 => ignored.
	//	 SEL,  BSY		 => indeterminate.					CmdReg: if the device supports the DEVICE RESET command, exit the
	//interrupt pending state.
	//
	//	 SEL, !BSY,  DRQ => ignored.						CmdReg: if the device supports the DEVICE RESET command, exit the
	//interrupt pending state.
	//
	...
	case ide_command_register:
		...
		return;
	case ide_feature_register: feature_register = value; return;
...
uint8 IdeDevice::getStatusRegister(Time t)
{
	if (t < busy_until || hd_mode != hd_idle)
	{
		...
		switch (io_mode) // t≥busy_until && hd_mode==hd_idle
		{
		...
		case io_read_disk: // read transfer disk -> buffer finished:
		{
			...
			else if (command_register == CFA_TRANSLATE_SECTOR) // read sector for test for erased => now transfer the
															   // sector info page
			{
				if (sector_id_is_valid())
				{
					create_cfa_translate_sector_page();
		...
		case io_write_disk: // write transfer buffer -> disk finished
		{
			...
			else
			{
				increment_sector_count_and_number();
				if (sector_count == 0)
					end_command(); // letzter Sektor geschrieben => fertig
				else			   // weitere Sektoren schreiben:
					if (command_register == CFA_ERASE_SECTORS)
						write_buffer_to_disk(t); // nächsten Sektor löschen
					else
						write_data_to_buffer(); // Datentransfer Host -> Buffer restarten
			}
...
void IdeDevice::handle_command(Time t, uint8 cmd)
{
	...
	case 0x00: // NOP: no-data, optional, MANDATORY		General: wir sagen in der identify_drive_page, dass wir's
			   // unterstützen
			   //	feature_register:
	...
	case 0x08: // DEVICE RESET: no-data, prohibited, MANDATORY	ATA3++		ATA5 pg.78
		...
		//	case 0x1_:	// RECALIBRATE:												up to ATA2; obsolete in ATA3; retired since
		//ATA4

	case 0x20: // READ SECTOR(S): PIO-in, MANDATORY, MANDATORY				ATA1++
		if (is_packet)
			break; // TODO: In response to this command, devices that implement the PACKET Command feature set shall
				   // post command aborted and place the PACKET Command feature set signature in the Cylinder High and
				   // the Cylinder Low register (see 9.12).
		read_buffer_from_disk(t);
		return;
	...
	case 0x30: // WRITE SECTOR(S): PIO-out, MANDATORY, prohibited			ATA1++
		...
		//	case 0x34:	// WRITE SECTOR(S) EXT: PIO-out, optional, prohibited		obsolete(?) in ATA5; defined in
		//ATA6++ 	case 0x35:	// WRITE DMA EXT: DMA, optional, prohibited					ATA6++ 	case 0x36:	// WRITE DMA
		//QUEUED EXT: DMA-queued, optional, prohibited	ATA6++ 	case 0x37:	// SET MAX ADDRESS EXT: no-data, optional,
		//prohibited		ATA6++
	...
	case 0x70: //	SEEK TRACK:	no-data, MANDATORY, prohibited				up to ATA6, obsolete since ATA7		ATA5
			   //pg.162 	in: HCS or LBA set
		if (is_packet) break;
		return; //	TODO: evtl. könnten wir bei einem HDD berechnete Wartezeiten einbauen.

		//	case 0x7_:	// SEEK TRACK:												up to ATA2; obsolete in ATA3; retired since
		//ATA4
	...
	case 0x91: //	INITIALISE DEVICE PARAMETERS:	no-data, MANDATORY, prohibited		up to ATA5, obsolete since ATA6
			   //ATA5 pg.120
		if (is_packet) break;
		...
		//	case 0x94:	// STANDBY IMMEDIATE:		up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE0
		//	case 0x95:	// IDLE IMMEDIATE:			up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE1
		//	case 0x96:	// STANDBY:					up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE2
		//	case 0x97:	// IDLE:					up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE3
		//	case 0x98:	// CHECK POWER MODE:		up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE5
		//	case 0x99:	// SET SLEEP MODE:			up to ATA3; retired since ATA4  -->  ATA8 pg.394		same function:
		//0xE6 	case 0xA0:	// PACKET: Packet, prohibited, MANDATORY					ATA3++	TODO ATAPI ESXDOS 	case
		//0xA1:	// IDENTIFY PACKET DEVICE: PIO-in, prohibited, MANDATORY	ATA3++	TODO ATAPI ESXDOS 	case 0xA2:	//
		//SERVICE: Packet/DMA-queued, optional, optional			ATA3++ 	case 0xB0:	// SMART: no-data, optional,
		//prohibited						ATA3++ 	case 0xB1:	// CFA DEVICE CONFIGURATION: no-data, CFA, optional
		//ATA6++ 	case 0xB6:	// NV CACHE: optional, prohibited, optional					ATA8++ (draft)
	...
	case 0xC4: // READ MULTIPLE: PIO-in, MANDATORY, prohibited				ATA1++			ATA5 pg.142
			   // unterscheidet sich von READ SECTORS im Wesentlichen nur in den Interrupts, und die behandeln wir eh
			   // (noch) nicht
		if (is_packet) break;
	...
	case 0xC5: // WRITE MULTIPLE: PIO-out, MANDATORY, prohibited			ATA1++
			   // unterscheidet sich von WRITE SECTORS im Wesentlichen nur in den Interrupts, und die behandeln wir eh
			   // (noch) nicht
		if (is_packet) break;
		if (sectors_per_multiple == 0) break;
		write_data_to_buffer();
	...
	case 0xE0: // STANDBY IMMEDIATE: no-data, MANDATORY, MANDATORY			ATA1++
			   ...
			   // Power Management feature set is mandatory when power management is not implemented by a PACKET power
			   // management feature set. This command is mandatory when the Power Management feature set is
			   // implemented. This command causes the device to immediately enter the Standby mode: e.g. spin down the
			   // drive at once.
		powermode = Standby;
		return;
	...
	case 0xE1: // IDLE IMMEDIATE: no-data, MANDATORY, MANDATORY			ATA1++
			   ...
			   // Power Management feature set is mandatory when power management is not implemented by a PACKET power
			   // management feature set. This command is mandatory when the Power Management feature set is
			   // implemented. The IDLE IMMEDIATE command allows the host to immediately place the device in the Idle
			   // mode.
		powermode = Idle;
		return;
	...
	case 0xE2: // STANDBY: no-data, MANDATORY, optional					ATA1++
			   // Power Management feature set.
			   // Power Management feature set is mandatory when power management is not implemented by a PACKET power
			   // management feature set. This command is mandatory when the Power Management feature set is implemented
			   // when the PACKET Command feature set is not implemented.
			   //
			   // The value in the Sector Count register shall determine the time period programmed into the Standby
			   ...
			   // The value in the Sector Count register shall be used to determine the time programmed into the Standby
			   // timer If the Sector Count register is zero then the Standby timer is disabled.
			   //
	...
	case 0xE3: // IDLE: no-data, MANDATORY, optional						ATA1++
			   ...
			   // Power Management feature set is mandatory when power management is not implemented by a PACKET power
			   // management feature set. This command is mandatory when the Power Management feature set is implemented
			   // and the PACKET Command feature set is not implemented.
			   //
	...
	case 0xE5: // CHECK POWER MODE: no-data, MANDATORY, MANDATORY			ATA1++
			   ...
			   // The CHECK POWER MODE command shall not cause the device to change power or affect the operation of the
			   // Standby timer.
			   //
	...
	case 0xE6: // SLEEP: no-data, MANDATORY, MANDATORY						ATA1++
			   ...
			   // This command causes the device to set the BSY bit to one, prepare to enter Sleep mode, clear the BSY
			   // bit to zero and assert INTRQ. The host shall read the Status register in order to clear the interrupt
			   // pending and allow the device to enter Sleep mode. In Sleep mode, the device only responds to the
			   // assertion of the RESET signal and the writing of the SRST bit in the Device Control register and
			   // releases the device driven signal lines. The host shall not attempt to access the Command Block
			   // registers while the device is in Sleep mode. Because some host systems may not read the Status
			   // register and clear the interrupt pending, a device may automatically release INTRQ and enter Sleep
			   // mode after a vendor specific time period of not less than 2 s. The only way to recover from Sleep mode
			   // is with a software reset, a hardware reset, or a DEVICE RESET command. A device shall not power-on in
			   // Sleep mode nor remain in Sleep mode following a reset sequence.
		busy_until		= t + 24 * 60 * 60;
		status_register = 0; // !RDY.  das müsste funktionieren...
		return;

	case 0xE7:	// FLUSH CACHE: no-data, MANDATORY, optional				ATA4++
		return; // we have no caches							TODO: we could wait for write to file on helper thread to
				// complete
	...
	case 0xEF: // SET FEATURES: no-data, MANDATORY, MANDATORY				ATA1++
			   ...
			   // Enable/Disable Media Status Notification sub commands are mandatory if the Removable Media feature set
			   // is implemented. All other subcommands are optional.
			   //
		...
		case 0x8A: // Disable CFA power mode 1
			if (!is_cfa) break;
			return; //	CFA devices may consume up to 500 mA maximum average RMS current for either 3.3V or 5V operation
					//in Power Mode 1. 	CFA devices revert to Power Mode 1 on hardware or power-on reset. 	CFA devices
					//revert to Power Mode 1 on software reset except when Set Features disable reverting to power-on
					//defaults is set. 	Enabling CFA Power Mode 1 does not spin up rotating media devices. 	CFA devices
					//may consume up to 75 mA maximum average RMS current for 3.3V or 100 mA maximum average RMS current
					//for 5V operation in Power Mode 0. 	A device in Power Mode 0 shall accept the following commands: 	−
					//IDENTIFY DEVICE 	− SET FEATURES (function codes 0Ah and 8Ah) 	− STANDBY 	− STANDBY IMMEDIATE 	− SLEEP
					//	− CHECK POWER MODE
					...
					//the Power Mode 0 current restrictions. 	Commands that require more current than specified for Power
					//Mode 0 shall be rejected with an abort error.

		case 0x03: // Set transfer mode based on value in Sector Count register.
			if (sector_count <= 0x0F)
				return; // PIO mode. whatever
			else
				break;
		}
		...
		//	case 0xF2:	// SECURITY UNLOCK: PIO-out, optional, optional				ATA3++	vendor specific up to ATA2; e.g.
		//similar to 0xE2 (STANDBY) 	case 0xF3:	// SECURITY ERASE PREPARE: no-data, optional, optional		ATA3++
		//vendor specific up to ATA2; e.g. similar to 0xE3 (IDLE)
