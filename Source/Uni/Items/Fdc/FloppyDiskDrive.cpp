...
void FloppyDiskDrive::setMotor(Time t, bool f)
{
	...
	update(t);
	motor_on = f;
	if(f) { if(sound_running_index==sound_running_size) sound_running_index = 0; }
...
void FloppyDiskDrive::audioBufferEnd(Time dt)
{
	...
	if(sound_insert_index<sound_insert_size)
	{
		...
		for(uint i=0;i<n;i++)
		{
			Dsp::audio_out_buffer[i] += sound_insert[sound_insert_index++];
		}
	}
	...
	if(sound_eject_index<sound_eject_size)
	{
		...
		for(uint i=0;i<n;i++)
		{
			Dsp::audio_out_buffer[i] += sound_eject[sound_eject_index++];
		}
	}
	...
	for(uint ii=0; ii<NELEM(sound_step_index); ii++)
	{
		...
		{
			int i = 0; if(ssi<0) { i = -ssi; ssi=0; }
			int e = min(DSP_SAMPLES_PER_BUFFER,i+sound_step_size-ssi);
			while(i<e)
			{
				Dsp::audio_out_buffer[i++] += sound_step[ssi++];
			}
		}
	}
}
