...
{ 	0xff,0x0f,0xff,0x0f,0xff,0x0f, 	// channel A,B,C period
	...
	0/*0xff*/,0/*0xff*/,0x00,		// envelope period and shape	note: some sq-tracker demos assume registers=0 after reset
	0xff,0xff 						// i/o ports
};
...
{
	envelopevolume = n;
	if (use_envelope) { volume = n; if (amp_in) output = n; }
}
...
/* ----	set envelope volume -------------------------------------
		...
		immediately sets new volume if envelope is enabled
*/
inline void Channel::setEnvelopeVolume ( uchar n )
{
	setEnvelopeSample(logVol[n]);
}
...
*/
inline Sample Channel::getOutput ( )
{
	return output;
}
...
void Envelope::trigger ( )
{
	...
	if (index & 0xF0)		// ramp finished
	{
		if (repeat)
		{
			if (invert) { index = ~index; direction = -direction; }
		}
		else
		{
			direction=0; if (!invert) index = ~index;
	...
	envelope(1/freq,&channel_A,&channel_B,&channel_C)
{
	...
	#if XXLOG
		{ logIn("AY volume table:"); for(uint i=0;i<15;i++) logline("%f",logVol[i]); }
	#endif
...
void Ay::setRegister ( Time when, uint regnr, uint8 newvalue )
{
	...
	// who is next ?
		...
		if ( envelope.when-now<0 )
		{
			...
			{ who=2; now=envelope.when; }	// envelope is next
	...
	// handle next:
		...
		//	if (AYreg[regnr]==newvalue && regnr!=13) break;		---> eel_demo !!!
			...
			case 7:
				{
					...
					if(t&1) channel_A.enableSound(c&1);	if(t&8)  channel_A.enableNoise(c&8);
					if(t&2) channel_B.enableSound(c&2);	if(t&16) channel_B.enableNoise(c&16);
					if(t&4) channel_C.enableSound(c&4);	if(t&32) channel_C.enableNoise(c&32);
					if(t&0xc0)
					{
			...
			case 14: if(ay_reg[7]&0x40 && ay_reg[14]!=newvalue) portAOutputValueChanged( when, newvalue ); break;
			case 15: if(ay_reg[7]&0x80 && ay_reg[15]!=newvalue) portBOutputValueChanged( when, newvalue ); break;
			}
...
{
	if( (addr&select_mask)==select_bits ) ay_reg_nr = byte&0x0f; else
	if( (addr&write_mask) == write_bits )
	{
...
void Ay::setClock( Frequency psg_cycles_per_second )
{
	...
	channel_A.when = now + (channel_A.when-now) / freq_factor;	channel_A.time_for_cycle /= freq_factor; channel_A.reload /= freq_factor;
	channel_B.when = now + (channel_B.when-now) / freq_factor;	channel_B.time_for_cycle /= freq_factor; channel_B.reload /= freq_factor;
	channel_C.when = now + (channel_C.when-now) / freq_factor;	channel_C.time_for_cycle /= freq_factor; channel_C.reload /= freq_factor;
	noise.when = now + (noise.when-now) / freq_factor;	noise.time_for_cycle /= freq_factor; noise.reload /= freq_factor;
	envelope.when = now + (envelope.when-now) / freq_factor;	envelope.time_for_cycle /= freq_factor; envelope.reload /= freq_factor;
}
...
//	read_data(fd,&write_bits);
	...
	fd.read(stereo_mix);
	fd.read(volume);
	fd.read(ay_reg_nr); ay_reg_nr &= 0x0f;
	uint8 n[16]; fd.read_data(n,16); setRegisters(n);
}
