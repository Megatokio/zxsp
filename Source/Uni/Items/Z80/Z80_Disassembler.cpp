...
static char cmd_00[64][3] = {{NOP, 0, 0},	{LD, BC, NN},  {LD, XBC, A},  {INC, BC, 0},	 {INC, B, 0},	 {DEC, B, 0},
...
static char c_blk[]	   = {LDI, CPI, INI,  OUTI, 0,	  0,	0, 0, LDD, CPD, IND,  OUTD, 0,	  0,
...
static char* mnemoIX(uint8 op2)
{
	...
	if (s[1] == HL)
	{
		if (op2 != 0xEB /*EX_DE_HL*/) s[1] = IX;
		return s;
	}
	if (s[2] == HL)
	{
		if (op2 != 0xEB /*EX_DE_HL*/) s[2] = IX;
		return s;
	}
	if (s[1] == H) s[1] = XH;
	if (s[1] == L) s[1] = XL;
...
static char* mnemoIY(uint8 op2)
{
	...
	if (s[1] == HL)
	{
		if (op2 != 0xEB /*EX_DE_HL*/) s[1] = IY;
		return s;
	}
	if (s[2] == HL)
	{
		if (op2 != 0xEB /*EX_DE_HL*/) s[2] = IY;
		return s;
	}
	if (s[1] == H) s[1] = YH;
	if (s[1] == L) s[1] = YL;
...
//
inline int illegalCB(uint8 op) { return op >= 0x30 && op < 0x38 ? IllegalOpcode : LegalOpcode; }
