...
static int8 c_blk[]		= { LDI,CPI,INI,OUTI,0,0,0,0,LDD,CPD,IND,OUTD,0,0,0,0, LDIR,CPIR,INIR,OTIR,0,0,0,0,LDDR,CPDR,INDR,OTDR };
...
{	static char s[20];
	...
	if (m[1])
	{	strcat ( s, "    " ); s[5]=0;	strcat ( s, word[(int)m[1]] );
		if (m[2])  { strcat ( s, "," );	strcat ( s, word[(int)m[2]] ); };
	}
	return s;
}


char* Z80::getOpcodeMnemo( uint16 ip ) const
{
	return Z80OpMnemo ( peek(ip), peek(ip+1), peek(ip+3) );
}
...
int Z80::getOpcodeLength( uint16 ip ) const
{
	return Z80OpcodeLength(peek(ip),peek(ip+1));
}
...
{	char const* il = "1111111111110101111100111111001111110001111100011011000011110000";
...
int Z80::getOpcodeLegalState( uint16 ip ) const
{
	return Z80OpLegal ( peek(ip), peek(ip+1), peek(ip+3) );
}
...
{	uint8 op;
	...
	if (m[1])
	{	strcat ( s, "    " ); s[5]=0;	strcat ( s, _xword(m[1],ip) );
		if (m[2])	{ strcat ( s,"," );	strcat ( s, _xword(m[2],ip) ); }
	}
