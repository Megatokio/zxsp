...
{
	#define FLAGS0(A)	( A&0x80 )  +  ( (A==0)<<6 )  +  (  ( (~A+(A>>1)+(A>>2)+(A>>3)+(A>>4)+(A>>5)+(A>>6)+(A>>7))&1 ) << 2  )
...
Z80::Z80( Z80_MACHINE* m, cstr name )
:	Z80_BASECLASS(m,isa_Z80,name)
{
	Z80_INFO_CTOR;
...
void Z80::init(int32 cc)
{
	...
	crtc = machine->getCrtc();	XXASSERT( crtc!=NULL );
	instr_cnt   = 0;			// current instruction counter
	cpu_cycle	= cc;
	reset_registers();
}
...
inline void c2b ( uint16 const* q, uint8* z, uint n )		{ for( uint i=0; i<n; i++ ) z[i] = q[i]; }
inline void b2c ( uint8  const* q, twobytes* z, uint n )	{ for( uint i=0; i<n; i++ ) z[i].lo = q[i]; }
...
*/
void Z80::mapMem ( uint16 addr, uint16 size, uint16* r_data, uint16* w_data, uint8 const* waitmap, int wm_size )
{
...
void Z80::mapRam ( uint16 addr, uint16 size, uint16* data, uint8 const* waitmap, int wm_size )
{
	size -= CPU_PAGESIZE;			// note: size=0  =>  size=0x10000
	...
	XXTRAP(size>0xffff-addr);		// wrap around address space end
	XXTRAP(!waitmap&&data[0]&cpu_waitmap);	// note: wartezyklen werden nur addiert, wo auch data[x]&cpu_waitmap gesetzt ist!
...
void Z80::mapWom ( uint16 addr, uint16 size, uint16* data, uint8 const* waitmap, int wm_size )
{
	size -= CPU_PAGESIZE;			// note: size=0  =>  size=0x10000
...
void Z80::mapRom ( uint16 addr, uint16 size, uint16* data, uint8 const* waitmap, int wm_size )
{
	size -= CPU_PAGESIZE;			// note: size=0  =>  size=0x10000
...
void Z80::unmapWom( uint16 addr, uint16 size, uint8 const* waitmap, int wm_size )
{
	...
	PgInfo* a = page + (addr>>CPU_PAGEBITS);
	PgInfo* e = a    + (size>>CPU_PAGEBITS);
	uint16* w = nowritepage-addr;				XXXASSERT( size_t(w)+addr == size_t(nowritepage) );	// fuckin' gcc p'ple
...
void Z80::unmapRom( uint16 addr, uint16 size, uint8 const* waitmap, int wm_size )
{
	...
	PgInfo* a = page + (addr>>CPU_PAGEBITS);
	PgInfo* e = a    + (size>>CPU_PAGEBITS);
	uint16* r = noreadpage-addr;				XXXASSERT( size_t(r)+addr == size_t(noreadpage) );	// fuckin' gcc p'ple
