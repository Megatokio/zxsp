...
case HALT:
	pc--;
	Z80_INFO_HALT;
	LOOP;							   // 4 T  ((executes NOPs))
case NOP: LOOP;						   // 4 T
case DI: IFF1 = IFF2 = disabled; LOOP; // 4 T

case EI:
	IFF1 = IFF2 = enabled; // enable maskable interrupts
	Z80_INFO_EI;
...
case EX_HL_xSP:
	w = HL; // pc:4, sp:3, sp+1:3,1, sp+1:3, sp:3,2x1		kio tested 2005-01-10/14/15
	PEEK(RL, SP);
	PEEK(RH, SP + 1);
	SKIP_1CC(SP + 1);
	POKE(SP + 1, wh);
