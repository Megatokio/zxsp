...
{ // keep the syntax checker happy
	...
	case EX_HL_xSP:
		w = rz; // pc:4, pc+1:4, sp:3, sp+1:3,1, sp+1:3, sp:3,2x1		((total:4+19; seq.: m1,m1,r,r,w,w))
		PEEK(rzl, SP);
		PEEK(rzh, SP + 1);
		SKIP_1CC(SP + 1);
		POKE(SP + 1, w >> 8);
		...
		case LD_xHL_N:
			GET_N(dis);
			GET_N(c);
			SKIP_2X1CC(pc - 1); // timing:	pc:4, pc+1:4, pc+2:3, pc+3:3,2x1, ix+n:3		korr kio 2005-01-14/15
			POKE_AND_LOOP(rz + dis, c);
		case DEC_xHL:
			GET_N(dis);
			SKIP_5X1CC(pc - 1);
