...
struct FourBytes
{
	uint8 flags3, flags2, flags1, data;
};
...
struct FourBytes
{
	uint8 data, flags1, flags2, flags3;
};
#else
  #error "endian macro"
#endif
...
struct PgInfo
{
	...
	const uint8* waitmap_r;
	const uint8* waitmap_w;
	int			 waitmap_r_size;
	int			 waitmap_w_size;
...
public:
	...
	// T cycle counter:
	int32  cpuCycle() const volatile { return cpu_cycle; }
	int32& cpuCycleRef() { return cpu_cycle; }
	void   setCpuCycle(int32 cc) { cpu_cycle = cc; }

	// Interrupt:
	int32 interruptStart() const volatile { return cc_irpt_on; }
	int32 interruptEnd() const volatile { return cc_irpt_off; }
	bool  interruptPending() const volatile { return cpu_cycle >= cc_irpt_on && cpu_cycle < cc_irpt_off; }

	void setInterrupt(int32 a, int32 e)
	{
		cc_irpt_on	= a;
		cc_irpt_off = e;
	}
	void setInterrupt_wLen(int32 a, int32 dur)
	{
		cc_irpt_on	= a;
		cc_irpt_off = a + dur;
	}
	void raiseInterrupt(int32 dur)
	{
		cc_irpt_on	= cpu_cycle;
		cc_irpt_off = cpu_cycle + dur;
	}
	void raiseInterrupt()
	{
		cc_irpt_on	= cpu_cycle;
		cc_irpt_off = 0x7fffffff;
	}
	void clearInterrupt() { cc_irpt_on = 0x7fffffff; }
	void shiftInterrupt(int32 cc)
	{
		cc_irpt_on -= cc;
		cc_irpt_off -= cc;
	}
	...
	// NMI:
	int32 nmiCycle() const volatile { return cc_nmi; }
	bool  nmiPending() const volatile { return cpu_cycle >= cc_nmi; }
	...
	Z80Regs const volatile& getRegisters() const volatile { return registers; }

	// Attach memory:
	void mapDummyRomPage(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size);
	void mapDummyWomPage(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size);
	void mapDummyWom2Page(uint16 addr, uint16 size, CoreByte* data);
	void mapRam(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size);
	void mapRom(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size);
	void mapWom(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size);
	void mapMem(uint16 addr, uint16 size, CoreByte* rdata, CoreByte* wdata, const uint8* wm, int wm_size);
	void unmapRom(uint16 addr, uint16 size, const uint8* waitmap = nullptr, int wm_size = 0);
	void unmapWom(uint16 addr, uint16 size, const uint8* waitmap = nullptr, int wm_size = 0);
	void unmapRam(uint16 addr, uint16 size, const uint8* waitmap = nullptr, int wm_size = 0);
	void unmapAllMemory()
	{
		unmapRam(0, 0);
		unmapWom2(0, 0);
	...
	void copyBufferToRam(const uint8* q, uint16 z_addr, uint16 _cnt) noexcept;
	...
	// Disassembler:
	int getOpcodeLegalState(uint16 ip) const;
	enum { legal, illegal, weird };
	int	  getOpcodeLength(uint16 ip) const;
	char* getOpcodeMnemo(uint16 ip) const;
	char* disassemble(uint16& ip) const;


	static inline void c2b(const CoreByte* q, uint8* z, uint n)
	{
		for (uint i = 0; i < n; i++) z[i] = q[i];
	}
	static inline void b2c(const uint8* q, FourBytes* z, uint n)
	{
		for (uint i = 0; i < n; i++) z[i].data = q[i];
	}
	static inline void b2c(const uint8* q, CoreByte* z, uint n)
	{
		for (uint i = 0; i < n; i++) ((FourBytes*)z)[i].data = q[i];
	}
