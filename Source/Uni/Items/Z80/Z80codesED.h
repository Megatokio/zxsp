...
{ // keep the syntax checker happy
	...
	*/
	...
	ldir:
		PEEK(c, HL); // timing:  pc:4, pc+1:4, hl:3, de:3,2x1 [de:5x1]		kio tested 2005-01-14/15
		POKE(DE, c);
	...
	ldi:
		PEEK(c, HL);
		HL += w; // timing:  pc:4, pc+1:4, hl:3, de:3,2x1		kio tested 2005-01-14/15
		POKE(DE, c);
	...
	inir:
		cc += 1; // timing: pc:4, pc+1:5, IO, hl:3,[5*1]
		INPUT(BC, c);
		POKE(w, c);
		if (--RB)
		{
	...
	ini:
		cc += 1; // timing: pc:4, pc+1:5, IO, hl:3
		INPUT(BC, c);
	...
	otir:
		cc += 1; // timing:  pc:4, pc+1:5, hl:3, IO, [hl:5*1]
		PEEK(c, w);
	...
	outi:
		cc += 1; // timing: pc:4, pc+1:5, hl:3, IO
		PEEK(c, w);
		--RB; // kio 2005-11-18: OUTI: decr B before putting it on the bus [post on css by Alvin Albrecht]
		OUTPUT(BC, c);
		rf = N_FLAG + (RB ? 0 : Z_FLAG);
