...
Z80::Z80(Machine* m) : Item(m, isa_Z80, isa_Z80, internal, nullptr, nullptr)
{
	xlogIn("new Z80");
...
*/
void Z80::copyBufferToRam(const uint8* q, uint16 z, uint16 cnt) noexcept
{
...
/*	copy buffer to cpu memory
	...
	=> if core_w2 is present, then writes to core_w2 too.		2016-02-29
*/
void Z80::copyBufferToRam(const CoreByte* q, uint16 z, uint16 cnt) noexcept
{
	uint16 n = CPU_PAGESIZE - (z & CPU_PAGEMASK);
	do {
		if (n > cnt && cnt) n = cnt;
...
*/
void Z80::mapMem(uint16 addr, uint16 size, CoreByte* r_data, CoreByte* w_data, const uint8* waitmap, int wm_size)
{
...
void Z80::mapRam(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size)
{
	size -= CPU_PAGESIZE; // note: size=0  =>  size=0x10000
	...
	assert(
		waitmap ||
		(data[0] & cpu_waitmap) == 0); // note: wartezyklen werden nur addiert, wo auch data[x]&cpu_waitmap gesetzt ist!
...
void Z80::mapDummyWomPage(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size)
{
	size -= CPU_PAGESIZE; // note: size=0  =>  size=0x10000
...
void Z80::mapWom(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size)
{
	size -= CPU_PAGESIZE; // note: size=0  =>  size=0x10000
...
void Z80::mapWom2(uint16 addr, uint16 size, CoreByte* data)
{
	...
	for (; a <= e; a++) { a->core_w2 = data; }
}


/*	map a single cpu page mirrored multiple times into address space
 */
void Z80::mapDummyRomPage(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size)
{
	size -= CPU_PAGESIZE; // note: size=0  =>  size=0x10000
...
void Z80::mapRom(uint16 addr, uint16 size, CoreByte* data, const uint8* waitmap, int wm_size)
{
	size -= CPU_PAGESIZE; // note: size=0  =>  size=0x10000
...
void Z80::unmapWom(uint16 addr, uint16 size, const uint8* waitmap, int wm_size)
{
	...
	assert(size_t(w + addr) == size_t(&nowritepage[0]));
...
void Z80::unmapRom(uint16 addr, uint16 size, const uint8* waitmap, int wm_size)
{
	...
	assert(size_t(r + addr) == size_t(&noreadpage[0]));
...
void Z80::unmapMemory(CoreByte* a, uint32 size)
{
	...
	for (int32 i = 0; i < 0x10000; i += CPU_PAGESIZE)
	{
		...
		p = wrPtr2(i);
		if (p >= a && p < e) // page.core_w2 may be null, but that's ok here
		{
			PgInfo& p = page[i >> CPU_PAGEBITS];
			p.core_w2 = nullptr;
		}
...
slow_loop:
	...
	else // handle interrupt
	{
		...
		case 0: //	mode 0: read instruction from bus
			...
			{
			case RST00:
			case RST08: //	M1: 2+5 cc: opcode RSTxx  ((RST is 5 cc))
			case RST10:
