...
/*	this class represents a SPECTRA video interface.
	...
	Display mode register:	((handled in SpectraVideo))
		...
		data byte:	%FSDB.SCLL					F=full/half cell; S=Shadow bank; D=Display bank; B=Standard/Enhanced Border;
												S=Single/Double Byte color; C=Basic/Extra colors; LL=Line height
...
:
	...
	if1_rom_hooks_enabled(settings.get_bool(key_spectra_enable_if1_rom_hooks,false)),
	rom(nullptr),
	filepath(nullptr),
...
int32 SpectraVideo::updateScreenUpToCycle( int32 cc )
{
	...
	do
	{
		...
		do
		{
			...
			*zp++ = *qp++; *zp++ = *qa1++; *zp++ = *qa2++;		// 1st pixel octet:	pixels im 1. byte; attr im 2. byte, 2nd attr im 3. byte
			*zp++ = *qp++; *zp++ = *qa1++; *zp++ = *qa2++;		// 2nd pixel octet: ""
...
int32 SpectraVideo::doFrameFlyback(int32 /*cc*/)	// called from runForSound()
{
	...
	if(screen)
	{
		...
		bool new_buffers_in_use =
			ScreenZxspPtr(screen)->ffb_or_vbi( ioinfo, ioinfo_count, attr_pixel, cc_screen_start,
									cc_per_side_border+128, get_flash_phase(), 90000/*cc_frame_end*/ );
...
void SpectraVideo::drawVideoBeamIndicator(int32 cc)		// called from runForSound()
{
	...
	{
		updateScreenUpToCycle(cc);
		bool new_buffers_in_use = ScreenZxspPtr(screen)->ffb_or_vbi( ioinfo, ioinfo_count, attr_pixel,
										cc_screen_start, cc_per_side_border+128, get_flash_phase(), cc );
...
void SpectraVideo::enableNewVideoModes(bool f)
{
	...
	if(port_7fdf==0) { new_video_modes_enabled = f; return; }
...
void SpectraVideo::setRS232Enabled(bool f)
{
	rs232_enabled = f;
}

void SpectraVideo::setJoystickEnabled(bool f)
{
	joystick_enabled = f;
}
...
void SpectraVideo::init_rom()
{
	if(rom.ptr())
	{
		if(if1_rom_hooks_enabled) { activate_hooks(); deactivateRom(); }
		else { deactivate_hooks(); activateRom(); }
	}
...
void SpectraVideo::markVideoRam()
{
	...
	e = &video_ram[0x0000]; while(v<e) { a=v; v+=0x100; if( *a&cpu_crtc) while(a<v) *a++ &= ~cpu_crtc; }
	e = &video_ram[0x1800]; while(v<e) { a=v; v+=0x100; if(~*a&cpu_crtc) while(a<v) *a++ |=  cpu_crtc; }
	e = &video_ram[a1[i]];  while(v<e) { a=v; v+=0x100; if( *a&cpu_crtc) while(a<v) *a++ &= ~cpu_crtc; }
	e = &video_ram[e1[i]];  while(v<e) { a=v; v+=0x100; if(~*a&cpu_crtc) while(a<v) *a++ |=  cpu_crtc; }
	e = &video_ram[a2[i]];  while(v<e) { a=v; v+=0x100; if( *a&cpu_crtc) while(a<v) *a++ &= ~cpu_crtc; }
	e = &video_ram[e2[i]];  while(v<e) { a=v; v+=0x100; if(~*a&cpu_crtc) while(a<v) *a++ |=  cpu_crtc; }
	e = &shadowram[0x7FFF]; while(v<e) { a=v; v+=0x100; if( *a&cpu_crtc) while(a<v) *a++ &= ~cpu_crtc; }
}
