...
/*	creator for use by derived classes:
	for models with mmu
*/
MmuZxsp::MmuZxsp(Machine* m, isa_id id, cstr o_addr, cstr i_addr) : Mmu(m, id, o_addr, i_addr)
{
	xlogIn("new MmuZxsp");
}
...
*/
MmuZxsp::MmuZxsp(Machine* m) : Mmu(m, isa_MmuZxsp, 0, 0) { xlogIn("new MmuZxsp"); }


MmuZxsp::~MmuZxsp() { xlogIn("~MmuZxsp"); }
