...
UlaInves::UlaInves ( Machine* m )
:
	UlaZxsp(m,isa_UlaInves,io_addr,io_addr)
{
	xlogIn("new UlaInves");
	...
	// insert bit from mic socket
		...
		else								// EAR_OUT==0
		{
			...
			// TODO: simulate capacitor
				...
				Sample threshold = (ula_out_byte>>MIC_OUT_SHIFT)&1
					? machine->model_info->earin_threshold_mic_lo
...
int32 UlaInves::addWaitCycles( int32 cc, uint16 /*addr*/) volatile const
{
	return cc;
}

uint8 UlaInves::getFloatingBusByte( int32 /*cc*/)
{
	return 0xff;
}
