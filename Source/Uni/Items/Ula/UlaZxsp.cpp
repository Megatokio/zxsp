...
// Waitmap:
	...
	if(wm)
	{
		...
		for(int d=0,i=7; i>=0; i--) { waitmap[i] = wm&1 ? ++d : (d=0); wm>>=1; }
...
*/
int32 UlaZxsp::addWaitCycles( int32 cc, uint16 addr ) volatile const
{
...
// --- ear in ---
	...
	Sample threshold = ula_out_byte & MIC_OUT_MASK
		? earin_threshold_mic_hi
		: earin_threshold_mic_lo;
...
int32 UlaZxsp::updateScreenUpToCycle( int32 cc )
{
	...
	do
	{
		...
		do
		{
			...
			*zp++ = *qp++; *zp++ = *qa++;		// pixels im 1. byte; attr im 2. byte
			*zp++ = *qp++; *zp++ = *qa++;
...
{
	uint int_dur = machine->model==pentagon128
			? 36  // 36:	pentagon128.gif from sblive.narod.ru
...
void UlaZxsp::drawVideoBeamIndicator(int32 cc)	// called from runForSound()
{
	...
	bool new_buffers_in_use = ScreenZxspPtr(screen)->ffb_or_vbi( ioinfo, ioinfo_count, attr_pixel, cc_screen_start,
													   cc_per_side_border+128, getFlashPhase(), cc );
