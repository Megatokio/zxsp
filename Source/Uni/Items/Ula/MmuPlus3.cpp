...
zx128 or plus3 in normal mode:
	...
	3:	Select normal (0) or shadow (1) screen to be displayed. The normal screen is in bank 5, whilst the shadow screen is in bank 7.
...
static
const int ram_only_mode_page_table[] = { 0,1,2,3, 4,5,6,7, 4,5,6,3, 4,7,6,3 };
...
uint MmuPlus3::getPageC000() volatile const noexcept	// for UlaInsp
{
	return isRamOnlyMode() ? ram_only_mode_page_table[2*(port_1ffd&6)+3] : port_7ffd&7;
}

uint MmuPlus3::getPage8000() volatile const  noexcept	// for UlaInsp
{
	return isRamOnlyMode() ? ram_only_mode_page_table[2*(port_1ffd&6)+2] : 2;
}

uint MmuPlus3::getPage4000() volatile const  noexcept	// for UlaInsp
{
	return isRamOnlyMode() ? ram_only_mode_page_table[2*(port_1ffd&6)+1] : 5;
}

uint MmuPlus3::getPage0000() volatile const  noexcept	// for UlaInsp
{
...
{
	int const* pp = ram_only_mode_page_table + 2*(port_1ffd&6);
...
void MmuPlus3::saveToFile( FD& fd ) const noexcept(false) /*FileError,bad_alloc*/
{
...
void MmuPlus3::loadFromFile( FD& fd ) noexcept(false) /*FileError,bad_alloc*/
{
