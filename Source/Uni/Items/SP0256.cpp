	...
	bits(0)
{
	...
	if(is_bitswapped) for(uint i=0;i<2048;i++) { uint8 b = rom[i]; rom[i] = X8(b); }
...
SP0256::~SP0256()
{
	...
	if(PRINT_STATISTICS)
	{
		...
		for(uint i=5;i<64;i++)
		{
			...
			logline("  %s%s: max. %.4f - c1: %.4f - c2: %.4f",
				al2_allophone_names[i], al2_allophone_names[i][2]?"":" ",
				s.max_sample,
				s.max_sample_at_c1,
				s.max_sample_at_c2);
		}
...
void SP0256::set_clock(Frequency xtal)
{
	time_per_sample = 312.0 / xtal;
}
...
{
	if(bits<n) { byte += rom[pc++ & 0x7ff] << bits; bits += 8; }
	bits -= n;
...
*/
	...
	{
		pc = stack; stack = byte = bits = 0;	// pop address
		if(pc)
		{
		...
		if(command_valid)
		{
			xxlogline("SP0256: RTS: next command = %u = %s", command, command<64?al2_allophone_names[command]:"???");
			pc = 0x1000 + (command<<1);
			command_valid = no;
			stand_by = no;
...
*/
	...
	if(mode&1)	// x1
	{
		B3 = nextL(6)>>1;	F3 = nextL(7);
		B4 = next8();		F4 = next8();
	}
	else		// x0
	{
		B3 = nextL(4)>>1;	F3 = nextL(6);
		B4 = nextL(7);		F4 = nextL(6);
	}
...
*/
	...
	if(mode&1)	// x1
	{
		B0 = nextL(6)>>1;	F0 = nextL(6);
		B1 = nextL(6)>>1;	F1 = nextL(6);
		B2 = nextL(6)>>1;	F2 = nextL(6);
		B3 = nextL(6)>>1;	F3 = nextL(7);
		B4 = next8();		F4 = next8();
	}
	else		// x0
	{
		B0 = nextL(3)>>1;	F0 = nextL(5);
		B1 = nextL(3)>>1;	F1 = nextL(5);
		B2 = nextL(3)>>1;	F2 = nextL(5);
		B3 = nextL(4)>>1;	F3 = nextL(6);
		B4 = nextL(7);		F4 = nextL(6);
	}
	if(mode&2)	// 1x
...
*/
	...
	if(mode&1)	// x1
	{
		B0 += nextSL(4)>>3;	F0 += nextSL(4)>>2;
		B1 += nextSL(4)>>3;	F1 += nextSL(4)>>2;
		B2 += nextSL(4)>>3;	F2 += nextSL(4)>>2;
		B3 += nextSL(4)>>3;	F3 += nextSL(5)>>2;
		B4 += nextSL(5)>>3;	F4 += nextSL(5)>>3;
	}
	else		// x0
	{
		B0 += nextSL(3)>>1;	F0 += nextSL(3)>>2;
		B1 += nextSL(3)>>1;	F1 += nextSL(3)>>2;
		B2 += nextSL(3)>>1;	F2 += nextSL(3)>>2;
		B3 += nextSL(3)>>2;	F3 += nextSL(4)>>2;
		#if 0
		B4 += nextSL(4)>>2;	F4 += nextSL(4)>>2;	// Zbiciak: 6 bits		i can't hear a difference,
		#else
		B4 += nextSL(4)>>2;	F4 += nextSL(4)>>3;	// MAME: 7 bits			but i tend to MAME beeing right
		#endif
	}

	if(mode&2)
	{
		B5 += nextSL(5)>>3;	F5 += nextSL(5)>>3;
	}
}
...
void SP0256::run_statemachine(Time t)
{
	...
	for(;;)
	{
		if(stand_by && command_valid)
		{
			...
			xxlogline("SP0256: SBY: next command = %u = %s",
		...
		else	// load next microcode
		{
			...
			if(repeat==0) { logline("SP0256: repeat=0"); continue; }
		...
		for(; repeat; --repeat)
		{
			...
			for(_i = pitch ? pitch : 0x40; _i; _i--)
			{
				...
				// run pulse through filters and update feedback values:
				{
					...
					for(uint j=0;j<12;)
					{
						z0 += _z[j] * _c[j] / 512; _z[j] = _z[j+1];	j++;
						z0 += _z[j] * _c[j] / 256; _z[j] = z0;		j++;
					}
...
//	float64 ff = 1.0 - exp( -(1.0/samples_per_second) / (33e3 * 22e-9) );
	...
	if(a==e)
	{
		...
		sample_at_c2 += d = (sample_at_c1-sample_at_c2) * ff2; sample_at_c1 -= d;
	...
	else
	{
		...
		while( a<e )
		{
			...
			sample_at_c2 += d = (sample_at_c1-sample_at_c2) * ff; sample_at_c1 -= d;
			if(fabsf(sample_at_c2)>2.0f) xxlog("sample>2");
...
void SP0256::disassAllophones()
{
	...
	for(uint i=0;i<64;i++)
	{
		...
		pc = i<<1;
		byte=0;bits=0;repeat=0;
		stack=0;mode=0;page=0;
		command_valid=no;
		stand_by=no;
		...
		while(!stand_by)
		{
			...
			repeat += (instr&15);
			log("%2u: %s: m=%u, r=%u%s ",
				current_opcode, opcode_names[current_opcode],
				mode, repeat,repeat>9?"":" ");
			repeat=0;
...
*/
	...
	else		// x0
	{
		...
		B2 += nextSL(3)>>1;	F2 += nextSL(3)>>2;
		B3 += nextSL(3)>>2;	F3 += nextSL(4)>>2;
		B4 += nextSL(4)>>2;	F4 += nextSL(4)>>3;	// Zbiciak: 6 bits, MAME: 7 bits
	}
