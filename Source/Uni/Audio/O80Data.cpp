	...
	TapeData(isa_O80Data)
{
	switch(q.isaId())
	{
	case isa_TapData:   new(this) O80Data(); trust_level = conversion_failed; break;
...
*/
O80Data::O80Data( CswBuffer const& bu )
:
	TapeData(isa_O80Data),
	is_zx80(no),
	is_zx81(no)
{
	...
	*/
	...
	uint const ccpulse		= (zx81lo+zx81hi)/2;
	uint const ccpulse_max	= ccpulse*3/2;
	uint const cclooo_min	= zx81ooo - 2*ccpulse;
	uint const cclooo_max	= zx81ooo + 2*ccpulse;
...
start:
	...
	n=0; do { n++; cc = bu.readPulse(); } while(cc && cc<=ccpulse_max);
	if(cc==0)
	{
		return; /* no_data */
	}
	...
	b:	byte += byte + (n>13);
		if(byte&0x100) { data[zi++] = byte; byte=1;}
	...
	is_zx81 =
		namelen<zi &&													// not stopped at buffer end
		data[namelen]>=0x80 && data[namelen]<0xC0 && ++namelen<128 &&	// last char of file name and name length
	...
	if(is_zx80||is_zx81)
	{
		if(is_zx80&&is_zx81)
		{
			...
			if(zi==zx81_len) is_zx81=yes; else		// Länge passt exakt: das wird's wohl sein
			if(zi==zx80_len) is_zx80=yes; else		// Länge passt exakt: das wird's wohl sein
			// sonst Heuristik:
			if(namelen>31)   is_zx80=yes; else		// unglaublich langer Name => ZX81 unwahrscheinlich
			if(zi>4 kB)      is_zx81=yes; else		// > 4kB => ZX80 ist unwahrscheinlich wg. orig. ZX80 Rampack
			// letzte Idee: wer weniger Bytes zuviel hat:
			if(zx80_len > zx81_len-namelen+9) is_zx80=yes; else is_zx81=yes;
		}
		...
		trust_level = data.last()==0x80
			? decoded_data
			: checksum_error;		// last byte of VARS != 0x80
	...
	data.shrink(zi);
	if(byte==1 && zi>=32 && cc>cclooo_max) { trust_level = truncated_data_error; return; }
	if(zi>0) { trust_level = truncated_data_error; return; }

	data.purge();
	if(zi==0 && gap_a_pulses>666) { trust_level = conversion_failed; return; }
	trust_level = no_data;
}
...
//
inline
int zx81_progname_len( cu8ptr p, int n )
{
	if(n>127) n=127;
...
static
cstr zx81_progname_str( cu8ptr p, int n )
{
...
CswBuffer::CswBuffer(O80Data const& o80data, uint32 ccps)
:
