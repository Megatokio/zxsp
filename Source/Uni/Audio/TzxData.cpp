...
// timing constants:
//
static const Time SPCC = 1.0 / 3500000; // seconds per cpu cycle @ 3.5MHz

// timing constants for ZX Spectrum:
//
static const uint32 cc_zxsp_pilot = 2168; // cpu cycles per pilot half phase
static const uint32 cc_zxsp_bit0  = 855;  // cpu cycles per 0-bit half phase
static const uint32 cc_zxsp_bit1  = 1710; // cpu cycles per 1-bit half phase
static const uint32 cc_zxsp_sync1 = 667;  // cpu cycles for 1st sync phase
static const uint32 cc_zxsp_sync2 = 735;  // cpu cycles for 2nd sync phase
...
//
static const uint32 cc_jup_pilot = 2011 * 350 / 325; // 2011 @ 3.25 MHz
static const uint32 cc_jup_sync1 = 601 * 350 / 325;	 // 601
static const uint32 cc_jup_sync2 = 791 * 350 / 325;	 // 791
static const uint32 cc_jup_bit0	 = 798 * 350 / 325;	 // 1:795,  0:801
static const uint32 cc_jup_bit1	 = 1588 * 350 / 325; // 1:1585, 0:1591
...
TzxBlock* TzxBlock::readFromFile(FD& fd, uint stopper) noexcept(false) // DataError, file_error
{
	...
	while (p) // p==nullptr -> eof
	{
		...
		if (stopper)
		{
			if (p->id == stopper) break;
		}
		else
		{
			if (p->is_end_block()) break;
		}
...
{
	for (const TzxBlock* p = this; p; p = p->next)
	{
		fd.write_uint8(p->id);
		p->write(fd);
	}
...
{
	for (const TzxBlock* p = this; p; p = p->next) { p->write(bu); }
}
...
void TzxBlock10::read(FD& fd)
{
	...
	if (cnt == 0) throw DataError("tzx block 0x10: count==0");
	data = new uint8[cnt];
	fd.read_bytes(data, cnt);
}
...
void TzxBlock12::write(CswBuffer& bu) const { bu.writePureTone(num_pulses, cc_per_pulse * SPCC); }
...
void TzxBlock13::read(FD& fd)
{
	...
	data	  = new uint16[numpulses];
	fd.read_data(data, numpulses);
}
...
struct TzxBlock18 : public TzxBlock
{
	...
	TzxBlock18(const CswBuffer&, bool last_pulse_is_pause);
...
void TzxBlock18::write(CswBuffer& bu) const
{
	...
	for (uint i = 0; i < blen - 10; i++)
	{
		...
		{
			bu.writePulse(peek4Z(data + (i + 1)) * sps);
			i += 4;
		}
	}
...
void SymDef::read(FD& fd, uint max) // read from file
{
	...
	flags = fd.read_uint8();
	xlogline("flags = %i", int(flags));
	pulses = 0;
	data   = new uint16[max + 1];
	memset(data, 0, (max + 1) * 2); // add final stopper if cnt==max
	uint n = 0;
	uint i = 0;
	xxlog("cc = ");
...
{
	if (p_symdef)
		for (uint i = 0; i < p_symdef_cnt; i++) delete[] p_symdef[i].data;
	if (d_symdef)
		for (uint i = 0; i < d_symdef_cnt; i++) delete[] d_symdef[i].data;
	delete[] p_symdef;
	delete[] p_data;
	delete[] d_symdef;
	delete[] d_data;
...
void TzxBlock19::read(FD& fd)
{
	...
	blen		 = fd.read_uint32_z();
	uint32 bend	 = fd.file_position() + blen;
	pause_ms	 = fd.read_uint16_z();
	p_data_cnt	 = fd.read_uint32_z();
	p_symdef_max = fd.read_uint8();
	if (p_symdef_max == 0) p_symdef_max = 256;
	p_symdef_cnt = fd.read_uint8();
	if (p_symdef_cnt == 0) p_symdef_cnt = 256;
	d_data_cnt	 = fd.read_uint32_z();
	d_symdef_max = fd.read_uint8();
	if (d_symdef_max == 0) d_symdef_max = 256;
	d_symdef_cnt = fd.read_uint8();
	if (d_symdef_cnt == 0) d_symdef_cnt = 256;
...
void TzxBlock19::write(CswBuffer& bu) const
{
	...
	for (uint32 i = 0; i < p_data_cnt; i++)
		for (uint r = get_p_repeat(i); r; r--) get_p_symbol(i).store_pulses(bu);
...
void TzxBlock20::write(FD& fd) const { fd.write_uint16_z(pause_ms); }

void TzxBlock20::write(CswBuffer& bu) const { bu.writeTzxPause(pause_ms ? pause_ms * 0.001 : 5.0); }
...
{
	if (blocks)
		for (uint r = 0; r < cnt; r++) blocks->storeCsw(bu);
}
...
void TzxBlock2A::write(FD& fd) const { fd.write_uint32_z(len); }

void TzxBlock2A::write(CswBuffer& bu) const { bu.writeTzxPause(5.0); }
...
void TzxBlock2B::write(CswBuffer& bu) const { bu.setPhase(phase); }
...
struct TzxBlock32 : public TzxBlock
{
	...
	{
		if (texte)
			for (int i = 0; i < cnt; i++) { delete[] texte[i]; }
		delete[] texte;
	}
	void read(FD&);
	void write(FD&) const;
...
void TzxBlock32::read(FD& fd)
{
	...
	cnt	  = fd.read_uint8();
	texte = new str[cnt + 1];
	memset(texte, 0, sizeof(str) * (cnt + 1));
...
void TzxBlock33::read(FD& fd)
{
	...
	cnt	 = fd.read_uint8();
	data = new uint8[cnt * 3];
	fd.read_bytes(data, cnt * 3);
}
...
a:
	if (fd.file_remaining() == 0) return nullptr;
	...
	case 0x28:
		xlogline("readTzxBlock: skipping block 0x28 SELECT_BLOCK"); // ignore
		...
		fd.skip_bytes(fd.read_uint16_z());
		goto a;
	...
	case 0x35:															 // CUSTOM_INFO_BLOCK deprecated in tzx 1.20
		...
		fd.skip_bytes(16);
		fd.skip_bytes(fd.read_uint32_z());
		goto a;
...
*/
TzxBlock10::TzxBlock10(const TapData& q) : TzxBlock(0x10, yes), data(nullptr)
{
	pause_ms = uint16(minmax(1.0, q.pause * 1000 + 0.5, 10000.0));
	cnt		 = q.count();
	data	 = new uint8[cnt];
	memcpy(data, q.getData(), cnt);
}
...
/*	convert TapData to Block11: turbo speed tape block
	...
	• "next level" after this block is 0 (as after TzxPause)
*/
TzxBlock11::TzxBlock11(const TapData& q) : TzxBlock(0x11, yes), data(nullptr)
{
	...
	data(nullptr)
{
	...
	const uint16* q = bu.getData();
	const uint16* e = q + bu.getTotalPulses();

	if (q == e)
	{
		xlogline("num_pulses=0");
		return;
	}
	...
	if (last_pulse_is_pause)
	{
		...
		if (q == e)
		{
			xlogline("num_pulses=0 after replacing last pulse with pause");
			return;
		}
	}
...
TzxData::~TzxData() { delete data; }


TzxData::TzxData(TzxBlock* data, TrustLevel trustlevel) : TapeData(isa_TzxData, trustlevel), data(data) {}
...
*/
// static
void TzxData::writeFile(cstr fpath, TapeFile& tapeblocks, TzxConversionStyle style) noexcept(
		...
		*/
		...
		case TzxConversionIdealize:
			tzxdata = (tapdata = block->getTapData()) && tapdata->trust_level >= conversion_success ?
						  new TzxData(*tapdata) :
					  (o80data = block->getO80Data()) && o80data->trust_level >= conversion_success ?
						  new TzxData(*o80data) :
		...
		case TzxConversionExact:
			tzxdata = tzxdata && tzxdata->trust_level == original_data ?
						  tzxdata : // may be already a conversion from tap|o80
						  (tapdata = block->getTapData()) && tapdata->trust_level == original_data ?
						  new TzxData(*tapdata) :
					  (o80data = block->getO80Data()) && o80data->trust_level == original_data ?
						  new TzxData(*o80data) :
						  new TzxData(*block->cswdata->normalize(), style);
			break;
		}
...
{
	for (const TzxBlock* p = data; p; p = p->next)
	{
...
*/
TzxData::TzxData(const TapData& q) : TapeData(isa_TzxData, q.trust_level), data(nullptr)
{
...
*/
TzxData::TzxData(const O80Data& q) : TapeData(isa_TzxData, q.trust_level), data(nullptr) { data = new TzxBlock19(q); }
...
*/
CswBuffer::CswBuffer(const TzxData& tzxdata, uint32 ccps) : CswBuffer(ccps, 0, 666)
{
...
*/
TapData::TapData(const TzxData& /*q*/) : TapData(no, no) { TODO(); }
