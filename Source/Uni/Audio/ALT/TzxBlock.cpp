...
// timing constants:
//
static Frequency const cc_per_second = 3500000.0;	// cpu cycles per second
static uint32 const	cc_per_ms       = cc_per_second/1000;
...
// -------------------------------------------------------------
// Tzx Block base class

TzxBlock::TzxBlock(uint id, bool has_data)
:
	id(id),
	next(NULL),
	has_data(has_data)
{
	XXLogLine("Block%2x",id);
}
...
struct Block10 : public TzxBlock
{
	uint16  pause_ms;   // pause after this block [ms] {1000}
	uint16  cnt;        // num. bytes that follow
	uint8*	data;

	Block10()					:TzxBlock(0x10,yes),data(NULL){}
	Block10(TapData const&);
	~Block10()					{ delete[] data; }
	void read	(FD& fd);
...
void Block10::read(FD &fd)
{
	XXLogIn("Block 0x10: standard tape block");
	XXXASSERT(data==NULL);
...
void Block10::write(FD &fd) const
{
...
void Block10::write(CswBuffer& bu) const
{
	...
	if(XLOG && bu.getCurrentPhase()) LogLine("tzx block 0x10: inverted signal");

	bu.writePureTone( data[0]&0x80 ? 3223 : 8063, cc_zxsp_pilot );		// as specified
	bu.writePulse   ( cc_zxsp_sync1 );
	bu.writePulse   ( cc_zxsp_sync2 );
	bu.writePureData( data, cnt*8, cc_zxsp_bit0, cc_zxsp_bit1 );
	bu.writeTzxPause( pause_ms*cc_per_ms );
}
...
public:
		 Block11()				:TzxBlock(0x11,yes),data(NULL){}
		 Block11(TapData const&);
		~Block11()				{ delete[] data; }
	void read(FD& fd);
	void write(FD &fd)			const;
	void write(CswBuffer&)		const;
...
void Block11::read(FD &fd)
{
	XXLogIn("Block 0x11: turbo speed tape block");
	XXXASSERT(data==NULL);
	...
	XXLogLine("cc_pilot = %i",(int)cc_pilot);
	XXLogLine("cc_sync1 = %i",(int)cc_sync1);
	XXLogLine("cc_sync2 = %i",(int)cc_sync2);
	XXLogLine("cc_bit0  = %i",(int)cc_bit0);
	XXLogLine("cc_bit1  = %i",(int)cc_bit1);
	XXLogLine("ppulses  = %i",(int)ppulses);
	XXLogLine("lastbits = %i",(int)lastbits);
	XXLogLine("pause_ms = %i",(int)pause_ms);
	XXLogLine("data cnt = %i",(int)cnt);
}

void Block11::write(FD& fd) const
{
...
void Block11::write(CswBuffer& bu) const
{
	if(XLOG && bu.getCurrentPhase()==(ppulses&1)) LogLine("tzx block 0x11: inverted signal");

	bu.writePureTone( ppulses, cc_pilot );
	bu.writePulse   ( cc_sync1 );
	bu.writePulse   ( cc_sync2 );
	bu.writePureData( data, (cnt-1)*8+lastbits, cc_bit0, cc_bit1 );
	bu.writeTzxPause( pause_ms*cc_per_ms );
}
...
struct Block12 : public TzxBlock
{
	uint16  cc_per_pulse;
	uint16  num_pulses;

	Block12()					:TzxBlock(0x12,yes){}
	Block12(uint16 cc, uint16 n):TzxBlock(0x12,yes),cc_per_pulse(cc),num_pulses(n){}
	void read(FD&);
	void write(FD&)				const;
	void write(CswBuffer&)		const;
};

void Block12::read(FD &fd)
{
	XXLogIn("Block 0x12: pure tone");
	cc_per_pulse = fd.read_uint16_z();
	num_pulses   = fd.read_uint16_z();
}

void Block12::write(FD& fd) const
{
	fd.write_uint16_z(cc_per_pulse);
	fd.write_uint16_z(num_pulses);
}

void Block12::write(CswBuffer& bu) const
{
	bu.writePureTone(num_pulses,cc_per_pulse);
}
...
struct Block13 : public TzxBlock
{
	uint8	numpulses;		// num pulses
	uint16*	data;			// cc_per_pulse

	Block13()				:TzxBlock(0x13,yes),data(0){}
    ~Block13()				{ delete[] data; }
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
void Block13::read(FD &fd)
{
	XXLogIn("Block 0x13: pulses of various length");
	XXXASSERT(data==NULL);
...
void Block13::write(FD& fd) const
{
	fd.write_uint8(numpulses);
	fd.write_data(data,numpulses);
}

void Block13::write(CswBuffer& bu) const
{
	for(uint i=0;i<numpulses;i++) bu.writePulse(peek2Z(data+i));
}
...
struct Block14 : public TzxBlock
{
	uint16  cc_bit0;        // 855
	uint16  cc_bit1;        // 1710
	...
	Block14()				:TzxBlock(0x14,yes),data(0){}
	~Block14()				{ delete[] data; }
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
void Block14::read(FD &fd)
{
	XXLogIn("Block 0x12: pure data block");
	XXXASSERT(data==NULL);
...
void Block14::write(FD& fd) const
{
	fd.write_uint16_z(cc_bit0);
	fd.write_uint16_z(cc_bit1);
	fd.write_uint8   (lastbits);
...
void Block14::write(CswBuffer& bu) const
{
...
struct Block15 : public TzxBlock
{
	...
	Block15()				:TzxBlock(0x15,yes),data(0){}
	~Block15()				{ delete[] data; }
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
void Block15::read(FD& fd)
{
	XXLogIn("Block 0x15: direct recording (samples)");
	XXXASSERT(!data);
	...
	if(cc_per_sample<8000) throw data_error(usingstr("tzx block 0x15: sample rate too low: %u",(uint)cc_per_sample));
...
void Block15::write(FD& fd) const
{
...
void Block15::write(CswBuffer& bu) const
{
	bool bit = data[0]>>7;
	...
	while(i<cnt-1 || ((i==cnt-1)&&j<lastbits))
	{
		...
		if(++j==8) { i++; j=0; }
		if(b==bit) n++;
		else { bu.writePulse(n*cc_per_sample,bit); bit=b; n=1; }
	}
	bu.writePulse(n*cc_per_sample,bit);

	if(pause_ms) bu.writeTzxPause(pause_ms*cc_per_ms);
	else		 bu.writePulse(0);       // -> current level := not toggled
}
...
struct Block18 : public TzxBlock
{
	...
	Block18()				:TzxBlock(0x18,yes),data(NULL){}
	Block18(CswBuffer const&, bool last_pulse_is_pause);
	~Block18()				{ delete[] data; }
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
void Block18::read(FD &fd)
{
	XXLogIn("Block 0x18: CSW recording");
	XXXASSERT(!data);
...
void Block18::write(FD& fd) const     // TODO: compress data before writing
{
...
void Block18::write(CswBuffer& bu) const
{
	float64 cc_per_sample = cc_per_second/sam_per_sec;
	for(uint i=0;i<blen-10;i++)
	{
		if(data[i]) { bu.writePulse( (data[i]+0.5)*cc_per_sample ); }
		else { bu.writePulse( (peek4Z(data+(i+1))+0.5)*cc_per_sample ); i += 4; }
	}

	if(pause_ms) bu.writeTzxPause(pause_ms*cc_per_ms);
	else		 bu.writePulse(0);      // last pulse level := don't toggle
}
...
void SymDef::read(FD &fd, uint max)     // read from file
{
	...
	uint i = 0;
	XXLogLine("cc = ");
	while( i<max && (n=fd.read_uint16_z()) )		// don't delete: there's an action in the condition!
...
void SymDef::store_pulses(CswBuffer& bu) const  // store pulses into CswBuffer
{
	...
	switch(flags)
	{
	case 1:	bu.appendToPulse(*d++); break;
	case 2:
	case 3:	// if(bu.getCurrentPhase()!=(flags&1))
			// LogLine("store_pulse appends to pulse");
			bu.writePulse(*d++,flags&1); break;
	default: break;
	}
	while(*d) { bu.writePulse(*d++); }
}
...
struct Block19 : public TzxBlock  // Generalized d_data block
{
	uint32  blen;           // block length (without these 4 bytes)
	uint16  pause_ms;       // pause after this block [ms]
	...
	Block19		()              :TzxBlock(0x19,yes),p_symdef(0),p_data(0),d_symdef(0),d_data(0){}
	Block19		(O80Data const&);
	~Block19	();
	void read	(FD& fd);
	void write	(FD &fd)	const;
...
void Block19::read(FD &fd)
{
	XXLogIn("Block 0x19: generalized data block");
	XXXASSERT(p_symdef==NULL);
	XXXASSERT(p_data==NULL);
	XXXASSERT(d_symdef==NULL);
	XXXASSERT(d_data==NULL);
	...
	XXLogLine("blen = %i",blen);
	XXLogLine("pause_ms = %i",pause_ms);
	XXLogLine("p_data_cnt = %i",p_data_cnt);
	XXLogLine("p_symdef_max = %i",p_symdef_max);
	XXLogLine("p_symdef_cnt = %i",p_symdef_cnt);
	XXLogLine("d_data_cnt = %i",d_data_cnt);
	XXLogLine("d_symdef_max = %i",d_symdef_max);
	XXLogLine("d_symdef_cnt = %i",d_symdef_cnt);
	...
	XXXASSERT(fd.file_position()==bend);
}

void Block19::write(FD& fd) const
{
	 fd.write_uint32_z(blen);
	 fd.write_uint16_z(pause_ms);
...
void Block19::write(CswBuffer& bu) const
{
	// write pilot&sync:
	for(uint32 i=0;i<p_data_cnt;i++) for( uint r=get_p_repeat(i);r;r-- ) get_p_symbol(i).store_pulses(bu);

	// write data:
	for(uint32 i=0;i<d_data_cnt;i++) get_d_symbol(i).store_pulses(bu);
...
struct Block20 : public TzxBlock
{
	uint16      pause_ms;

	Block20()				:TzxBlock(0x20,yes),pause_ms(0){}
	Block20(uint16 ms)		:TzxBlock(0x20,yes),pause_ms(ms){}
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
void Block20::read(FD &fd)
{
	XXLogIn("Block 0x20: pause");
	pause_ms = fd.read_uint16_z();
}

void Block20::write(FD& fd) const
{
	fd.write_uint16_z(pause_ms);
}

void Block20::write(CswBuffer& bu) const
{
	bu.writeTzxPause(pause_ms ? pause_ms*cc_per_ms : 5000*cc_per_ms);
}
...
public:
	Block21()				:TzxBlock(0x21,yes),text(NULL),blocks(NULL){}
	~Block21()				{ delete[]text; delete blocks; }
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
...
bool Block21::is_end_block() const
{
	bool f = no;
...
void Block21::read(FD &fd)
{
	XXLogIn("Block 0x21: group start");
	XXXASSERT(!text);
	XXXASSERT(!blocks);
...
void Block21::write(FD& fd) const
{
	fd.write_uint8(len);
...
void Block21::write(CswBuffer& bu) const
{
...
struct Block22 : public TzxBlock
{
	Block22()				:TzxBlock(0x22,no){}
	void read(FD&)			{ XXLogLine("Block 22: group end"); }
	void write(FD&)			const {}
};
...
// endet dieser Block mit "Pause"?
bool Block24::is_end_block() const
{
	bool f = no;
	for(TzxBlock* p = blocks; p; p=p->next)
	{
...
void Block24::read(FD &fd)
{
	XXLogIn("Block 0x24: loop start");
	XXXASSERT(!blocks);
...
void Block24::write(FD& fd) const
{
	fd.write_uint16_z(cnt);
	if(blocks) blocks->writeToFile(fd);
}

void Block24::write(CswBuffer& bu) const
{
...
struct Block25 : public TzxBlock
{
	Block25()			:TzxBlock(0x25,no){}
	void read(FD&)		{ XXLogLine("Block 25: loop end"); }
	void write(FD&)		const {}
};
...
struct Block2A : public TzxBlock
{
	uint32  len;

	Block2A()				:TzxBlock(0x2A,yes){}
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
	bool is_end_block()		const { return yes; }
...
void Block2A::read(FD& fd)
{
	XXLogIn("Block 0x2A: stop the tape in 48k mode");
	len = fd.read_uint32_z();
	if(len!=0) throw data_error("tzx block 0x2A: wrong len");
}

void Block2A::write(FD& fd) const
{
	fd.write_uint32_z(len);
}

void Block2A::write(CswBuffer& bu) const
{
	bu.writeTzxPause(2000*cc_per_ms);
}
...
struct Block2B : public TzxBlock
{
	uint32  len;    // 1
	uint8   phase;  // 0 or 1

	Block2B()					:TzxBlock(0x2B,yes){}
	Block2B(bool p)				:TzxBlock(0x2B,yes),len(1),phase(p){}
	void read(FD&);
	void write(FD&)			const;
	void write(CswBuffer&)	const;
};

void Block2B::read(FD& fd)
{
	XXLogIn("Block 0x2B: set signal level");
...
void Block2B::write(FD& fd) const
{
	fd.write_uint32_z(len);
	fd.write_uint8(phase);
}

void Block2B::write(CswBuffer& bu) const
{
	bu.setPhase(phase);
}
...
struct Block30 : public TzxBlock
{
	uint8   len;
	str     text;

	Block30()               :TzxBlock(0x30,no),text(NULL){}
	~Block30()              { delete[]text; }
	void read(FD&);
	void write(FD&)			const;
	cstr get_info()			const { return text; }
...
void Block30::read(FD& fd)
{
	XXLogIn("Block 0x30: textual description");
	XXXASSERT(!text);
...
void Block30::write(FD& fd) const
{
	fd.write_uint8(len);
	fd.write_bytes(text,len);
}
...
struct Block31 : public TzxBlock
{
	uint8   time;       // time to display [sec]
	uint8   len;
	str     text;

	Block31()				:TzxBlock(0x31,no),text(NULL){}
	~Block31()				{ delete[] text; }
	void read(FD&);
	void write(FD&)			const;
...
void Block31::read(FD& fd)
{
	XXLogIn("Block 0x31: message");
	XXXASSERT(!text);
...
void Block31::write(FD& fd) const
{
	fd.write_uint8(time);
	fd.write_uint8(len);
	fd.write_bytes(text,len);
...
struct Block32 : public TzxBlock
{
	...
	Block32()			:TzxBlock(0x32,no),cnt(0),texte(NULL){}
	~Block32()			{ if(texte) for(int i=0;i<cnt;i++) { delete[] texte[i]; } delete[] texte; }
	void read(FD&);
	void write(FD&)		const;
};

void Block32::read(FD& fd)
{
	XXLogIn("Block 0x32: archive info");
	XXXASSERT(!texte);
...
void Block32::write(FD& fd) const
{
...
struct Block33 : public TzxBlock
{
	uint8   cnt;
	...
	Block33()				:TzxBlock(0x33,no),data(NULL){}
	~Block33()				{ delete[] data; }
	void read(FD&);
	void write(FD&)			const;
};

void Block33::read(FD& fd)
{
	XXLogIn("Block 0x33: hardware type");
	XXXASSERT(!data);
...
void Block33::write(FD& fd) const
{
	fd.write_uint8(cnt);
...
*/
//static
TzxBlock* TzxBlock::read_next(FD &fd) throw(data_error, file_error)
{
...
a:  if(fd.file_remaining()==0) return NULL;
	...
	switch(id)
	{
	case 0x10:  block = new Block10; break;
	case 0x11:  block = new Block11; break;
	case 0x12:  block = new Block12; break;
	case 0x13:  block = new Block13; break;
	case 0x14:  block = new Block14; break;
	case 0x15:  block = new Block15; break;
	...
	case 0x18:  block = new Block18; break;
	case 0x19:  block = new Block19; break;
	case 0x20:  block = new Block20; break;
	case 0x21:  block = new Block21; break;
	case 0x22:  block = new Block22; break;
	case 0x23:  throw data_error("unsupported tzx block: 0x23 JUMP");   // tape probably won't load
	case 0x24:  block = new Block24; break;
	case 0x25:  block = new Block25; break;
	...
	case 0x2A:  block = new Block2A; break;
	case 0x2B:  block = new Block2B; break;
	case 0x30:  block = new Block30; break;
	case 0x31:  block = new Block31; break;
	case 0x32:  block = new Block32; break;
	case 0x33:  block = new Block33; break;
	...
	default:    if(id>=0x10&&id<0x60) { fd.skip_bytes(fd.read_uint32_z()); return NULL; }  // skip tzx 1.30++ block
				throw data_error(catstr("ill. tzx block number: 0x",hexstr(id,2)));
	}

	block->read(fd);

	if(id==0x30 && eq(lowerstr(leftstr(((Block30*)block)->text,8)),"created ")) // "Created with bla bla"
	{ delete block; goto a; }
...
/*  read block from tzx file:
	Lese einen TapeData-Block aus einem Tzx-File.
	...
	read next TZX block and all following blocks until EOF or an "ending" block is found
		ending block: stopper=0: is_end_block() else block.id==stopper
	all blocks are linked in a list via "next".
	returns NULL at eof
*/
TzxBlock* TzxBlock::readFromFile( FD& fd, uint stopper ) throw(data_error, file_error)
{
...
//TzxBlock* TzxBlock::readFromFile(FD& fd, bool& phase ) throw(data_error, file_error)
//{
//	TzxBlock* block = readFromFile(fd,0);
//	if(!block) return NULL;

//	if(!block->setsPhase0() && block->hasData()) block = &(*new Block2B(phase) + block);
//	phase = block->nextPhase(phase);
//	return block;
//}

void TzxBlock::writeToFile( FD& fd ) const throw(data_error)
{
...
cstr TzxBlock::getBlockInfo() const
{
	for(TzxBlock const* p = this; p; p=p->next)
	{
...
bool TzxBlock::hasData() const
{
	for(TzxBlock const* p=this; p; p=p->next)
	{
		if(p->has_data) return yes;
	}
	return no;
}
...
*/
Block10::Block10(TapData const& q)
:
	TzxBlock(0x10,yes),
	data(NULL)
{
...
/*	convert TapData to Block11: turbo speed tape block
	...
	• "next level" after this block is 0 (as after TzxPause)
*/
Block11::Block11(TapData const& q)
:
	TzxBlock(0x11,yes),
	data(NULL)
{
...
*/
Block18::Block18(CswBuffer const& bu, bool last_pulse_is_pause)
:
	TzxBlock(0x18,yes),
	...
	data(NULL)
{
	...
	if(last_pulse_is_pause)
	{
		...
		pause_ms = (uint16) minmax(1000.0, (double)pause_cc/cc_per_ms+0.5, 10000.0);
	...
	XXXASSERT(z-data == blen-10);
}



/*	convert O80Data to TzxBlock
	• the converted signal is prepended with 1/4 sec low pulse
	...
	• "next level" after this block is 0 (as after TzxPause)
*/
Block19::Block19( O80Data const& q )
:
	TzxBlock(0x19,yes),
	p_symdef(NULL),
	p_data(NULL),
	...
	d_data(NULL)
{
	...
	p_data[1] = 25;				// repeat.lo		25 * 1/100 sec = 1/4 sec
	p_data[2] = 0;				// repeat.hi
	...
	memcpy(d_symdef[0].data, qdata+10*sizeof(uint16), 9*sizeof(uint16));
...
/*	convert TapData to TzxBlock
	• the TzxBlock is always saved with not inverted polarity:
	  first block is a Block2B 'set polarity' which behaves as in new CswBuffer(TapData)
	  assuming an initial phase 0 this should be a nop for ZX Spectrum and a toggle for Jupiter Ace.
	• creates slightly different blocks for ZX Spectrum or Jupiter Ace:
	  • main block for ZX Spectrum is a Block10, for Jupiter Ace it's a Block11
	  • the Ace adds an additional final pulse (903cc@3.25MHz),
	    therefore the pause itself has phase 0, then the "next phase" is set to 0 as well by pause
	    and will be fixed with the initial Block2B 'set polarity' in the next block
	• "next phase" after this block is 0 (as after TzxPause).
	  there is at least 1ms pause at end of this block.
*/
TzxBlock* newTzxBlock(TapData const& q)
{
	Block2B* pol = new Block2B(q.pilot_pulses&1^1);	// block 2B: set polarity: even|odd => start with hi|lo pulse

	if(q.isJupiter())
	{
		Block11* dat = new Block11(q);	// allows using the slightly different timing
		dat->pause_ms = 0;				// no pause at end of this block
		Block12* one = new Block12(903*350/325,1);	// final pulse
		Block20* gap = new Block20((uint16) minmax(1000.0, q.pause*1000 + (0.5-903/3250.0), 10000.0));	// pause
		return &(*pol + dat + one + gap);
	}
	else	// ZX Spectrum or unknown:
	{
		Block10* dat = new Block10(q);	// standard tape block
		return &(*pol + dat);
	}
}
...
*/
TzxBlock* newTzxBlock(O80Data const& q)
{
	return new Block19(q);
}


/*	convert CswBuffer -> TzxBlock
	...
	• if num_pulses = 0 then a Block20 'pause' with pause=0ms is returned which means 'stop the tape'.
	  The caller SHOULD TEST FOR AN EMPTY BUFFER before converting it into a TzxBlock to avoid this condition.
	• TODO: handle TzxConversionStyle
*/
TzxBlock* newTzxBlock(CswBuffer const& bu, TzxConversionStyle)
{
	XXXASSERT(bu.getTotalSamples()>0);
	XXXASSERT(bu.is_normalized());

	Block2B* pol = new Block2B(bu.getPhase0());
	Block18* dat = new Block18(bu,yes);

	if(dat->num_pulses) return &(*pol + dat);

	Block20* gap = new Block20(dat->pause_ms);
	delete dat;
	return &(*pol + gap);
}
