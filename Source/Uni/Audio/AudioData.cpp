...
*/
static
void classify_audio( int16 const* samples, uint32 count, uint32 samples_per_second, Array<AudioRange>& ranges )
{
	...
	for(p=samples+1; p<e && *p >= *(p-1)+mindelta; p++) { if(*p > +silence_level) { p=samples; goto a; } }
	for(p=samples+1; p<e && *p <= *(p-1)-mindelta; p++) { if(*p < -silence_level) { p=samples; goto a; } }
	...
	while(p<e)
	{
	a:	const int16* start = p;					// start of "data" block
		const int16* p0 = p;					// p0 = start of pulse
		uint32 n = 0;							// num. pulses

	b:	if(*p>0) while(p<e && *p> 0) { p++; }	// find zero crossing
		else	 while(p<e && *p<=0) { p++; }	// find zero crossing

		if(p <= p0+max_samples_for_pulse) { n++; p0 = p; if(p<e) goto b; }	// a pulse
		...
		if(*p0>0) while(p<e && *p >= -silence_level) { p++; }
		else	  while(p<e && *p <= +silence_level) { p++; }
...
/*	Copy Creator
*/
AudioData::AudioData(AudioData const& q)
:
	TapeData(isa_AudioData),
...
*/
AudioData::AudioData(TapeData const& q)
:
	TapeData(isa_AudioData),
	stereo(no),
	samples_per_second(0),
	...
	adc_end_pos()
{
	...
	{
	case isa_AudioData:
		new(this) AudioData( reinterpret_cast<AudioData const&>(q) );
		break;
	default:
		new(this) AudioData( CswBuffer(q,3500000), ::samples_per_second );
		break;
	}
}
...
*/
AudioData::AudioData(CswBuffer const& q, uint32 sps )
:
	TapeData(isa_AudioData),
	stereo(no),
	...
	cc_offset(0)
{
	...
	if((count=qa.int16_samples.count()))		// int16_samples[] exists:
	{
		if(qa.stereo)
		{
			...
			int16 const* q = qa.int16_samples.getData();
			for(int16 *z=samples, *ze=z+count; z<ze;) { int16 v = *q++; *z++ = (v>>1) + (*q++>>1); }
		}
	...
	else if((count=qa.float_samples.count()))	// float_samples[] exists:
	{
		if(qa.stereo)
		{
			count = count / 2;				// frame_count = sample_count/2
			samples = new int16[count];
			Sample const* q = qa.float_samples.getData();
	...
	CswBuffer(ccps,0,666)
{
	...
	*/
	...
	// (i don't know whether this test ever succeeds.)
	for(i=1; i<count && samples[i] >= samples[i-1]+mindelta; i++) { if(samples[i] > +silence_level) goto a; }
	for(i=1; i<count && samples[i] <= samples[i-1]-mindelta; i++) { if(samples[i] < -silence_level) goto a; }
...
a:	i = 0;
	...
	while(i<count)
	{
		...
		{
			if(neg) while(i<count && samples[i] <= +silence_level) { i++; }
			else	while(i<count && samples[i] >= -silence_level) { i++; }
...
//				{		// das erfasst auch den Fall, dass s1<s0 oder gar s1<0, also ein Puls mit nur 1 Sample
//					e0 = 0;
//				}
			}
...
void AudioData::readFile(cstr fpath, TapeFile& tapeblocks) noexcept(false) // file_error,data_error,bad_alloc
{
	...
	for(uint i=0; i<ranges.count(); i++)
	{
			...
			csw->getTotalPulses() >= tfd->tapdata->csw_pause + 666 )		 // kommen danach noch sehr viele Pulse?
		{
			...
			TapData tapdata(csw->getData()+csw_splitposition,			// Versuche, den Remainder auch in einen
					csw->getTotalPulses()-csw_splitposition, ccps);		// TapData-Block zu konvertieren
