...
*/
void SpectraRenderer::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8* attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase, uint32 cc_vbi )
{
	...
	uint32 cc_end_of_visible_screen	  = min(cc_vbi,cc_start_of_visible_screen + height * cc_per_scanline - cc_row_flyback);
			...
			while(p<ee)
			{
				...
				// draw screen row:
					...
					while(p<e)
					{
						...
						{
							if(flashphase)
							{	if(attr1&0x80) { if(attr2&0x80) pixels ^= 0xff; else pixels = 0x00; }
							...
							if(video_mode&EXTRA_COLOURS)	// 2-byte attr, extra colors
							{
								...
								if(video_mode&HALFCELLMODE) { for(;m>15;m=m>>1) { *p++ = pixels&m ? color2 : black; } for(;m;m=m>>1) { *p++ = pixels&m ? color1 : black; } }
								else for(;m;m=m>>1) { *p++ = pixels&m ? color1 : color2; }
							}
							else	// 2-byte attr, standard colors
							{
								color1 = standard_rgba_colors[(attr1&7) + ((attr1>>3)&8)];	// fullcell: pen; halfcell: penR
								color2 = standard_rgba_colors[(attr2&7) + ((attr2>>3)&8)];	// fullcell: pap; halfcell: papR
								color3 = standard_rgba_colors[(attr1>>3)&15];				//				  halfcell: penL
								color4 = standard_rgba_colors[(attr2>>3)&15];				//				  halfcell: papL

								if(video_mode&HALFCELLMODE) { for(;m>15;m=m>>1) { *p++ = pixels&m ? color3 : color4; } for(;m;m=m>>1) { *p++ = pixels&m ? color1 : color2; } }
								else for(;m;m=m>>1) { *p++ = pixels&m ? color1 : color2; }
							}
						}
						else	// 1-byte attr
						{
							...
							{																//				  halfcell: pap=black
								color1 = enhanced_rgba_colors[attr1&0x3F];					// fullcell: pen; halfcell: penR
								color2 = attr1&0x40 ? standard_rgba_colors[7] : black;		// fullcell: pap; halfcell: penL
							}
		...
		bordercolor = video_mode & ENHANCED_BORDER								// enhanced border?
			? video_mode & EXTRA_COLOURS										//  enhanced border: extra colours?
				...
				: (borderbyte&0x80) && flashphase								//   enhanced border, basic colours: flash?
					...
					: standard_rgba_colors[(borderbyte&7) + ((borderbyte>>3)&8)]//    not flashing: 8 basic colours bright or dimm
...
void SpectraGifWriter::drawScreen(IoInfo* ioinfo, uint ioinfo_count, uint8 *attr_pixels, uint cc_per_scanline, uint32 cc_start_of_screenfile, bool flashphase )
{
	assert((cc_start_of_screenfile&3) == 0);
					...
					for(e+=screen_width; p<e;)
					{
						...
						if(video_mode&DOUBLE_BYTE_COLOUR)
						{
							...
							if(video_mode&EXTRA_COLOURS)	// 2-byte attr, extra colors
							{
								...
								if(video_mode&HALFCELLMODE) { for(;m>15;m=m>>1) { *p++ = pixels&m ? color2 : 0/*black*/; } for(;m;m=m>>1) { *p++ = pixels&m ? color1 : 0/*black*/; } }
								else for(;m;m=m>>1) { *p++ = pixels&m ? color1 : color2; }
							}
							else	// 2-byte attr, standard colors
							{
								color1 = basic_colors[(attr1&7) + ((attr1>>3)&8)];	// fullcell: pen; halfcell: penR
								color2 = basic_colors[(attr2&7) + ((attr2>>3)&8)];	// fullcell: pap; halfcell: papR
		...
		bordercolor = video_mode & ENHANCED_BORDER						// enhanced border?
			? video_mode & EXTRA_COLOURS								//  enhanced border: extra colours?
				...
				: (borderbyte&0x80) && flashphase						//   enhanced border, basic colours: flash?
					...
					: basic_colors[(borderbyte&7) + ((borderbyte>>3)&8)]//    not flashing: 8 basic colours bright or dimm
