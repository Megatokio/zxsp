...
private:
	TapeFileDataBlock(const TapeFileDataBlock&)			   = delete;
	TapeFileDataBlock& operator=(const TapeFileDataBlock&) = delete;
...
public:
	...
	void setMajorBlockInfo(cstr s)
	{
		delete[] major_block_info;
		major_block_info = newcopy(s);
	}
	void setMinorBlockInfo(cstr s)
	{
		delete[] minor_block_info;
		minor_block_info = newcopy(s);
	}
	...
	{
		assert(cswdata);
		return cswdata->isAtStart();
	}
	bool isAtEnd()
	{
		assert(cswdata);
		return cswdata->isAtEnd();
	}
	bool isNearStart(Time proximity)
	{
		assert(cswdata);
		return cswdata->getCurrentTime() <= proximity;
	}
	bool isNearEnd(Time proximity)
	{
		assert(cswdata);
		return getTimeRemaining() <= proximity;
	}
	uint32 getCurrentCcPos()
	{
		assert(cswdata);
		return cswdata->getCurrentCc();
	}
	Time getCurrentTimePos()
	{
		assert(cswdata);
		assert(cswdata->ccps);
		return cswdata->getCurrentTime();
	}
	Time getTimeRemaining()
	{
		assert(cswdata);
		return cswdata->getTotalTime() - cswdata->getCurrentTime();
	}
	void seekTimePos(Time t)
	{
		assert(cswdata);
		assert(mode == stopped);
		cswdata->seekTime(t);
	}
	void seekCcPos(uint32 cc)
	{
		assert(cswdata);
		assert(mode == stopped);
		cswdata->seekCc(cc);
	...
	void stop(uint32 cc);
	bool input(uint32 cc)
	{
		assert(cswdata);
		assert(mode == playing);
		return cswdata->inputCc(cc);
	}
	void output(uint32 cc, bool b)
	{
		assert(cswdata);
		assert(mode == recording);
		cswdata->outputCc(cc, b);
	}
