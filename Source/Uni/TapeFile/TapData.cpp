...
struct Timing
{
	const uint32 ccps;			  // cpu cycles per second
	const uint32 cc_pilot;		  // cpu cycles per pilot half phase
	const uint32 cc_bit0;		  // cpu cycles per 0-bit half phase
	const uint32 cc_bit1;		  // cpu cycles per 1-bit half phase
	const uint32 cc_sync1;		  // cpu cycles for 1st sync phase
	const uint32 cc_sync2;		  // cpu cycles for 2nd sync phase
	const Time	 pause[2];		  // trailing pause after [0]=header: 1 sec; [1]=data: 2 sec
	const uint32 pilot_pulses[2]; // pilot pulses: [0]=8064 Header; [1]=3220 Data
...
*/
uint calcTapeBlockChecksum(const uint8* data, int blen, bool jupiter)
{
	const uint8* e	 = data + blen;
	uint		 sum = 0;
	if (jupiter) data++;
...
*/
bool isaJupiterHeaderBlock(const uint8* data, int blen)
{
...
bool isaZxSpectrumHeaderBlock(const uint8* data, int blen)
{
...
*/
cstr calcMajorTapBlockInfo(const uint8* data, int blen)
{
	...
	*d = 0;
	...
	if (is_jupiter) { return catstr(htyp ? "Binary data: " : "Dictionary: ", d == name ? "(unnamed)" : name); }
	else // ZX Spectrum
	{
		static const cstr typstr[] = {"Program: ", "Data: ", "Data: ", "Code: "};
...
*/
cstr calcMinorTapBlockInfo(const uint8* data, int blen)
{
	...
	if (btyp == 0x00)				  // else not a header block
		if (blen == 27 || blen == 19) // else long header
		{
			...
			{
				if (htyp == 0 /*prog*/)
					return usingstr(dpos > 9999 ? "length: %i bytes" : "%i bytes, starts at line %i", dlen, dpos);
...
*/
void calcTapBlockInfos(const uint8* data, int blen, cstr& major, cstr& minor)
{
...
*/
TapData::TapData(const TapeData& q) : TapeData(isa_TapData)
{
	switch (q.isaId())
	{
	case isa_TapData: new (this) TapData(static_cast<const TapData&>(q)); break;
	case isa_O80Data:
		new (this) TapData(no, no);
		trust_level = conversion_failed;
		break;
	case isa_TzxData: new (this) TapData(static_cast<const TzxData&>(q)); break;
 ...
 */
CswBuffer::CswBuffer(const TapData& q, uint32 ccps) :
...
*/
TapData::TapData(const CswBuffer& bu) : TapeData(isa_TapData)
{
	...
	csw_pause(0)
{
	...
	const Timing& t		   = timing[ccps == timing[1].ccps];
	...
	const uint pulses_per_pilot_sequence_min = 256;									 // min expected pilot pulses
	const uint cc_per_pilot_min				 = cc_pilot - (cc_pilot - cc_sync1) / 2; // ultimate limit
	const uint cc_per_sync1_max				 = cc_sync1 + (cc_pilot - cc_sync1) / 3; // ultimate limit
	const uint cc_per_sync12_max			 = (cc_sync1 + cc_sync2) * 3 / 2;		 // ultimate limit for sync1+sync2
	const uint cc_per_bit11_max				 = cc_bit1 * 2 * 11 / 9; // ultimate limit for 2 data bit pulses


	const uint16* q = bu;	  // running pulse pointer
	const uint16* e = q + sz; // end of pulses
...
p5:
	...
	if (!is_zxsp && !is_jupiter && data[0] != 0)
	{
		...
		else if (sum == data[0])
		{
			is_jupiter = true;
			sum		   = 0;
		} // correct checksum for Jupiter Ace
	}
...
void TapData::readFromFile(FD& fd, bool add_typebyte)
{
	...
	uint32 n = fd.read_uint16_z();
	if (n + add_typebyte < 2) n += 0x10000;
...
void TapData::writeFile(cstr fpath, TapeFile& data)
{
	...
	FD fd;
	fd.open_file_w(fpath); // throw file_error

	for (uint i = 0; i < data.count(); i++)
	{
		TapeFileDataBlock* db = data[i];
		db->tapdata			  = db->tapdata ? db->tapdata :
								db->o80data ? nullptr :
		...
		if (db && db->tapdata->trust_level >= truncated_data_error) db->tapdata->writeToFile(fd, no);
	}
}
...
void TapData::readFile(cstr fpath, TapeFile& data)
{
	...
	const Timing& t			   = timing[is_jupiter];
