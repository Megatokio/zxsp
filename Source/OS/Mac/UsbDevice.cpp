...
inline CFMutableDictionaryRef newMatchingDictForService(cstr service)
{
...
CFTypeRef usbSearchProperty(io_registry_entry_t registry_entry, QCFString property_key, uint recursive)
{
	...
	return IORegistryEntrySearchCFProperty(
		registry_entry, kIOUSBPlane /*kIOServicePlane*/, property_key, nullptr, recursive);
}
...
MyUSBDeviceInterface** newUSBDeviceInterfaceForDevice(io_service_t device)
{
	...
	HRESULT				   err =
		(*plugInInterface)
			->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(kIOUSBDeviceInterfaceID), (void**)&deviceInterface);
...
MyUSBInterfaceInterface** newUSBInterfaceInterfaceForInterface(io_service_t interface)
{
	...
	HRESULT					  err =
		(*plugInInterface)
			->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(kIOUSBInterfaceInterfaceID), (void**)&deviceInterface);
...
MyHIDDeviceInterface** newHIDDeviceInterfaceForService(io_service_t device)
{
	...
	HRESULT				   err =
		(*plugInInterface)
			->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(kIOHIDDeviceInterfaceID), (void**)&deviceInterface);
...
static cstr getDeviceName(io_registry_entry_t registry_entry, cstr* key)
{
...
static io_registry_entry_t parentService(io_registry_entry_t service)
{
...
static cstr usb_pipe_direction_str(uint direction)
{
	switch (direction)
	{
...
static cstr usp_pipe_transfertype_str(uint transferType)
{
	switch (transferType)
	{
...
void logString(CFTypeRef value) { logline("%s", quotedstr(toStr(reinterpret_cast<CFStringRef>(value)))); }
...
void logArray(CFTypeRef value)
{
	...
	logIn("Array[%u]", size);
	for (uint i = 0; i < size; i++) { logValue(CFArrayGetValueAtIndex(array, i)); }
}
...
void logDict(CFTypeRef value)
{
	...
	for (uint i = 0; i < size; i++)
		maxlen = max(maxlen, uint(CFStringGetLength(reinterpret_cast<CFStringRef>(keys[i]))));
...
IOReturn ConfigureDevice(MyUSBDeviceInterface** dev, uint configuration_idx)
{
	...
	err = (*dev)->GetNumberOfConfigurations(dev, &num_configurations);
	assert(!err);
	...
	if (err)
	{
		logline("GetConfigurationDescriptorPtr: error = 0x%08X", err);
		return err;
	}
	...
	if (err)
	{
		logline("SetConfiguration: error = 0x%08X", err);
		return err;
	}
...
int testProlific2305pipe(MyUSBDeviceInterface** dev_if, MyUSBInterfaceInterface** if_if, int pipe_idx)
{
	...
	err = (*if_if)->GetPipeProperties(if_if, pipe_idx, &direction, &number, &transferType, &maxPacketSize, &interval);
	if (err)
	{
		logline("GetPipeProperties(%d) error: 0x%08X", pipe_idx, err);
		return -1;
	}
	...
	if (err)
	{
		logline("GetInterfaceNumber error: 0x%08X", uint(err));
		return -1;
	}
...
int testProlific2305interface(MyUSBDeviceInterface** dev_if, MyUSBInterfaceInterface** if_if)
{
	...
	if (err)
	{
		logline("GetInterfaceClass error: 0x%08X", uint(err));
		return -1;
	}
	err = (*if_if)->GetInterfaceSubClass(if_if, &if_subclass);
	if (err)
	{
		logline("GetInterfaceSubClass error: 0x%08X", uint(err));
		return -1;
	}
	...
	if (err)
	{
		logline("GetInterfaceProtocol error: 0x%08X", uint(err));
		return -1;
	}
	...
	if (err)
	{
		logline("GetAlternateSetting error: 0x%08X", uint(err));
		return -1;
	}
	...
	if (err)
	{
		logline("USBInterfaceOpen error: 0x%08X", uint(err));
		return -1;
	}
	...
	if (err)
	{
		logline("SetAlternateSetting error: 0x%08X", uint(err));
		return -1;
	} // 0xE00002CD
...
x:
	...
	if (err)
	{
		logline("USBInterfaceClose error: 0x%08X", uint(err));
		r = -1;
	}
...
int testProlific2305device(io_service_t device)
{
	...
	if (err)
	{
		logline("USBDeviceOpen error: 0x%08X", uint(err));
		goto xx;
	}
	...
	if (err)
	{
		logline("GetNumberOfConfigurations error: 0x%08X", uint(err));
		goto x;
	}
	...
	for (uint config = 0; config < num_configs; config++)
	{
		...
		if (err)
		{
			logline("ConfigureDevice error: 0x%08X", uint(err));
			r = -1;
			continue;
		}
...
x:
	...
	if (err)
	{
		logline("USBDeviceClose: error 0x%08X", uint(err));
		r = -1;
	}
...
void printSomething(io_registry_entry_t interface)
{
	...
	//	if(err) logline("GetUSBDeviceInformation returned error 0x%08X",uint(err));	// -> 0xe000405f =
	// kIOUSBNoAsyncPortErr 	else    logDeviceInformationBits(info);
	...
	//	err = (*dev_if)->GetConfiguration(dev_if, &configNum);
	//	if(err) logline("GetConfiguration returned error 0x%08X", uint(err));	// -> 0xE000404E = Interface ref not
	// recognized 	else logline("Location ID: 0x%02X", uint(configNum));
...

	{
		...
		for (int pipeRef = 1; pipeRef <= interfaceNumEndpoints; pipeRef++)
		{
			...
			err = (*if_if)->GetPipeProperties(
				if_if, pipeRef, &direction, &number, &transferType, &maxPacketSize, &interval);
			if (err != kIOReturnSuccess)
			{
			...
			if (err)
			{
				...
				if (err != kIOReturnSuccess)
				{
					logline("AbortPipe returned error 0x%08X", uint(err));
					continue;
				}
			...
			message = "Hallo Kio !\n\r";
			err		= (*if_if)->WritePipe(
				if_if, pipeRef, (void*)message, strlen(message)); // 0xE00002ed device not responding
			if (err != kIOReturnSuccess)
			{
...
void showUSBPrinters()
{
	...
	for (io_registry_entry_t interface; (interface = IOIteratorNext(iter)); IOObjectRelease(interface))
	{
		...
		logDict(dict);
		bool ok;
		if (usbSearchShortIntProperty(interface, key_InterfaceClass, no, &ok) == 7 && ok) printSomething(interface);
	}
	IOObjectRelease(iter);
}
...
void showSerialDevices()
{
	...
	for (io_registry_entry_t service; (service = IOIteratorNext(iter)); IOObjectRelease(service))
	{
		...
		for (; service; service = parentService(service))
		{
			...
			if (v_ok && p_ok && portName && devicePath && deviceName && vendorName && serialNumber) break;
		}
...
cstr num_keys[] = {key_VendorID,		   key_ProductID,			key_DeviceClass,		 key_DeviceSubClass,
				   key_DeviceProtocol,	   key_DeviceMaxPacketSize, key_DeviceReleaseNumber, key_ManufacturerStrIdx,
				   key_ProductStrIdx,	   key_SerialNumberStrIdx,	key_NumConfigurations,	 key_InterfaceNumber,
	...
	for (io_service_t device; (device = IOIteratorNext(iter)); IOObjectRelease(device))
	{
		...
		for (;;)
		{
			...
			for (uint i = 0; i < NELEM(num_keys); i++)
			{
				...
				if (ok) logline("%s = 0x%04X", key, value);
			...
			for (uint i = 0; i < NELEM(str_keys); i++)
			{
				...
				if (value) logline("%s = %s", key, value);
				// else logline("%s NOT FOUND",key);
			}
		...
		/*
			...
			IOReturn (*GetConfigurationDescriptorPtr)(void *self, UInt8 configIndex, IOUSBConfigurationDescriptorPtr
		   *desc); IOReturn (*GetConfiguration)(void *self, UInt8 *configNum); IOReturn (*SetConfiguration)(void *self,
		   UInt8 configNum); IOReturn (*GetBusFrameNumber)(void *self, UInt64 *frame, AbsoluteTime *atTime); IOReturn
		   (*ResetDevice)(void *self); IOReturn (*DeviceRequest)(void *self, IOUSBDevRequest *req); IOReturn
		   (*DeviceRequestAsync)(void *self, IOUSBDevRequest *req, IOAsyncCallback1 callback, void *refCon); IOReturn
		   (*CreateInterfaceIterator)(void *self, IOUSBFindInterfaceRequest *req, io_iterator_t *iter); IOReturn
		   (*USBDeviceOpenSeize)(void *self); IOReturn (*DeviceRequestTO)(void *self, IOUSBDevRequestTO *req); IOReturn
		   (*DeviceRequestAsyncTO)(void *self, IOUSBDevRequestTO *req, IOAsyncCallback1 callback, void *refCon);
