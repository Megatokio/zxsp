...
void MemoryHexInspector::save_settings()
{
...
int MemoryHexInspector::height_for_rows(int n)
{
	return VERT_MARGINS + n*rh;
}

int MemoryHexInspector::rows_for_height(int h)
{
	return (h-VERT_MARGINS + rh/3) / rh;
}
...
void MemoryHexInspector::validate_bytes_per_row()
{
...
void MemoryHexInspector::validate_scrollposition()
{
	...
	while(scroll_offset > max) { scroll_offset -= bytes_per_row; updateAll(); }
	if(scroll_offset<0) { scroll_offset = 0; updateAll(); }
}

inline
QColor pen_color_for_corebyte(CoreByte cb)
{
...
{
	if(cb&cpu_break_rwx) { hex_view->setPenColor(pen_color_for_corebyte(cb)); hex_view->setAttributes(BOLD); }
	hex_view->printAt(row, show_words ? col&1 ? (col-1)/2*5 : col/2*5 +2 : col*3,hexstr(cb,2));
	if(cb&cpu_break_rwx) { hex_view->setPenColor(Qt::black); hex_view->clearAttributes(BOLD); }
...
*/
void MemoryHexInspector::print_rows(int row_a, int row_e, uint address, volatile const CoreByte* bytes, int cnt)
{
	assert(row_a<=row_e);
	assert(row_a>=0);
	assert(row_e<=rows);
...
//	assert(cnt >= (row_e-row_a-1)*bytes_per_row+1);
	if(cnt < (row_e-row_a-1)*bytes_per_row+1)												// kann beim Umschalten auf kleinere Page passieren
		logline("MemoryHexInspector::print_rows: cnt < (row_e-row_a-1)*bytes_per_row+1");	// die implementierung hier ist aber robust dagegen
	...
	// print hex:
		...
		for(int i=0;i<n;i++) if(bytes[i]&cpu_break_rwx) print_byte(row_a, i, bytes[i]);	// print the colored bytes

	// print ascii:
		for(int i=0;i<n;i++) as[i] = printablechar( bytes[i] );
		ascii_view->printAt(row_a,0,QLatin1String(as));
		ascii_view->setAttributes(INVERTED);
		for(int i=0;i<n;i++) if(bytes[i]&0x80) { ascii_view->printAt(row_a,i,printablechar(bytes[i]&0x7F)); }
...
void MemoryHexInspector::print_byte_seen_by_cpu(uint16 addr)
{
	...
	{
		const CoreByte* page = data_source==RamPages || data_source==AllRam
...
void MemoryHexInspector::step_left_in_hex()
{
	...
	if(show_words)
	{
		...
		{
			addr = addr+1 < uint32(data.size)
				 ? addr+1					// step left => high byte des selben wortes
				 : addr-2;					// sonst low byte des vorhergehenden wortes
		}
	}
	else							// bytes
	{
		if(addr) addr -= 1; else hex_edit_nibble ^= 1;
	}
}
...
void MemoryHexInspector::step_right_in_hex()
{
	...
	{
		if(addr+1<uint32(data.size)) addr += 1; else hex_edit_nibble ^= 1;
	}
}
...
inline
bool MemoryHexInspector::is_editing_in_hex()
{
	return hex_view->hasFocus();
}

inline
bool MemoryHexInspector::is_editing_in_ascii()
{
...
void MemoryHexInspector::show_cursor()
{
	...
	if(is_editing_in_hex()) show_hex_cursor(1); else
	if(is_editing_in_ascii()) show_ascii_cursor(1);
}
...
void MemoryHexInspector::hide_cursor()
{
	if(edit_flashphase)
	{
		if(is_editing_in_hex()) show_hex_cursor(0); else
		if(is_editing_in_ascii()) show_ascii_cursor(0);
		edit_flashphase = 0;
	}
}
...
void MemoryHexInspector::adjustMaxSizeDuringResize()
{
	...
	*/
	...
	setMaximumSize(
		...
		max(height(),height_for_rows(maxrows)));	// begrenze Höhe, aber nicht unter aktuelle Höhe, weil sonst die Fensteroberkante wandert...
}
...
void MemoryHexInspector::slotSetWordMode(bool words)
{
	...
	updateAll();
	setMinimumWidth(width_for_bytes(MIN_BYTES_PER_ROW));		// set limits => resize mit alter size auf queued connection => scheiße
	setMaximumWidth(width_for_bytes(MAX_BYTES_PER_ROW));
	emit signalSizeConstraintsChanged();
}
...
// update parent:
	...
	if(rows*bytes_per_row > int(displayed_data.count()))
		displayed_data.grow(rows*bytes_per_row);
	...
	volatile const CoreByte* newdata;
	if(data_source==AsSeenByCpu)
	{
	...
	{
		newdata = (data_source==RamPages||data_source==AllRam
				? machine->ram.getData()
				: machine->rom.getData()) + data.baseoffset + scroll_offset;
	}

// zeilen vor first_valid_line schreiben:
	volatile const CoreByte* np = newdata;
...
// register-highlight aktualisieren:
	...
	// remove old:
	print_byte_seen_by_cpu(pc); pc = regs.pc;
	print_byte_seen_by_cpu(bc); bc = regs.bc;
	print_byte_seen_by_cpu(de); de = regs.de;
	print_byte_seen_by_cpu(hl); hl = regs.hl;
	print_byte_seen_by_cpu(ix); ix = regs.ix;
	print_byte_seen_by_cpu(iy); iy = regs.iy;
	print_byte_seen_by_cpu(sp); sp = regs.sp;
...
// edit position aktualisieren:
	if(system_time >= edit_flashtime+0.35) { edit_flashtime+=0.35; edit_flashphase ^= 1; }
	if(is_editing_in_hex()) show_hex_cursor(edit_flashphase); else
	if(is_editing_in_ascii()) show_ascii_cursor(edit_flashphase);
...
// get the local coordinates inside hex and ascii view:
	...
	if(hex_view->rect().contains(lpos_hex,true))
	{
		...
		if(show_words) { if(x%5==4) goto X; else x = x/5*2 + (x%5<2); }
		else		 { if(x%3==2) goto X; else x = x/3;             }
		in_hex = yes;
	}
...
X:		QToolTip::showText(gpos, nullptr, ascii_view, QRect());
	...
	if(offset<data.size)
	{
		CoreByte byte = data_source==AsSeenByCpu
			? *machine->cpu->rdPtr(offset)
			: data_source==RomPages || data_source==AllRom
				? machine->rom[data.baseoffset+offset]
				: machine->ram[data.baseoffset+offset];

		cstr fmt = "$%04X";
		if(pageOffsetForCpuAddress(pc)==offset) fmt = "pc -> $%04X"; else
		if(pageOffsetForCpuAddress(sp)==offset) fmt = "sp -> $%04X"; else
		if(pageOffsetForCpuAddress(hl)==offset) fmt = "hl -> $%04X"; else
		if(pageOffsetForCpuAddress(de)==offset) fmt = "de -> $%04X"; else
		if(pageOffsetForCpuAddress(ix)==offset) fmt = "ix -> $%04X"; else
		if(pageOffsetForCpuAddress(iy)==offset) fmt = "iy -> $%04X"; else
		if(pageOffsetForCpuAddress(bc)==offset) fmt = "bc -> $%04X";
		...
		if(in_hex)
		{
			...
			if(byte&cpu_break_rwx) fmt = catstr(fmt,": breakpoints: ",binstr((byte&cpu_break_rwx)/cpu_break_x,"---","rwx"));
...
void MemoryHexInspector::mousePressEvent(QMouseEvent*e)
{
	...
	if(in_hex)
	{
		...
		if(show_words) { if(x%5==4) return; else x = x/5*2; }
		else		 { if(x%3==2) return; else x = x/3;   }
	}
	else //if(in_ascii)
	{
		x = (lpos_ascii.x()+mouse_x_offset) / cw;
	...
	CoreByte* byte2 = show_words && in_hex && (offset+1<data.size) ? (CoreByte*)dataReadPtrForOffset(offset+1) : byte1;
	...
	{
		hide_cursor();
		if(is_editing_in_hex()) { hex_edit_offset = offset; hex_edit_nibble = 0; }
...
void MemoryHexInspector::keyPressEvent(QKeyEvent* e)
{
	...
	X:		while(hex_edit_offset<uint32(scroll_offset)) { setScrollOffset(scroll_offset-bytes_per_row); }
			while(hex_edit_offset>=uint32(scroll_offset)+rows*bytes_per_row) { setScrollOffset(scroll_offset+bytes_per_row); }
			show_cursor();
			break;
		}
	}
	...
	//	case Qt::Key_Enter:
		...
		case Qt::Key_Escape:
			...
			break;
		case Qt::Key_Up:
			ascii_edit_offset -= bytes_per_row;
			goto Y;//break;
		case Qt::Key_Left:
			ascii_edit_offset -= 1;
			goto Y;//break;
	...
	Y:		if(int32(ascii_edit_offset) < 0) ascii_edit_offset=0;
			...
			while(ascii_edit_offset>=uint32(scroll_offset)+rows*bytes_per_row) { setScrollOffset(scroll_offset+bytes_per_row); }
			show_cursor();
			break;
		}
	}
