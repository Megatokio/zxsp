...
{
public:
VIR CoreByte const*	pointer (uint32 address) = 0;
...
public:
	...
	CoreByte const*	pointer (uint32 address)		{ return cpu->rdPtr(address); }
};
...
public:
	...
	uint32		baseaddress;
public:
				CoreDisass(CoreByte const* core, uint32 size, uint32 baseaddress)	:core(core),size(size),baseaddress(baseaddress){}
	CoreByte const*	pointer	(uint32 address)		{ return core+((address-baseaddress)%size); }
};
	...
	hex_edit_col(0)
{
	...
	case AllRom:
	case RomPages:		disass = new CoreDisass(machine->rom.getData()+data.baseoffset,data.size,data.baseaddress); break;
	case AllRam:
	case RamPages:		disass = new CoreDisass(machine->ram.getData()+data.baseoffset,data.size,data.baseaddress); break;
	}
...
// create child widgets:
	...
	bool f;
	f = connect(hex_view,&SimpleTerminal::focusChanged,this,&MemoryDisassInspector::slotFocusChanged);	  assert(f);
	f = connect(disass_view,&SimpleTerminal::focusChanged,this,&MemoryDisassInspector::slotFocusChanged); assert(f);
...
// toolbar:
	...
	f = connect(button_edit_mode,&QPushButton::toggled,this,&MemoryDisassInspector::slotSetEditMode);	assert(f);
	button_edit_mode->setFixedSize(3*W,H);
	button_edit_mode->setCheckable(1);

	button_breakpoint_r = new QPushButton("R",widget_edit_mode);
	f = connect(button_breakpoint_r,&QPushButton::toggled,this,&MemoryDisassInspector::slotSetBreakpointR);	assert(f);
	...
	f = connect(button_breakpoint_w,&QPushButton::toggled,this,&MemoryDisassInspector::slotSetBreakpointW);	assert(f);
	...
	f = connect(button_breakpoint_x,&QPushButton::toggled,this,&MemoryDisassInspector::slotSetBreakpointX);	assert(f);
...
int MemoryDisassInspector::height_for_rows(int n)
{
	return VERT_MARGINS + n*rh;
}

int MemoryDisassInspector::rows_for_height(int h)
{
...
void MemoryDisassInspector::validate_rows()
{
	limit(MIN_ROWS,rows,MAX_ROWS);
}

void MemoryDisassInspector::validate_disass_cols(int& n)
{
	limit(MIN_DISASS_COLS,n,MAX_DISASS_COLS);
}
...
inline bool MemoryDisassInspector::editing_in_hex()
{
	return hex_view->hasFocus();
}

inline bool MemoryDisassInspector::editing_in_disass()
{
...
{
	int row = displayed_data_row_for_address(addr); if(row==-1) return;
...
{
	int row = displayed_data_row_for_address(hex_edit_address); if(row==-1) return;
...
{
	int row = displayed_data_row_for_address(disass_edit_address); if(row==-1 || disass_edit_address!=displayed_data[row].address) return;
...
int MemoryDisassInspector::print_row(int row, int32 address)
{
	...
	int n = disass->opcodeLength(address);		assert(n>=1&&n<=4);
...
void MemoryDisassInspector::slotSetMemoryPage(int newpage)
{
	...
	delete disass; disass = new CoreDisass(&mem[data.baseoffset],data.size,data.baseaddress);
...
void MemoryDisassInspector::mousePressEvent(QMouseEvent*e)
{
	...
	if(edit_mode==EDITMODE_EDIT)
	{
		if(in_hex)
		{
			...
			if(hex_edit_address>=data.baseaddress+data.size) { hex_edit_address = data.baseaddress+data.size-1; hex_edit_col = 1; }
		}
		else
		{
	...
	else // if(edit_mode==EDITMODE_BREAKPOINTS)
	{
		...
		else // in disass
		{
			...
			else for(int i=0;i<opcodelen;i++) { *(CoreByte*)disass->pointer(address+i) |=  breakpoint_mask; }
		}
	}
}
...
void MemoryDisassInspector::keyPressEvent(QKeyEvent* e)
{
	...
	else	// editing_in_disass
	{
		...
		case Qt::Key_Down:
			if(next_opcode(disass_edit_address)<data.baseaddress+data.size) disass_edit_address = next_opcode(disass_edit_address);
			disass_edit_string = disass->disassemble(disass_edit_address);
			disass_edit_string_valid = assembles_ok(disass_edit_address,disass_edit_string);//yes;
			goto Y;
