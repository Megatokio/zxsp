...
DivIDEInspector::~DivIDEInspector()
{
...
void DivIDEInspector::mousePressEvent(QMouseEvent* e)
{
	...
	if(box_jumper_E.contains(p))
	{
		NVPtr<DivIDE>(divide())->setJumperE(!state.jumper_E);
	}
	if(box_nmi_button.contains(p))
	{
		NVPtr<Machine>(machine)->nmi();
	}
	if(box_eeprom.contains(p))
	{
		load_rom();
	}
...
void DivIDEInspector::updateWidgets()
{
	...
	if(!object) { xlogline("DivIDEInspector.updateWidgets: !item"); return; }
	...
	//state.led_green depends on power on, and an intermediate power off state during power cycling is not yet implemented
	...
	if(!eq(state.diskname,divide()->getDiskFilename()))
	{
		...
		update(box_module);
		module->setCursor( divide()->isDiskInserted() ?
			  QCursor( QPixmap(":Icons/mouse/arrow_up_black.png"),  10,1 )
...
void DivIDEInspector::fillContextMenu(QMenu* menu)
{
	...
	menu->addAction("Recent EEproms …")->setMenu(
			new RecentFilesMenu(RecentDivideRoms,	this, [=](cstr path){load_rom(path);}));
	...
	menu->addAction("Recent discs …")->setMenu(
			new RecentFilesMenu(RecentDivideDisks,	this, [=](cstr fpath){insert_disk(fpath);}));
