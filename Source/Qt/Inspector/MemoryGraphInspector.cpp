...
// resize:
	...
	xlogline("new MemoryGraphInsp resized acc. to bpr+rows to %i x %i",width_for_bytes(bytes_per_row),height_for_rows(rows));
...
// update graphics_view:
	...
	{
		const CoreByte* q = data_source==AllRom || data_source==RomPages ? &machine->rom[data.baseoffset] : &machine->ram[data.baseoffset];
		for(int n,r=0,a=scroll_offset; r<rows && a<data.size; r++,a+=n)
		{
			n = min(bytes_per_row, data.size-a);
