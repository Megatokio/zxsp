	...
	Inspector(w, mc, m->ula, "/Backgrounds/light-150-s.jpg")
{
	...
	values.top_rows				= 0;
	inputs.top_rows				= new_led("0", l60);
	values.screen_rows			= 192;
	inputs.screen_rows			= new_led("192", l60);
	values.bottom_rows			= 0;
	inputs.bottom_rows			= new_led("0", l60);
	values.screen_columns		= 256;
	inputs.screen_columns		= new_led("256", r60);
	values.bytes_per_row		= 0;
	inputs.bytes_per_row		= new_led("0", l60);
	values.cpu_cycles_per_row	= 0;
	inputs.cpu_cycles_per_row	= new_led("0", r60);
	values.frames_per_second	= 0.0f;
	inputs.frames_per_second	= new_led("0", l60);
	values.cpu_cycles_per_frame = 0;
	inputs.cpu_cycles_per_frame = new_led("0", r60);
	...
	if (ula()->isA(isa_UlaZxsp) && UlaZxspPtr(ula())->hasWaitmap())
	{
		...
		values.waitmap_offset = 0;
		inputs.waitmap_offset = new_led("0", l60);
		values.waitmap		  = 0x00;
		inputs.waitmap		  = new_led("0-0-0-0-0-0-0-0", r160);
	...
	values.ear_bit		= 0;
	inputs.ear_bit		= new_led("0", r20);
	values.frames_hit	= 100.0f;
	inputs.frames_hit	= new_led("100%", l60);
	...
	if (has_7ffd)
	{
		...
		values.port_7ffd		   = 0x00;
		inputs.port_7ffd		   = new_led("$00", l60);
		values.page_c000		   = 0;
		inputs.page_c000		   = new_led("page 0", r60);
		values.page_8000		   = 0;
		inputs.page_8000		   = new_led("page 0", r60);
		values.page_4000		   = 0;
		inputs.page_4000		   = new_led("page 0", r60);
		values.page_0000		   = 0;
		inputs.page_0000		   = new_led("page 0", r60);
		values.video_page		   = 0;
		inputs.video_page		   = new_led("page 0", r60);
		inputs.checkbox_mmu_locked = new QCheckBox("mmu port locked");
		if (has_1ffd)
		{
			values.port_1ffd		 = 0x00;
			inputs.port_1ffd		 = new_led("$00", l60);
			inputs.checkbox_ram_only = new QCheckBox("ram only");
			values.printer_strobe	 = 0;
			inputs.printer_strobe	 = new_led("0", r20);
			values.disc_motor		 = 0;
			inputs.disc_motor		 = new_led("0", r20);
		}
...
void UlaInsp::updateWidgets()
{
	...
	if (values.cpu_clock != machine->cpu_clock)
	{
		...
		cstr s = tostr(overdrive);
		if (strchr(s, 'e')) s = "0.000";
		s = leftstr(s, 5);
		if (lastchar(s) == '.') s = leftstr(s, 4);
