...
void GWidget::resizeEvent(QResizeEvent*) { resize_canvas(size()); }
...
MemoryGraphInspector::MemoryGraphInspector(QWidget* parent, IsaObject* item) : MemoryInspector(parent, item, MemGraph)
{
...
void MemoryGraphInspector::updateWidgets()
{
	...
	{
		CoreByte* q = data_source == AllRom || data_source == RomPages ? &machine->rom[data.baseoffset] :
...
void MemoryGraphInspector::update_tooltip()
{
	...
	if (offset >= data.size)
	{
		QToolTip::showText(gpos, NULL, graphics_view, QRect());
		graphics_view->x = -99;
		return;
	}
