	...
	case zx81:
		{
			...
			int const x0[] = {17+12,17+36,17+24,17};	// left border of leftmost key
...
//	case tk95:		// preliminary, as long as it uses the zxsp keyboard image due to lack of own keyboard image
	...
	case zxsp_i3:
		{
			...
			int const x0[]={20,45,33,8};	// x-Wert für 1. Taste einer Reihe (bei CSH: normalisiert auf Std-Tastenbreite)
			...
			if(spec==CSH) { x=8; w=49; }	// CAPS SHIFT ist breiter
	...
	case jupiter:
		{
			...
			int const i = 1;						// inset. else the inverting key box has unesthetic corners
			int const x0[]={32+i,54+i,41+i,19+i};	// x-Wert für 1. Taste einer Reihe
	...
	case tc2048:
		...
		// Komposit- und Sondertasten:
			switch(spec)
			{
			case CSH:	/* 1. CSH */	x=13;      y=37;   w=58;  goto x;	// 1. CAPS SHIFT Taste: breiter ((die linke))
			case CSH2:	/* 2. CSH */	x=70+45*9; y=37;   w=58;  goto x;	// 2. CAPS SHIFT Taste: zus. Taste ((die rechte))
			case ENT:	/* ENTER  */	x=47+45*9; y=37+39;w=81;  goto x;	// ENTER: breiter
			case SPC:	/* SPACE  */	x=115;     y=0;    w=316; goto x;	// SPACE: unter allen anderen Tasten und richtig breit
			case cSPC:	/* BREAK  */	x=70+45*8; y=37;   w=46;  goto x;	// BREAK: neue Komposittaste
			}

		// normale Taste:
			if (x>=5) goto xx;				// error
			if (y>=4) { y=7-y; x=9-x; }		// right side?  =>  y=0..3  x=5..9
			int const x0[]={25,47,36,13};	// x-wert für 1. taste einer reihe (bei CSH-Taste: extrapolierte Position)
			x = 45*x + x0[y];
			y = 39*y + 37;
			break;
		}
	...
	case zxplus2a_span:
		...
		// Komposit- und Sondertasten:
			switch(spec)
			{
			case EXT:/*EXTEND MODE*/	x=0+i;   y=80+i; w=f?61-2*i:71-2*i; goto x;
			case sO: /* ; */			x=40+i;  y=0+i;			 goto x;
			case sP: /* " */            x=80+i;  y=0+i;			 goto x;
			case sN: /* , */            x=460+i; y=0+i;			 goto x;
			case sM: /* . */			x=410+i; y=40+i;		 goto x;
			case cZ1: /*EDIT*/			x=f?60+i:70+i; y=80+i; w=f?51-2*i:41-2*i;	goto x;
			case cZ2: /*CAPS LOCK*/		x=90+i;  y=40+i;		 goto x;
			case cZ3: /*TRUE VIDEO*/	x=0+i;   y=160+i;		 goto x;
			case cZ4: /*INV VIDEO*/		x=40+i;  y=160+i;		 goto x;
			case cZ5: /*CURSOR LEFT*/	x=120+i; y=0+i;			 goto x;
			case cZ6: /*CURSOR DOWN*/	x=420+i; y=0+i;			 goto x;
			case cZ7: /*CURSOR UP*/		x=380+i; y=0+i;			 goto x;
			case cZ8: /*CURSOR RIGHT*/	x=160+i; y=0+i;			 goto x;
			case cZ9: /*GRAPHICS*/		x=60+i;  y=120+i;		 goto x;
			case cZ0: /*DELETE*/		x=0+i;   y=120+i; w=61-2*i;	 goto x;
			case cSPC:/*BREAK*/			x=480+i; y=160+i; w=61-2*i;	 goto x;
			case SSH2:/*2. SSH Taste*/	x=0+i;   y=0+i;			 goto x;
			case CSH2:/*2. CSH Taste*/	x=0+i;   y=40+i;  w=91-2*i;	 goto x;
			case CSH:					x=450+i; y=40+i;  w=91-2*i;  goto x;
			case SSH:					x=500+i; y=0+i;			 goto x;
			case SPC:					x=200+i; y=0+i;   w=181-2*i; goto x;
//			case ENT:					x=470+i; y=80+i;  w=81-2*i;  goto x;	// L-förmig!
			case ENT:					x=500+i; y=80+i;  h=81-2*i;  goto x;	// L-förmig!
			case L:		if(model==pentagon128)		  w=51-2*i;  break;
			}

		// normale Taste:
			if (x>=5) goto xx;				// error
			if (y>=4) { y=7-y; x=9-x; }		// right side?  =>  y=0..3  x=5..9
			int const x0[]={90+i,110+i,100+i,80+i};	// x-wert für 1. taste einer reihe (bei CSH-Taste: extrapolierte Position)
			x = 40*x + x0[y];
			y = 40*y + 40+i;
			break;
		}
	...
	default:				// error exit:
		{
			static bool f=1; if(f) { logline("KbdPanel:KeyRect: model %s not yet supported",zx_info[model].name); f=0; }
xx:			x=y=w=h=0;
			break;
		}
	}
...
QRegion KeyboardInspector::keyRegion( uint8 spec )
{
	...
	case zxplus2a:
	case zxplus2a_span:
		return regio.united(QRect(500-40+11, 201-80-41, 30, 41));

	case pentagon128:
		return regio.united(QRect(500+1-19, 201-(80+1)-41+2, 19, 41-2));
	}
	return regio;
}
...
QRect Tk90xKbdInsp::keyRect(uint8 spec)
{
	...
	if(spec==CSH) { x=22; w=44; }	// CAPS SHIFT ist breiter
...
QRect Tk85KbdInsp::keyRect(uint8 spec)
{
	...
	if(spec==CSH) { x1=9; w=47; }	// CAPS SHIFT ist breiter
...
// Zeile 1:
	...
	case cSPC:		x=X(x1,13); break;				 // BREAK

// Zeile 2:
	case cZ9:		y=y2-1; w=x2; break;			 // GRAPHICS
	case cZ7:		y=y2-1; x=X(x2,10); break;		 // UP
	case cZ6:		y=y2-2; x=X(x2,11); break;		 // DOWN
	case ENT:		y=y2-2; x=X(x2,12); w+=18; break;// ENTER

// Zeile 3:
	case EXT:		y=y3;   w=x3; break;			 // Extended mode
	case sO:		y=y3;   x=X(x3,9); break;		 // ;
	case cZ5:		y=y3-1; x=X(x3,10); break;		 // LEFT
	case cZ8:		y=y3-1; x=X(x3,11); break;		 // RIGHT
	case cZ0:		y=y3-1; x=X(x3,12); w+=9; break; // DELETE

// Zeile 4:
	case CSH:		y=y4-1; w=x4; break;			 // CAPS SHIFT
	case cZ2:		y=y4;   x=x4; break;			 // CAPS LOCK
	case sM:		y=y4;   x=X(x4,8);  break;		 // .
	case sN:		y=y4;   x=X(x4,9);  break;		 // ,
	case CSH2:		y=y4;   x=X(x4,10); break;		 // CAPS SHIFT 2
	case SSH:		y=y4-1; x=X(x4,11); w*=3; break; // Symbol shift

// Zeile 5:
	case SPC:		y=y5; x=105; w=321; break;		 // SPACE

// Keys with default width at original position:
	default:		if(col>=5) return QRect();
					if (row>=4) { row=7-row; col=9-col; }				// right side?  =>  y=0..3  x=5..9
					x = X(xx[row],col);
					y = yy[row] - col/5;
					break;
	}
...
uint8 KeyboardInspector::findKeyForPoint( QPoint where )
{
	...
	for( int j=0;j<8;j++ )
	{
		...
		if( spec==ENT ? keyRegion(spec).contains(where) : keyRect(spec).contains(where) )
			return spec;
	}
...
bool KeyboardInspector::event(QEvent* e)
{
	...
	if(e->type()==QEvent::ContextMenu && (QGuiApplication::keyboardModifiers()&Qt::META))
		return true/*handled*/;
