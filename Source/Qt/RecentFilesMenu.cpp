...
*/
RecentFilesList::RecentFilesList(cstr key) : key(key), fpaths(settings.value(key).toStringList())
{
	for (int i = 0; i < fpaths.count(); i++)
	{
		if (!is_file(fpaths.at(i).toUtf8().data())) fpaths.removeAt(i--);
	}
 ...
 */
void clearRecentFiles(ListId id) { recent_files.at(id)->clear_list(); }

/*	global function: Add file to recent files list[id]
 */
void addRecentFile(ListId id, QString fpath) { recent_files.at(id)->add_file(fpath); }
