...
SimpleTerminal::SimpleTerminal(QWidget*p, QString fontname, int fontsize, bool antialias)
:	QWidget(p)
{
...
void SimpleTerminal::set_colors()	// Helper
{
	...
	if(attr&INVERTED) { painter->setPen(paper); painter->setBackground(pen); painter->setBrush(pen); }
	else		      { painter->setPen(pen); painter->setBackground(paper); painter->setBrush(paper);}
}
...
void SimpleTerminal::setFont(QString fontname, int size, bool antialias)
{
	...
	font.setStyleStrategy(antialias ? QFont::ForceIntegerMetrics : (QFont::StyleStrategy)(QFont::ForceIntegerMetrics|QFont::NoAntialias));
...
void SimpleTerminal::scrollScreen(int n)
{
	if(n>0)		// up
	{
		...
		row -= n;        if(row<0)        blob = off;
		pushed_row -= n; if(pushed_row<0) pushed_blob = off;
		update();
	}
	else if(n<0)	// down
	{
		...
		row += n;		if(row>=rows) blob = off;
		pushed_row +=n;	if(pushed_row>=rows) pushed_blob = off;
		update();
	}
}
...
void SimpleTerminal::copyRect(int z_row, int z_col, int z_rows, int z_cols)
{
	...
	z_rows = min(z_rows,rows-row); z_rows = min(z_rows,rows-z_row);
	z_cols = min(z_cols,cols-col); z_cols = min(z_cols,cols-z_col);
...
void SimpleTerminal::print(QString s, int n)
{
	...
	{
		if(s.length()<cols) { print(s+s,n/2); if(n&1) print(s,1); }
		else while(n) print(s,n--);
	}
}
