...
Machine* MachineController::new_machine_for_model(Model model)
{
	...
	case pentagon128:
					return new MachinePentagon128(this);

	case zxplus_span:
					showAlert("spanisch Sinclair ZX Spectrum+: TODO");
					return new MachineZxsp(this,zxplus);

	case scorpion:	showAlert("Scorpion: TODO");
					return new MachineZxsp(this,zxsp_i3);
...
void MachineController::loadSnapshot( cstr filename )
{
	...
	try
	{
		...
		{
			if(!model_info->canAttachDivIDE()) machine = init_machine(zx128,0,s,j,r,yes);	// powered on & suspended
...
//			screen->removeAllOverlays();
		...
		{
			Model id = jupiter;
			if(model!=id) machine = init_machine(id,0,s,j,no,no);	// note: don't add rampack now: rampack will be added by loadAce(),
		...
		else if( eq(ext,".tzx")||eq(ext,".aiff")||eq(ext,".aif")||eq(ext,".aifc")||eq(ext,".wav")||
...
QAction* MachineController::
new_action( cstr icon, cstr title, const QKeySequence& key, std::function<void(bool)> fu, isa_id id )
{
...
QAction* MachineController::
new_action( cstr icon, cstr title, const QKeySequence& key, std::function<void()> fu, isa_id id )
{
...
void MachineController::create_actions()
{
	...
	separator			= new QAction(this); separator->setSeparator(true);
	separator2			= new QAction(this); separator2->setSeparator(true);
	separator3			= new QAction(this); separator3->setSeparator(true);
	...
	action_newMachine	= new_action("new-window.png",	"&New window",          QKeySequence::New,  [=]{new MachineController("");} );
	...
	action_recordMovie	= new_action("video.gif",		"Record Gif movie",     Qt::Key_P|Qt::ALT,  [=](bool f){record_movie(f);} );
	action_pwrOnReset	= new_action("power_button.gif","Power-On reset",       Qt::Key_R|Qt::SHIFT,[=]{power_reset_machine();} );
	action_reset		= new_action("reset_button.gif","Push reset",           NOKEY,              [=]{reset_machine();} );
	action_nmi			= new_action("nmi_button.gif",	"Push NMI",             Qt::Key_N|Qt::SHIFT,[=]{NVPtr<Machine>(machine)->nmi();} );
	action_suspend		= new_action("run-pause.png",	"Halt CPU",             Qt::Key_H|Qt::SHIFT,[=](bool f){halt_machine(f);} );
	action_stepIn		= new_action("arrow-dn.png",	"Step in",              Qt::Key_I|Qt::SHIFT,[=]{assert(machine->isSuspended()); NV(machine)->stepIn();} );
	action_stepOver		= new_action("run-r.png",		"Step over",            Qt::Key_S|Qt::SHIFT,[=]{assert(machine->isSuspended()); NV(machine)->stepOver();} );
	action_stepOut		= new_action("arrow-up.png",	"Step out",             Qt::Key_O|Qt::SHIFT,[=]{assert(machine->isSuspended()); NV(machine)->stepOut();} );
	action_enable_breakpoints = new_action(NOICON,		"Enable breakpoints",	Qt::Key_B|Qt::SHIFT,[=](bool f){enable_breakpoints(f);} );
	...
	action_showMachineImage = new_action(NOICON, "Machine image",	  Qt::Key_I,		  [=](bool f){toggle_toolwindow(machine,action_showMachineImage,f);}, isa_Machine );
	action_showMemHex		= new_action(NOICON, "Memory hexview",	  Qt::Key_M,		  [=](bool f){toggle_toolwindow(mem[0],action_showMemHex,f);}, isa_MemHex);
	action_showMemDisass	= new_action(NOICON, "Memory disassemble",Qt::Key_M|Qt::SHIFT,[=](bool f){toggle_toolwindow(mem[1],action_showMemDisass,f);}, isa_MemDisass);
	action_showMemGraphical	= new_action(NOICON, "Memory graphical",  Qt::Key_M|Qt::ALT,  [=](bool f){toggle_toolwindow(mem[2],action_showMemGraphical,f);}, isa_MemGraphical);
	action_showMemAccess	= new_action(NOICON, "Memory access",	  Qt::Key_M|Qt::META, [=](bool f){toggle_toolwindow(mem[3],action_showMemAccess,f);}, isa_MemAccess);

	// add external item:
	action_addKempstonJoy    = new_action("joystick-k.gif","Kempston joystick interface",  NOKEY, ADDITEM(isa_KempstonJoy));
	...
	action_addDktronicsDualJoy=new_action("joystick-2.gif","dk'tronics dual joystick",     NOKEY, ADDITEM(isa_DktronicsDualJoy));
	...
	action_addPrinterLprint3 = new_action("printer.gif",   "LPrintIII printer interface",  NOKEY, ADDITEM(isa_PrinterLprint3));
	action_addPrinterTs2040  = new_action("printer.gif",   "TS2040 printer",               NOKEY, ADDITEM(isa_PrinterTs2040));
	action_addMultiface1     = new_action("nmi_button.gif","Romantic Robots Multiface ONE",NOKEY, ADDITEM(isa_Multiface1));
	action_addMultiface128   = new_action("nmi_button.gif","Romantic Robots Multiface 128",NOKEY, ADDITEM(isa_Multiface128));
	...
	action_addSpectraVideo	 = new_action(NOICON, "SPECTRA video interface",		 NOKEY, [=](bool f){add_spectra_video(f);}, isa_SpectraVideo);

	action_gifAnimateBorder	 = settings.action_gifAnimateBorder;

	action_setKbdBasic       = new_action("mini_zxsp.gif", "Keyboard BASIC mode",	Qt::Key_B,  [=]{set_keyboard_mode(kbdbasic);});
	action_setKbdGame        = new_action("mini_zxsp.gif", "Keyboard game mode",	Qt::Key_G,  [=]{set_keyboard_mode(kbdgame);});
	action_setKbdBtZXKbd	 = new_action("mini_zxsp.gif", "Recreated ZXKeyboard game mode", Qt::Key_G|Qt::SHIFT, [=]{set_keyboard_mode(kbdbtzxkbd);});
	...
	action_RzxRecordAutostart= new_action(NOICON, "Switch to recording on any key",	NOKEY, [=](bool f){set_rzx_autostart_recording(f);});
	action_RzxRecordAppendSna= new_action(NOICON, "Append snapshots to RZX file",	NOKEY, [=](bool f){set_rzx_append_snapshots(f);});

	action_setSpeed100_50	 = new_action(NOICON, "100% 50Hz", Qt::Key_5, [=](bool f){if(f)nvptr(machine)->set50Hz();});
	action_setSpeed100_60	 = new_action(NOICON, "100% 60Hz", Qt::Key_6, [=](bool f){if(f)nvptr(machine)->set60Hz();});
	...
	speedGrp->addAction(action_setSpeed100_50); action_setSpeed100_50->setCheckable(1);
	speedGrp->addAction(action_setSpeed100_60); action_setSpeed100_60->setCheckable(1);
	speedGrp->addAction(action_setSpeed120); action_setSpeed120->setCheckable(1);
	speedGrp->addAction(action_setSpeed200); action_setSpeed200->setCheckable(1);
	speedGrp->addAction(action_setSpeed400); action_setSpeed400->setCheckable(1);
	speedGrp->addAction(action_setSpeed800); action_setSpeed800->setCheckable(1);

	QActionGroup* kbdGrp = new QActionGroup(this);
	kbdGrp->addAction(action_setKbdBasic);	 action_setKbdBasic->setCheckable(true);
	kbdGrp->addAction(action_setKbdGame);	 action_setKbdGame->setCheckable(true);
	kbdGrp->addAction(action_setKbdBtZXKbd); action_setKbdBtZXKbd->setCheckable(true);

	QActionGroup* zoomGrp = new QActionGroup(this);
	zoomGrp->addAction(action_zoom[0]);		action_zoom[0]->setCheckable(1);
	zoomGrp->addAction(action_zoom[1]);		action_zoom[1]->setCheckable(1);
	zoomGrp->addAction(action_zoom[2]);		action_zoom[2]->setCheckable(1);
	zoomGrp->addAction(action_zoom[3]);		action_zoom[3]->setCheckable(1);
	zoomGrp->addAction(action_fullscreen);	action_fullscreen->setCheckable(1);
}
...
void MachineController::create_menus()
{
	...
	file_menu->addActions( QList<QAction*>() // <-- void QCocoaMenu::insertNative … Menu item is already in a menu …
				<< action_newMachine << separator
				<< action_openFile << action_recentFiles << action_reloadFile << separator2
				<< action_closeWindow << action_saveAs << separator3
				<< action_screenshot << action_recordMovie << action_gifAnimateBorder << action_showAbout
				<< action_showPreferences );

	control_menu->addActions( QList<QAction*>()
				<< action_pwrOnReset << action_reset << action_nmi << separator
				<< action_enable_breakpoints << action_suspend << action_stepIn
				<< action_stepOver << action_stepOut );

	options_menu->addAction(action_audioin_enabled);
	options_menu->addMenu(speed_menu);

	options_menu->addActions( QList<QAction*>()
				<< separator << action_setKbdBasic << action_setKbdGame << action_setKbdBtZXKbd
				<< separator2 << action_RzxRecord << action_RzxRecordAutostart << action_RzxRecordAppendSna);

	window_menu->addActions( QList<QAction*>()
				<< action_minimize << action_zoom[0] << action_zoom[1] << action_zoom[2] << action_zoom[3]
				<< action_fullscreen << separator2 << action_showMachineImage );

	context_menu->addActions( QList<QAction*>() // <-- void QCocoaMenu::insertNative … Menu item is already in a menu …
				<< action_setKbdBasic << action_setKbdGame << action_setKbdBtZXKbd << separator
				<< action_openFile << action_recentFiles << action_reloadFile << action_saveAs << separator2
				<< action_newInspector << separator3
				<< action_zoom[0] << action_zoom[1] << action_zoom[2] << action_zoom[3] << action_fullscreen );

	memory_menu->addActions( QList<QAction*>()
				<< action_showMemHex << action_showMemDisass << action_showMemGraphical << action_showMemAccess );
...
void MachineController::startup_open_toolwindows()
{
	...
	QTimer::singleShot(0, [=]
	{
		...
		if(settings.get_bool(key_startup_open_machine_image,no))
		{
			action_showMachineImage->setChecked(yes);
		}
	...
	// menus:			  initialized in create_mbar()
{
	...
	Model model = filepath.isEmpty() ?
		settings.get_Model(key_startup_model, zxsp_i3) :
	...
	if(zoom)
	{
		...
		{
			resize( min(320*zoom,main_screen.width()), min(zoom*240,main_screen.height()) );
		}
...
init_machine(Model model, uint32 ramsize, bool alwaysAddAy, bool alwaysAddJoy, bool alwaysAddRam, bool alwaysAddDivide)
{
	...
	if(!zx_info[model].is_supported) { model = zxsp_i3; ramsize = 0; }
...
// and add AY, Joystick, more Ram etc. depending on flags:
	...
	{
		add_actions = QList<QAction*>()
			<< action_addZx3kRam
			<< action_addZx16kRam
			<< action_addTs1016Ram
			<< action_addStonechip16kRam
			<< action_addMemotech16kRam
			<< action_addMemotech64kRam
			<< action_addZonxBox81
			<< action_addZxPrinter
	...
	else if(model_info->has_zxsp_bus)
	{
		add_actions = QList<QAction*>()
			...
			<< action_addProtekJoy
			<< action_addZxIf2
			<< action_addKempstonMouse
			<< action_addZonxBox
			<< action_addDidaktikMelodik
			<< (model_info->has_port_1ffd ? action_addMultiface3 :
				model_info->has_port_7ffd ? action_addMultiface128 : action_addMultiface1)
			...
			<< action_addZxIf1
			<< action_addDivIDE
			<< action_addFdcBeta128
			<< action_addFdcD80
			<< action_addFdcJLO
			<< action_addFdcPlusD
			<< action_addZxPrinter
			<< action_addPrinterAerco
			<< action_addPrinterLprint3
			<< action_addPrinterTs2040;

		if(model_info->rom_size==16*1024) add_actions.append(action_addCurrahMicroSpeech); // only 48k machines work
		...
		if(alwaysAddDivide&&!machine->model_info->has_floppy_drive&&machine->model_info->canAttachDivIDE()) action_addDivIDE->setChecked(true);
...
//	if(machine->isA(isa_MachineZxsp))
		filter = "ZXSP Snapshots (*.z80 *.sna);;"				// <-- default
	...
	if(machine->isA(isa_MachineZx81))
		filter = "ZX81 Snapshots (*.z80 *.p *.81 *.p81);;"		// <-- default
				 "ZX80 Snapshots (*.z80 *.o *.80);;"
	...
	if(machine->isA(isa_MachineZx80))
		filter = "ZX80 Snapshots (*.z80 *.o *.80);;"			// <-- default
				 "ZX81 Snapshots (*.z80 *.p *.81 *.p81);;"
				 "ZXSP Snapshots (*.z80 *.sna);;"
				 "Jupiter Ace Snapshots (*.z80 *.ace);;"
				 "Discs (*.dsk);;"
	...
	if(model==jupiter)
		filter = "Jupiter Ace Snapshots (*.ace *.z80);;"		// <-- default
				 "ZXSP Snapshots (*.z80 *.sna);;"
				 "ZX80 Snapshots (*.z80 *.o *.80);;"
				 "ZX81 Snapshots (*.z80 *.p *.81 *.p81);;"
				 "Discs (*.dsk);;"
...
void MachineController::reload_file()
{
	...
	cstr path = dupstr(filepath ? filepath : getRecentFile(RecentFiles,0).toUtf8().data());	// note: filepath will be deleted by init_with_file()
...
void MachineController::save_as()
{
	...
	cstr filter = catstr(
			...
			model==zx80 ? "ZX80 Snapshots (*.z80 *.o *.80);;"
						...
						: machine->isA(isa_MachineZxsp) && machine->ram.count()<=48 kB ?
										   "ZXSP Snapshots (*.z80 *.sna);;"
						: "ZXSP Snapshots (*.z80);;",
			"Rom (*.rom);;",
...
void MachineController::keyPressEvent(QKeyEvent*e)
{
	...
	else
	{
		...
		if(machine && machine->keyboard)
			machine->keyboard->realKeyDown(charcode,keycode,zxmodifiers(modifiers));
	}
...
void MachineController::keyReleaseEvent(QKeyEvent*e)
{
	...
	else
	{
		...
		if(machine && machine->keyboard)
			machine->keyboard->realKeyUp(charcode,keycode,zxmodifiers(modifiers));
	}
...
void MachineController::set_window_zoom(int factor)
{
	...
	int w = (32+2*4)*8*factor, h = (24+2*3)*8*factor,
...
void MachineController::halt_machine(bool f)
{
	...
	if(f) machine->suspend(); else machine->resume();
...
static
QAction* find_action_for_item(QList<QAction*>& array, volatile const IsaObject* item)   // helper
{
	foreach (QAction* action, array) { if(action->data().toInt() == item->id) return action; }
...
void MachineController::itemAdded(Item* item)   // callback from Item c'tor
{
	...
	QTimer::singleShot(0, [=]()	// trick: queued connection for lambda
	{
		...
		Item* i = machine->lastItem(); while(i && i!=item) { i = i->prev(); }
		if(!i) return;
		...
		switch(g)
		{
		case isa_TapeRecorder:	showaction->setShortcut(Qt::CTRL|Qt::Key_T); showaction->setIcon(QIcon(":/Icons/tape.gif"));  break;
		case isa_Ula:			showaction->setShortcut(Qt::CTRL|Qt::Key_U); showaction->setIcon(QIcon(":/Icons/screen.gif"));  break;
		case isa_Keyboard:		showaction->setShortcut(Qt::CTRL|Qt::Key_K); showaction->setIcon(QIcon(":/Icons/mini_zxsp.gif"));  break;
		case isa_Ay:			showaction->setShortcut(Qt::CTRL|Qt::Key_Y); showaction->setIcon(QIcon(":/Icons/ay.gif"));  break;
	//	case isa_Mmu:			// TCC
		case isa_MassStorage:	// +3 internal Floppy Disc: MassStorage wg. Inspector Window Gruppe
		case isa_Fdc:			showaction->setShortcut(Qt::CTRL|Qt::Key_D); showaction->setIcon(QIcon(":/Icons/save.png")); break;
		...
		case isa_Joy:			showaction->setShortcut(Qt::CTRL|Qt::Key_J); showaction->setIcon(QIcon(JoyPtr(item)->getNumPorts()==1 ? ":/Icons/joystick-1.gif":":/Icons/joystick-2.gif"));  break;
		}
...
{
	QTimer::singleShot(0, [this]()	// trick: queuedConnection for lambda
	{
...
void MachineController::machineRunStateChanged() volatile	// callback from machine
{
	...
	QTimer::singleShot(0, [this]()	// trick: queued connection for lambda
	{
...
QAction* MachineController::findShowActionForItem(volatile const IsaObject* item)
{
	return find_action_for_item(show_actions,item);
}

bool MachineController::isRzxAutostartRecording() const volatile
{
	return action_RzxRecordAutostart->isChecked();
}
...
ToolWindow* MachineController::findToolWindowForItem(volatile const IsaObject* item)
{
