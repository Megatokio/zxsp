...
{
	for (uint i = 0; i < windows; i++)
	{
		if (w == windowlist[i]) return i;
	}
	IERR(); // not in list
}
...
WindowMenu::WindowMenu(QMainWindow* w) : QMenu(tr("Window"), w)
{
...
void WindowMenu::addWindow(QAction* w) { insertAction(separator, w); }

void WindowMenu::rmWindow(QAction* w) { removeAction(w); }
