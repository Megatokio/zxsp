...
inline void ReadInputData( AudioBufferList const* inInputData )
{
	...
	case 1:		// 1 buffer => assume interleaved channels
		{		//   1 channel: mono
			...
			{	cSample* q = (cSample*)inInputData->mBuffers[0].mData;
				while(z<e) { *z++ = *(StereoSample*)(q); q+=channels; }
			}	break;
			}
			break;
		}
...
{
	IFDEBUG( { static union{uint64 i;double d;}; i=0; assert(d==0.0); } )
...
inline void WriteOutputData( AudioBufferList* outOutputData )
{
	...
	default:	// multiple buffers => assume 1 channel per buffer
		...
		// copy audio_out_buffer to audio-out buffers (mono -> stereo):
			...
			while(q<e) { *z0++ = q->left*volume; *z1++ = q->right*volume; q++; }
	...
	case 1:		// 1 buffer => assume interleaved channels
		{		//   1 channel: mono
			...
			case 2:		// interleaved stereo ((99% of cases))
				while(q<e) { z[0] = q->left*volume; z[1] = q->right*volume; q++; z+=2; }
				break;

			default:	// e.g. 6 channels: dolby 5.1?  l/r channel seem to be buffer[0/1]
				while(q<e)
				{
					z[0] = q->left*volume; z[1] = q->right*volume; q++; z+=2;
	...
	try
	{
		if( (inInputData && inInputData->mNumberBuffers != 0) || !audio_input_device_present )
		{
			...
			if(inInputTime->mSampleTime-lasttime != DSP_SAMPLES_PER_BUFFER && lasttime!=0.0)
				logline("WARNING audio-in at sample %lu: Δ samples = %lu",
			...
			{ ReadInputData( inInputData ); if(0)HighpassInputBuffer(); }
			else ClearInputBuffer();
		}
		else // audio-out-only callback for systems with different audio-in and -out device IDs
		{
			static bool f=1;
			if(f) { f=0; logline("Dsp:audioDeviceIOProc: separate audio-in and -out callbacks."); }
		}
		...
		if( outOutputData && outOutputData->mNumberBuffers != 0 )
		{
			...
			if(inOutputTime->mSampleTime-lasttime != DSP_SAMPLES_PER_BUFFER && lasttime!=0.0)
				logline("WARNING audio-out at sample %lu: Δ samples = %lu",
			...
			{ if(0)HighpassOutputBuffer(); WriteOutputData( outOutputData ); }
			...
			#if XLOG
			{
				...
				if(n==NELEM(times))
				{
					...
					xlogline("audio-out interrupt: min %.3f msec, max %.3f msec, average %.3f msec",
						min*1000, max*1000, time);
				}
			}
			#endif
		...
		else
		{
			#if XLOG
			{
				...
				if(n==NELEM(times))
				{
					...
					xlogline("audio-in  interrupt: min %.3f msec, max %.3f msec, average %.3f msec",
						min*1000, max*1000, time);
				}
			}
			#endif
...
// abort on any problem with the output device
	...
	{
		showAlert ( "Audio output setup failed:\n%s.\n"
...
//		setPlaythrough( playthrough_mode );
	...
	catch( any_error& e )
	{
		if(settings.get_bool(key_warn_if_audio_in_fails,yes))
		{
			...
			showWarning("Audio input setup failed:\n%s.\n"
...
/*	Enable / Disable Audio Input Device
*/
...
void setOutputVolume ( Sample volume )
{
	if( volume <= 0.0 )
	{
		audio_output_device_enabled = 0/*off*/;
	}
	else
	{
		audio_output_volume = fmin(volume,(Sample)1.0);
		audio_output_device_enabled = 1/*on*/;
	}
}


/*	store samples in audio_out_buffer:
*/
void outputSamples ( cStereoSample& sample, Time aa/*start [seconds]*/, Time ee/*end [seconds]*/ )
{
	...
	*pa -= sample * (aa-floor(aa));

	StereoSample* pe = audio_out_buffer + e;
	*pe += sample * (ee-floor(ee));
